...
class OperatorWebService:
    ...
    def _observability_snapshot(self) -> dict[str, Any]:
        ...
        runtime_events = _read_jsonl_file(runtime_event_log_path)
        ...
        return {
            ...
            "directive_completion_evaluation": dict(controller_summary.get("directive_completion_evaluation", {})),
    ...
    def render_timeline_page(self) -> str:
        ...
        body = (
            ...
            f"<p><strong>Runtime event log:</strong> {self._render_path_html(str(snapshot.get('runtime_event_log_path', '')))}</p>"
            "<p>This timeline is rendered from the persisted JSONL runtime event ledger. It shows observable actions and artifact paths only, not hidden reasoning.</p>"
            "</section>"
    ...
    def render_cycle_summary_page(self) -> str:
        ...
        body = (
            ...
            f"<pre>{html.escape(_json_dump(work_summary or implementation_summary or artifact_index or {'status': 'no_cycle_summary_available'}))}</pre>"
            "</section>"
            "</div>"
        )
        return _render_page(
