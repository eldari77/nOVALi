...
def _select_latest_successful_frontier_circularity_artifact(preferred_path: Path | None) -> Path | None:
    pattern = "proposal_learning_loop_v4_governed_work_loop_governance_recommendation_frontier_circularity_boundary_audit_v1_*.json"
    ...
    for candidate in candidates:
        ...
        result = dict(execution_summary.get("frontier_circularity_result", {}))
        ...
        if (
            str(result.get("classification", "")) in {"circularity_bounded_with_material_delta", "circularity_bounded_without_material_delta"}
...
def _frontier_signal_summary(execution_summary: dict[str, Any]) -> dict[str, Any]:
    frontier_artifact = dict(execution_summary.get("frontier_circularity_artifact_produced", {}))
    audit_output = dict(frontier_artifact.get("governance_recommendation_frontier_circularity_boundary_audit_output", {}))
    ...
    return {
        ...
        "frontier_circularity_status": str(
            result.get("classification", summary.get("frontier_circularity_boundary_status", ""))
        ),
        ...
        "frontier_signal_count": int(
            usefulness.get(
                "circularity_signal_count",
                summary.get("circularity_signal_count", signal.get("signal_count", 0)),
            )
            or 0
        ),
        "material_frontier_signal_detected": bool(
            usefulness.get(
                "material_circularity_boundary_delta_detected",
                summary.get(
                    "material_circularity_boundary_delta_detected",
...
def _operational_usefulness_assessment(execution_summary: dict[str, Any]) -> dict[str, Any]:
    ...
    operationally_meaningful = (
        ...
        and signal_summary["frontier_circularity_status"] == "circularity_bounded_with_material_delta"
    ...
    return {
        ...
        "audit_signal_summary": {
            "frontier_circularity_status": signal_summary["frontier_circularity_status"],
        ...
        "reason": (
            "the continuation produced a real directive-supportive frontier-circularity artifact with a new material frontier-quality signal and no failed checks"
            if operationally_meaningful
            else "the continuation did not produce enough bounded frontier-circularity signal to count as an operationally meaningful governed work-loop continuation"
        ),
    }
...
def _continuation_value_assessment(execution_summary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "reason": (
            "the continuation added real bounded frontier-circularity value beyond the prior chain while staying cleanly separated from capability use, reopen, and new-skill paths"
...
def _distinctness_chain_assessment(
    *,
    ...
    continuation_v6_name = str(dict(prior_chain_state.get("continuation_v6", {})).get("loop_candidate_name", ""))
    continuation_v7_name = str(dict(current_execution_summary.get("candidate_executed", {})).get("loop_candidate_name", ""))
    unique_names = {
        name
        for name in [
            direct_work_name,
            ...
            continuation_v7_name,
        ]
        if name
    }
    v7_checks = dict(
        dict(
            dict(current_execution_summary.get("frontier_circularity_artifact_produced", {})).get(
                "governance_recommendation_frontier_circularity_boundary_audit_output", {}
            )
        ).get("checks", {})
    )
    v7_vs_chain = dict(dict(v7_checks.get("frontier_materially_distinct", {})).get("observed", {}))
    structurally_distinct = (
        bool(prior_distinctness.get("passed", False))
        and int(prior_distinctness.get("unique_step_name_count", 0) or 0) == 7
        and len(unique_names) == 8
        and bool(v7_vs_chain.get("distinct_from_direct_work", False))
        and bool(v7_vs_chain.get("distinct_from_continuation_v1", False))
        and bool(v7_vs_chain.get("distinct_from_continuation_v2", False))
        and bool(v7_vs_chain.get("distinct_from_continuation_v3", False))
        and bool(v7_vs_chain.get("distinct_from_continuation_v4", False))
        and bool(v7_vs_chain.get("distinct_from_continuation_v5", False))
        and bool(v7_vs_chain.get("distinct_from_continuation_v6", False))
        and bool(v7_vs_chain.get("distinct_from_evidence_snapshot_v2", False))
        and bool(v7_vs_chain.get("distinct_from_evidence_snapshot_v3", False))
        and bool(v7_vs_chain.get("distinct_from_evidence_snapshot_v4", False))
        and bool(v7_vs_chain.get("distinct_from_evidence_snapshot_v5", False))
        and bool(v7_vs_chain.get("distinct_from_evidence_snapshot_v6", False))
    )
    return {
        ...
        "step_names": {
            ...
            "continuation_v7": continuation_v7_name,
        },
        "unique_step_name_count": int(len(unique_names)),
        "vs_prior_direct_work": {
            "continuation_v7": bool(v7_vs_chain.get("distinct_from_direct_work", False)),
        },
        "vs_prior_continuation_v1": {
            "continuation_v7": bool(v7_vs_chain.get("distinct_from_continuation_v1", False)),
        },
        "vs_prior_continuation_v2": bool(v7_vs_chain.get("distinct_from_continuation_v2", False)),
        "vs_prior_continuation_v3": bool(v7_vs_chain.get("distinct_from_continuation_v3", False)),
        "vs_prior_continuation_v4": bool(v7_vs_chain.get("distinct_from_continuation_v4", False)),
        "vs_prior_continuation_v5": bool(v7_vs_chain.get("distinct_from_continuation_v5", False)),
        "vs_prior_continuation_v6": bool(v7_vs_chain.get("distinct_from_continuation_v6", False)),
        "vs_evidence_snapshot_v2": bool(v7_vs_chain.get("distinct_from_evidence_snapshot_v2", False)),
        "vs_evidence_snapshot_v3": bool(v7_vs_chain.get("distinct_from_evidence_snapshot_v3", False)),
        "vs_evidence_snapshot_v4": bool(v7_vs_chain.get("distinct_from_evidence_snapshot_v4", False)),
        "vs_evidence_snapshot_v5": bool(v7_vs_chain.get("distinct_from_evidence_snapshot_v5", False)),
        "vs_evidence_snapshot_v6": bool(v7_vs_chain.get("distinct_from_evidence_snapshot_v6", False)),
        "reason": (
            "direct work, continuation v1, continuation v2, frontier containment, frontier stability delta, frontier persistence boundary, frontier recursion boundary, and frontier circularity boundary are all measurably distinct bounded steps rather than diminishing-return governance recursion"
...
def _repeated_bounded_success_assessment(
    *,
    ...
    continuation_v6_success = bool(dict(prior_chain_state.get("continuation_v6", {})).get("operational_success", False))
    continuation_v7_success = (
    ...
    repeated_bounded_success = success_count == 8
    strengthened = repeated_bounded_success and prior_success_count == 7
    return {
        ...
        "step_success": {
            ...
            "continuation_v7": continuation_v7_success,
        },
        "reason": (
            "repeated bounded success is now stronger at the chain level because the eighth bounded execution step also executed cleanly"
            if strengthened
...
def _posture_discipline_assessment(
    *,
    ...
    return {
        ...
        "reason": (
            "narrow posture discipline is still holding cleanly across the full reviewed chain through the frontier-circularity boundary execution"
            if discipline_holds
...
def _new_execution_signal_assessment(execution_summary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "reason": (
            "the frontier-circularity execution adds a new frontier-quality signal about recursion-boundedness, drift absence, and bounded alignment rather than merely restating prior chain conclusions"
            if materially_improved
            else "the frontier-circularity execution mostly restates prior chain conclusions without adding enough new frontier-quality evidence"
        ),
    }
...
def _structural_vs_recursive_assessment(
    *,
    ...
    low_recursion_check = bool(
        dict(
            dict(
                dict(execution_summary.get("frontier_circularity_artifact_produced", {})).get(
                    "governance_recommendation_frontier_circularity_boundary_audit_output", {}
                )
            ).get("checks", {})
        )
...
def _gate_status_assessment(
    *,
    ...
    return {
        ...
        "reason": (
            "the future posture-review gate remains defined but closed after the frontier-circularity execution"
            if gate_closed
...
def run_probe(cfg, proposal, *, rounds, seeds):
    ...
    continuation_v6_execution_snapshot = r._load_latest_diagnostic_artifact_by_template(
    ...
    if not all(
        [
            ...
            continuation_v6_execution_snapshot,
    ...
    if (
        ...
        or not readiness.intersection(
            {
                ...
                "ready_for_work_loop_evidence_review_v7_later",
        ...
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: branch must remain paused_with_baseline_held, governed work-loop policy state must exist, and state must be at or beyond ready_for_work_loop_evidence_review_v7",
    ...
    current_execution_artifact_path = _select_latest_successful_frontier_circularity_artifact(
        _resolve_artifact_path(
            governed_work_loop_policy.get("last_work_loop_continuation_execution_artifact_path"),
            "proposal_learning_loop_v4_governed_work_loop_governance_recommendation_frontier_circularity_boundary_audit_v1_*.json",
        )
    )
    continuation_v6_execution_artifact_path = _resolve_artifact_path(
    ...
    if not all(
        [
            ...
            continuation_v6_execution_artifact_path,
        ...
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: expected governed work-loop v7, posture, direct-work, or capability-use artifact paths could not be resolved",
    ...
    continuation_v6_execution_payload = _load_json_file(continuation_v6_execution_artifact_path)
    ...
    if not all(
        [
            ...
            continuation_v6_execution_payload,
        ...
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: one or more required governed work-loop v7, posture, direct-work, or capability-use artifacts could not be loaded",
    ...
    candidate_screen_summary = dict(candidate_screen_payload.get("governed_work_loop_candidate_screen_v7_summary", {}))
    continuation_admission_summary = dict(
        continuation_admission_payload.get("governed_work_loop_continuation_admission_v7_summary", {})
    )
    ...
    prior_chain_evidence_summary = dict(
        prior_chain_evidence_payload.get("governed_work_loop_evidence_v6_summary", {})
    )
    current_execution_summary = dict(
        current_execution_payload.get("governed_work_loop_continuation_execution_summary", {})
    )
    continuation_v6_execution_summary = dict(
        continuation_v6_execution_payload.get("governed_work_loop_continuation_execution_summary", {})
    )
    ...
    if not all(
        [
            ...
            current_execution_summary,
            continuation_v6_execution_summary,
        ...
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: required governed work-loop v7, evidence v1, evidence v6, or direct-work summaries were missing from the loaded artifacts",
    ...
    future_posture_reason = (
        "the eight-step governed work-loop chain is still accumulating distinct structural evidence under the narrow envelope, so the safe result remains to hold posture narrow and keep broader execution closed"
    ...
    next_template = (
        "memory_summary.v4_governed_work_loop_candidate_screen_snapshot_v8"
        if next_action_class == "bounded_candidate_search_later_supported"
        else "memory_summary.v4_governed_work_loop_evidence_snapshot_v8"
    ...
    question_answers = {
        ...
        "new_circularity_boundary_audit_materially_improves_evidence_picture": str(
            new_execution_signal.get("classification", "")
        )
        == "materially_improved_evidence_picture",
        "repeated_bounded_success_now_stronger_than_evidence_v6": str(
    ...
    artifact_path = _diagnostic_artifact_dir() / f"memory_summary_v4_governed_work_loop_evidence_snapshot_v7_{proposal['proposal_id']}.json"
    ...
    updated_work_loop_policy["governed_work_loop_evidence_review_schema"] = {
        ...
        "schema_version": "governed_work_loop_evidence_review_v7",
        "required_fields": [
    ...
    updated_work_loop_policy["last_work_loop_evidence_outcome"] = {
        ...
        "status": "hold_narrow_posture_after_evidence_v7",
    ...
    updated_current_state_summary.update(
        {
            ...
            "governed_work_loop_evidence_review_v7_in_place": True,
            ...
            "latest_governed_work_loop_evidence_outcome": "hold_narrow_posture_after_evidence_v7",
            ...
            "latest_governed_work_loop_operational_status": (
                "eight_step_bounded_governed_work_loop_chain_reviewed"
                if bool(repeated_bounded_success.get("passed", False))
                else "eight_step_bounded_governed_work_loop_chain_not_yet_clear"
            ),
            "latest_governed_work_loop_execution_outcome": str(
                dict(current_execution_summary.get("frontier_circularity_result", {})).get("classification", "")
            ),
            "latest_governed_work_loop_readiness": (
                "ready_for_work_loop_candidate_screen_v8_later"
                if next_action_class == "bounded_candidate_search_later_supported"
                else "ready_for_work_loop_evidence_review_v8_later"
                if next_action_class == "bounded_evidence_review_later_supported"
    ...
    chain_state = {
        ...
        "continuation_v6": {
            **dict(prior_chain_evidence_summary.get("chain_state_reviewed", {})).get("continuation_v6", {}),
        },
        "continuation_v7": {
            "loop_candidate_name": str(candidate_reviewed.get("loop_candidate_name", "")),
            ...
            "frontier_circularity_boundary_status": str(
                dict(current_execution_summary.get("frontier_circularity_result", {})).get("classification", "")
            ),
            "frontier_drift_assessment": str(
                dict(current_execution_summary.get("frontier_circularity_result", {})).get(
    ...
    evidence_inputs_used = {
        ...
        "governed_work_loop_candidate_screen_v7": str(candidate_screen_artifact_path),
        "governed_work_loop_continuation_admission_v7": str(continuation_admission_artifact_path),
        ...
        "governed_work_loop_evidence_v6": str(prior_chain_evidence_artifact_path),
        ...
        "governed_work_loop_continuation_execution_v6": str(continuation_v6_execution_artifact_path),
        "governed_work_loop_continuation_execution_v7": str(current_execution_artifact_path),
    ...
    operator_readable_conclusion = (
        "The eight-step governed work-loop chain is still accumulating distinct structural evidence under the narrow envelope; the frontier-circularity execution materially improved the evidence picture while circularity risk remains low, so the safe next move is another bounded candidate search later while keeping the gate closed."
        if next_action_class == "bounded_candidate_search_later_supported"
        else "The frontier-circularity execution stayed clean and bounded, but diminishing-return or circularity caution now argues for another evidence-only review later rather than another candidate search."
        if next_action_class == "bounded_evidence_review_later_supported"
    ...
    ledger_event = {
        "event_id": f"governed_work_loop_evidence_snapshot_v7::{proposal['proposal_id']}",
        "timestamp": _now(),
        "event_type": "governed_work_loop_evidence_snapshot_v7_materialized",
        ...
        "artifact_paths": {
            ...
            "governed_direct_work_execution_v1": str(direct_work_execution_artifact_path),
            "governed_capability_use_evidence_v1": str(capability_use_evidence_artifact_path),
            "governed_work_loop_evidence_v7": str(artifact_path),
        },
    ...
    artifact_payload = {
        ...
        "comparison_references": {
            ...
            "memory_summary.v4_governed_work_loop_evidence_snapshot_v6": _artifact_reference(prior_chain_evidence_snapshot, latest_snapshots),
            ...
            "proposal_learning_loop.v4_governed_work_loop_governance_recommendation_frontier_recursion_boundary_audit_v1": _artifact_reference(continuation_v6_execution_snapshot, latest_snapshots),
            "proposal_learning_loop.v4_governed_work_loop_governance_recommendation_frontier_circularity_boundary_audit_v1": _artifact_reference(current_execution_snapshot, latest_snapshots),
        ...
        "governed_work_loop_evidence_v7_summary": {
            "snapshot_identity_context": {
                "template_name": "memory_summary.v4_governed_work_loop_evidence_snapshot_v7",
                "proposal_id": str(proposal.get("proposal_id", "")),
                "evaluation_semantics": str(proposal.get("evaluation_semantics", "")),
            ...
            "recommended_next_action": {
                ...
                "reason": (
                    "the eight-step chain is still structurally yielding under the narrow envelope without clear circularity pressure, so another bounded candidate search later is the cleanest next move"
                    if next_action_class == "bounded_candidate_search_later_supported"
            ...
            "current_work_state_summary": {
                ...
                "continuation_v6_execution_summary_present": bool(continuation_v6_execution_payload),
                "continuation_v7_execution_summary_present": bool(current_execution_payload),
            ...
            "why_governance_remains_source_of_truth": {
                ...
                "reason": "the v7 evidence review is derived from directive, bucket, branch, self-structure, work-loop policy, work-loop posture, candidate-screen v7, continuation-admission v7, evidence v1, evidence v6, direct-work evidence, continuation executions, frontier-containment execution, frontier-stability execution, frontier-persistence execution, frontier-recursion execution, frontier-circularity execution, and capability-use evidence artifacts rather than from execution code",
            },
        },
        "analytics_context": {
        ...
        "observability_gain": {
            "passed": True,
            "reason": "the governed work-loop chain now has an explicit governance-owned v7 evidence review consolidating the frontier-circularity execution into an eight-step bounded operator-readable artifact",
            "artifact_paths": {
        ...
        "ambiguity_reduction": {
            "passed": True,
            "score": 0.99,
            "reason": "the v7 evidence review separates chain-level structural value, circularity-boundary signal, diminishing-return risk, circularity risk, posture pressure, and later bounded-next-step support in one grounded decision artifact",
        },
        "safety_neutrality": {
            "passed": True,
        ...
        "diagnostic_conclusions": {
            ...
            "new_circularity_boundary_audit_materially_changed_evidence_picture": bool(new_execution_signal.get("passed", False)),
    ...
    return {
        ...
        "shadow_contract": "diagnostic_probe",
        "proposal_semantics": "diagnostic",
        "reason": "diagnostic shadow passed: the eight-step governed work-loop chain now has a v7 evidence review that keeps posture narrow, keeps the gate closed, and explicitly tests whether diminishing returns or circularity are starting to appear",
