...
This handoff package is the current release-candidate shape for the `novali-v6` standalone handoff:

- package version: `novali-v6_rc07-standalone`
...
- `novali-v5` is now the frozen reference / operator surface baseline
- active development has moved to `novali-v6`
- `novali-v4` remains preserved historical context, not the primary live reference
...
- the home page now includes a stronger dashboard pulse with explicit system state, operator attention, current recommendation, and recent activity cards
- the operator surface now also leads with a clearer “what NOVALI just accomplished / why it matters / important outputs” layer so first-time viewers can understand the current result faster
- deeper views are now linked more intentionally from the dashboard so operators can move from top-level understanding into observability, workspace, timeline, cycle, and current artifact preview more quickly
- mission-level trusted-source policy is now explicit and artifact-backed: local-only when sufficient, indexed reuse before re-query, justified external re-query only when required, and operator review before aggressive escalation
- trusted-source escalation now also records explicit budget, retry, and cost-governance posture so operators can see when NOVALI stayed local, reused indexed knowledge, allowed bounded external use, or stopped for review because further retry was not worth it
- canonical rc07 handoffs include the bundled Docker image archive under `image/novali-v6-standalone.tar` together with `image/image_archive_manifest.json`
- canonical rc07 handoffs also include a bundled successor-improvement evidence set under `runtime_data/acceptance_evidence/successor_improvement/` together with `docs/SUCCESSOR_IMPROVEMENT_EVIDENCE.md`
- the browser home page now exposes a dedicated external trusted-source onboarding panel with provider base URL, session-only API credential input, non-secret validation artifacts, and packaged-runtime handshake readiness
...
19. If you need bounded external trusted-source access from the packaged runtime, use the `Trusted Sources` panel on the browser home page:
    ...
    - confirm the non-secret credential/provider/handshake/session-contract artifacts before relying on external evidence
