...
KEY_WORKSPACE_ARTIFACTS = {
    ...
    "artifacts/trusted_source_credential_status_latest.json",
    "artifacts/trusted_source_provider_status_latest.json",
    "artifacts/trusted_source_handshake_latest.json",
    "artifacts/trusted_source_session_contract_latest.json",
    "artifacts/trusted_source_response_template_latest.json",
    "artifacts/trusted_source_request_contract_latest.json",
    "artifacts/trusted_source_external_response_latest.json",
    "artifacts/trusted_source_knowledge_index_latest.json",
    "artifacts/trusted_source_knowledge_catalog_latest.json",
    "artifacts/trusted_source_reuse_decision_latest.json",
    "artifacts/trusted_source_reuse_summary_latest.json",
    "artifacts/trusted_source_knowledge_selection_latest.json",
    "artifacts/trusted_source_requery_decision_latest.json",
    "artifacts/trusted_source_reuse_vs_requery_latest.json",
    "artifacts/trusted_source_knowledge_freshness_latest.json",
    "artifacts/trusted_source_knowledge_reuse_readiness_latest.json",
    "artifacts/trusted_source_knowledge_invalidation_latest.json",
    "artifacts/trusted_source_knowledge_supersession_latest.json",
    "artifacts/trusted_source_knowledge_health_latest.json",
    "artifacts/trusted_source_external_zip_audit_report_latest.json",
    ...
    "docs/trusted_source_external_zip_audit_note.md",
    "src/successor_shell/trusted_reference_inventory.py",
    "src/successor_shell/trusted_zip_audit.py",
    "tests/test_trusted_reference_inventory.py",
    "tests/test_trusted_zip_audit.py",
}
SHOWCASE_ARTIFACT_PRIORITY = (
    (
        "trusted_source_reuse_summary_path",
        "Trusted-source reuse summary",
        "Latest reuse-vs-requery result for the current trusted-source capability cycle.",
    ),
    (
        "trusted_source_knowledge_selection_path",
        "Trusted-source knowledge selection",
        "Selection record showing which indexed knowledge entry was reused or why re-query was required.",
    ),
    (
        "trusted_source_external_zip_audit_report_path",
        "External zip audit report",
        "Validated external trusted-source audit result for the current delivered standalone zip.",
    ),
    (
        "trusted_source_knowledge_index_path",
        "Trusted-source knowledge index",
        "Indexed reusable knowledge created from bounded trusted-source evidence.",
    ),
    (
        "trusted_source_provider_status_path",
        "Trusted-source provider status",
        "Non-secret provider validation status for the current external trusted-source session.",
    ),
    (
        "trusted_source_capability_result_path",
        "Trusted-source capability result",
        "Latest bounded trusted-source capability validation result for the active workspace.",
    ),
    (
        "trusted_source_knowledge_pack_path",
        "Trusted-source knowledge pack",
        "Most recent bounded knowledge artifact distilled from approved trusted-source evidence.",
    ),
    (
        "trusted_source_skill_candidate_path",
        "Trusted-source skill candidate",
        "Generated helper or candidate capability synthesized from the trusted-source knowledge pack.",
    ),
    (
        "trusted_source_reference_inventory_path",
        "Trusted-source reference inventory",
        "Validated inventory of the frozen novali-v5 reference assets with provenance.",
    ),
    (
        "latest_summary_artifact_path",
        "Latest cycle summary",
        "Most recent bounded cycle summary produced by the operator flow.",
    ),
    (
        "implementation_summary_path",
        "Implementation summary",
        "Most recent implementation or planning bundle summary written inside the active workspace.",
    ),
    (
        "review_summary_path",
        "Review summary",
        "Most recent operator-review state and post-cycle interpretation.",
    ),
    (
        "next_objective_proposal_path",
        "Next objective proposal",
        "Current bounded next-step proposal derived from the latest result.",
    ),
    (
        "skill_pack_result_path",
        "Skill-pack result",
        "Latest bounded pack execution result, when a quality pack ran.",
    ),
    (
        "quality_improvement_summary_path",
        "Quality improvement summary",
        "Latest recorded quality delta against the current bounded reference target.",
    ),
    (
        "promotion_recommendation_path",
        "Promotion recommendation",
        "Most recent promotion review outcome for the current bounded result.",
    ),
    (
        "baseline_admission_review_path",
        "Admission review",
        "Most recent baseline-admission review state for a candidate bundle.",
    ),
    (
        "baseline_admission_decision_path",
        "Admission decision",
        "Most recent admission decision state recorded for the candidate bundle.",
    ),
    (
        "reference_target_path",
        "Reference target",
        "Current bounded reference target carried forward by the operator flow.",
    ),
)
EVENT_LABELS = {
    ...
    "trusted_source_capability_gap_recorded": "Trusted-source capability gap recorded",
    "trusted_source_request_recorded": "Trusted-source request recorded",
    "trusted_source_provider_validated": "Trusted-source provider validated",
    "trusted_source_handshake_materialized": "Trusted-source handshake materialized",
    "trusted_source_mission_policy_recorded": "Trusted-source mission policy recorded",
    "trusted_source_local_only_selected": "Trusted-source local-only path selected",
    "trusted_source_reuse_selected": "Trusted-source reuse selected",
    "trusted_source_requery_required": "Trusted-source re-query required",
    "trusted_source_mission_requery_required": "Trusted-source mission re-query required",
    "trusted_source_operator_review_selected": "Trusted-source operator-review path selected",
    "trusted_source_evidence_acquired": "Trusted-source evidence acquired",
    "trusted_source_external_response_acquired": "Trusted-source external response acquired",
    "trusted_source_knowledge_distilled": "Trusted-source knowledge distilled",
    "trusted_source_knowledge_indexed": "Trusted-source knowledge indexed",
    "trusted_source_knowledge_superseded": "Trusted-source knowledge superseded",
    "trusted_source_knowledge_health_recorded": "Trusted-source knowledge health recorded",
    "trusted_source_skill_candidate_synthesized": "Trusted-source skill candidate synthesized",
    "trusted_source_capability_validated": "Trusted-source capability validated",
    "trusted_source_mission_output_advanced": "Trusted-source mission output advanced",
    "successor_package_improvement_bundle_materialized": "Successor package improvement bundle materialized",
...
def _build_showcase_result_summary(
    *,
    ...
    if trusted_source_result_state:
...
class OperatorWebService:
    ...
    def _observability_snapshot(self) -> dict[str, Any]:
        ...
        def _prefer_workspace_artifact_path(
            ...
            return str(controller_summary.get(controller_key, "")).strip() or workspace_candidate
        ...
        *,
        ...
        def add_item(path: str, label: str, detail: str, *, tone: str = "info") -> None:
            ...
            items.append(
                {
                    "label": label,
                    "value": self._display_artifact_hint(normalized, snapshot=snapshot),
                    "detail": detail,
                    "preview_href": self._preview_link_for_path(normalized),
                    "tone": tone,
                }
    ...
    def _event_detail_lines(self, event: dict[str, Any]) -> list[str]:
        ...
        if str(event.get("audit_report_path", "")).strip():
            lines.append(
                "Audit report: "
                f"{self._render_path_html(str(event['audit_report_path']))}"
            )
    ...
    def render_observability_page(self) -> str:
        ...
        stats = [
            ...
            (
                "Trusted-Source Capability",
                snapshot.get("trusted_source_capability_id", "") or "<none>",
            ),
            (
                "Trusted-Source Result",
                snapshot.get("trusted_source_capability_result_state", "") or "<none>",
            ),
            (
        ...
        body = (
            ...
            "<h2>Outcome Story</h2>"
            f"{_render_state_pill('Current result', showcase_summary['recent_accomplishment'], tone=str(showcase_summary['recent_accomplishment_tone']))}"
            "</div>"
            "<p>Use this layer for the quick demo read: what NOVALI most recently accomplished, why that matters, and how the current objective connects to the next bounded move.</p>"
            f"<div class='story-grid'>{observability_showcase_cards}</div>"
            "</div>"
            "<div>"
            "<div class='section-heading'>"
            "<h3>Important Outputs</h3>"
            f"{_render_state_pill('Tracked outputs', showcase_summary['output_summary_label'], tone=str(showcase_summary['output_summary_tone']))}"
            "</div>"
            "<p>These are the most useful current artifacts to open first when you want proof, not just status. The richer listing still lives below and in Workspace.</p>"
            f"<div class='story-grid'>{observability_output_cards}</div>"
            f"{latest_cycle_links}"
            "</div>"
            "</div>"
            "</section>"
            ...
            "</section>"
            "<section class='card'>"
            "<h2>Trusted-Source Capability</h2>"
            f"<p><strong>Capability gap:</strong> <code>{html.escape(str(snapshot.get('trusted_source_gap_id', '') or '<none>'))}</code></p>"
            f"<p><strong>Request id:</strong> <code>{html.escape(str(snapshot.get('trusted_source_request_id', '') or '<none>'))}</code></p>"
            f"<p><strong>Provider id:</strong> <code>{html.escape(str(snapshot.get('trusted_source_provider_id', '') or '<none>'))}</code></p>"
            f"<p><strong>Credential state:</strong> <code>{html.escape(str(snapshot.get('trusted_source_credential_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Provider validation:</strong> <code>{html.escape(str(snapshot.get('trusted_source_provider_validation_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Handshake state:</strong> <code>{html.escape(str(snapshot.get('trusted_source_handshake_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Capability id:</strong> <code>{html.escape(str(snapshot.get('trusted_source_capability_id', '') or '<none>'))}</code></p>"
            f"<p><strong>Capability result:</strong> <code>{html.escape(str(snapshot.get('trusted_source_capability_result_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Selected model:</strong> <code>{html.escape(str(snapshot.get('trusted_source_selected_model', '') or '<none>'))}</code></p>"
            f"<p><strong>Knowledge state:</strong> <code>{html.escape(str(snapshot.get('trusted_source_knowledge_state', '') or '<none>'))}</code></p>"
            f"<p><strong>External zip audit state:</strong> <code>{html.escape(str(snapshot.get('trusted_source_external_zip_audit_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Mission policy state:</strong> <code>{html.escape(str(snapshot.get('trusted_source_mission_policy_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Escalation decision:</strong> <code>{html.escape(str(snapshot.get('trusted_source_escalation_decision_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Mission orchestration:</strong> <code>{html.escape(str(snapshot.get('trusted_source_mission_orchestration_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Aggressiveness state:</strong> <code>{html.escape(str(snapshot.get('trusted_source_aggressiveness_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Budget state:</strong> <code>{html.escape(str(snapshot.get('trusted_source_budget_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Retry state:</strong> <code>{html.escape(str(snapshot.get('trusted_source_retry_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Cost-governance state:</strong> <code>{html.escape(str(snapshot.get('trusted_source_cost_governance_state', '') or '<none>'))}</code></p>"
            f"<p><strong>External request budget:</strong> <code>{html.escape(str(snapshot.get('trusted_source_external_request_budget', '') or '<none>'))}</code></p>"
            f"<p><strong>Retry budget:</strong> <code>{html.escape(str(snapshot.get('trusted_source_retry_budget', '') or '<none>'))}</code></p>"
            f"<p><strong>Policy reason:</strong> {html.escape(str(snapshot.get('trusted_source_policy_reason', '') or '<none recorded>'))}</p>"
            f"<p><strong>Reuse state:</strong> <code>{html.escape(str(snapshot.get('trusted_source_reuse_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Re-query state:</strong> <code>{html.escape(str(snapshot.get('trusted_source_requery_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Selected knowledge id:</strong> <code>{html.escape(str(snapshot.get('trusted_source_selected_knowledge_id', '') or '<none>'))}</code></p>"
            f"<p><strong>Mission objective:</strong> <code>{html.escape(str(snapshot.get('trusted_source_mission_objective_id', '') or '<none>'))}</code></p>"
            f"<p><strong>Mission advancement state:</strong> <code>{html.escape(str(snapshot.get('trusted_source_mission_advancement_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Mission advancement summary:</strong> {html.escape(str(snapshot.get('trusted_source_mission_advancement_summary', '') or '<none>'))}</p>"
            f"<p><strong>Successor package improvement state:</strong> <code>{html.escape(str(snapshot.get('successor_package_improvement_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Successor package improvement summary:</strong> {html.escape(str(snapshot.get('successor_package_improvement_summary', '') or '<none recorded>'))}</p>"
            f"<p><strong>Handoff refresh recommended:</strong> {html.escape('yes' if bool(snapshot.get('successor_handoff_refresh_recommended', False)) else 'no')}</p>"
            f"<p><strong>Knowledge freshness:</strong> <code>{html.escape(str(snapshot.get('trusted_source_freshness_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Reuse readiness:</strong> <code>{html.escape(str(snapshot.get('trusted_source_reuse_readiness_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Validated for reuse:</strong> {html.escape('yes' if bool(snapshot.get('trusted_source_validated', False)) else 'no')}</p>"
            f"<p><strong>Tests passed:</strong> {html.escape('yes' if bool(snapshot.get('trusted_source_tests_passed', False)) else 'no')}</p>"
            f"<p><strong>Required frozen-reference roles / audit categories satisfied:</strong> {html.escape('yes' if bool(snapshot.get('trusted_source_inventory_required_roles_satisfied', False)) else 'no')}</p>"
            f"<p><strong>Present evidence items:</strong> {html.escape(str(trusted_source_evidence_bundle.get('present_evidence_item_count', 0) or 0))}</p>"
            f"<p><strong>Governance separation:</strong> {html.escape(str(trusted_source_capability_result.get('governance_authority_note', '') or 'Trusted-source evidence is implementation support only and does not replace directive or policy authority.'))}</p>"
            "<h3>Requested trusted sources</h3>"
            f"{_html_list(trusted_source_requested_source_lines)}"
            "<h3>Provenance rows</h3>"
            f"{_html_list(trusted_source_provenance_lines)}"
            "<h3>Indexed knowledge health</h3>"
            f"{_html_list(trusted_source_health_lines)}"
            f"<p><strong>Operator credential status artifact:</strong> {self._render_path_html(str(snapshot.get('operator_trusted_source_credential_status_path', '')))}</p>"
            f"<p><strong>Operator provider status artifact:</strong> {self._render_path_html(str(snapshot.get('operator_trusted_source_provider_status_path', '')))}</p>"
            f"<p><strong>Capability-gap artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_capability_gap_path', '')))}</p>"
            f"<p><strong>Trusted-source request artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_request_path', '')))}</p>"
            f"<p><strong>Capability-need artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_capability_need_path', '')))}</p>"
            f"<p><strong>Workspace credential status artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_credential_status_path', '')))}</p>"
            f"<p><strong>Workspace provider status artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_provider_status_path', '')))}</p>"
            f"<p><strong>Handshake artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_handshake_path', '')))}</p>"
            f"<p><strong>Session contract artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_session_contract_path', '')))}</p>"
            f"<p><strong>Response template artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_response_template_path', '')))}</p>"
            f"<p><strong>Request contract artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_request_contract_path', '')))}</p>"
            f"<p><strong>External response artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_external_response_path', '')))}</p>"
            f"<p><strong>Evidence bundle artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_evidence_bundle_path', '')))}</p>"
            f"<p><strong>Provenance artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_provenance_path', '')))}</p>"
            f"<p><strong>Acquisition summary artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_acquisition_summary_path', '')))}</p>"
            f"<p><strong>Knowledge pack artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_knowledge_pack_path', '')))}</p>"
            f"<p><strong>Distillation summary artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_distillation_summary_path', '')))}</p>"
            f"<p><strong>Knowledge index artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_knowledge_index_path', '')))}</p>"
            f"<p><strong>Knowledge catalog artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_knowledge_catalog_path', '')))}</p>"
            f"<p><strong>Reuse decision artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_reuse_decision_path', '')))}</p>"
            f"<p><strong>Reuse summary artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_reuse_summary_path', '')))}</p>"
            f"<p><strong>Knowledge selection artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_knowledge_selection_path', '')))}</p>"
            f"<p><strong>Re-query decision artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_requery_decision_path', '')))}</p>"
            f"<p><strong>Reuse-vs-requery artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_reuse_vs_requery_path', '')))}</p>"
            f"<p><strong>Knowledge freshness artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_knowledge_freshness_path', '')))}</p>"
            f"<p><strong>Knowledge reuse-readiness artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_knowledge_reuse_readiness_path', '')))}</p>"
            f"<p><strong>Mission policy artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_mission_policy_path', '')))}</p>"
            f"<p><strong>Escalation decision artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_escalation_decision_path', '')))}</p>"
            f"<p><strong>Mission orchestration artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_mission_orchestration_path', '')))}</p>"
            f"<p><strong>Aggressiveness policy artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_aggressiveness_policy_path', '')))}</p>"
            f"<p><strong>Escalation thresholds artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_escalation_thresholds_path', '')))}</p>"
            f"<p><strong>Budget policy artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_budget_policy_path', '')))}</p>"
            f"<p><strong>Budget state artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_budget_state_path', '')))}</p>"
            f"<p><strong>Retry policy artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_retry_policy_path', '')))}</p>"
            f"<p><strong>Retry state artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_retry_state_path', '')))}</p>"
            f"<p><strong>Cost governance artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_cost_governance_path', '')))}</p>"
            f"<p><strong>Knowledge invalidation artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_knowledge_invalidation_path', '')))}</p>"
            f"<p><strong>Knowledge supersession artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_knowledge_supersession_path', '')))}</p>"
            f"<p><strong>Knowledge health artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_knowledge_health_path', '')))}</p>"
            f"<p><strong>Skill candidate artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_skill_candidate_path', '')))}</p>"
            f"<p><strong>Skill synthesis summary artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_skill_synthesis_summary_path', '')))}</p>"
            f"<p><strong>Capability result artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_capability_result_path', '')))}</p>"
            f"<p><strong>Successor package improvement summary artifact:</strong> {self._render_path_html(str(snapshot.get('successor_package_improvement_summary_path', '')))}</p>"
            f"<p><strong>Successor handoff refresh manifest:</strong> {self._render_path_html(str(snapshot.get('successor_handoff_refresh_manifest_path', '')))}</p>"
            f"<p><strong>Successor package improvement note:</strong> {self._render_path_html(str(snapshot.get('successor_package_improvement_note_path', '')))}</p>"
            f"<p><strong>Reference inventory artifact:</strong> {self._render_path_html(str(snapshot.get('trusted_source_reference_inventory_path', '')))}</p>"
            f"<p><strong>Reference inventory note:</strong> {self._render_path_html(str(snapshot.get('trusted_source_reference_inventory_note_path', '')))}</p>"
            f"<p><strong>External zip audit report:</strong> {self._render_path_html(str(snapshot.get('trusted_source_external_zip_audit_report_path', '')))}</p>"
            f"<p><strong>External zip audit note:</strong> {self._render_path_html(str(snapshot.get('trusted_source_external_zip_audit_note_path', '')))}</p>"
            f"<p><strong>Generated helper module:</strong> {self._render_path_html(str(snapshot.get('trusted_source_helper_module_path', '')))}</p>"
            f"<p><strong>Generated helper test:</strong> {self._render_path_html(str(snapshot.get('trusted_source_helper_test_path', '')))}</p>"
            f"<p><strong>Knowledge pack id:</strong> <code>{html.escape(str(trusted_source_knowledge_pack.get('knowledge_pack_id', '') or '<none>'))}</code></p>"
            f"<p><strong>Inventory roles / audit categories missing:</strong> <code>{html.escape(', '.join(str(item) for item in list(snapshot.get('trusted_source_missing_role_ids', []))) or '<none>')}</code></p>"
            f"<p><strong>Requested scope:</strong> {html.escape(str(trusted_source_request_payload.get('requested_scope_topic', '') or '<none recorded>'))}</p>"
            f"<p><strong>Expected result:</strong> {html.escape(str(dict(snapshot.get('successor_trusted_source_capability_need', {})).get('expected_result', '') or '<none recorded>'))}</p>"
            f"<p><strong>Response template contract:</strong> {html.escape(str(trusted_source_response_template.get('template_id', '') or '<none recorded>'))}</p>"
            f"<p><strong>Knowledge catalog state:</strong> <code>{html.escape(str(trusted_source_knowledge_catalog.get('catalog_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Knowledge invalidation state:</strong> <code>{html.escape(str(trusted_source_knowledge_invalidation.get('action_state', '') or trusted_source_knowledge_invalidation.get('knowledge_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Knowledge supersession state:</strong> <code>{html.escape(str(trusted_source_knowledge_supersession.get('action_state', '') or trusted_source_knowledge_supersession.get('knowledge_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Workspace audit note:</strong> {html.escape(str(trusted_source_external_zip_audit_report.get('audit_summary', '') or '<none recorded>'))}</p>"
            f"<p><strong>Session contract state:</strong> <code>{html.escape(str(trusted_source_session_contract.get('session_contract_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Request contract state:</strong> <code>{html.escape(str(trusted_source_request_contract.get('request_contract_state', '') or '<none>'))}</code></p>"
            f"<p><strong>External response state:</strong> <code>{html.escape(str(trusted_source_external_response.get('response_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Operator provider handshake:</strong> <code>{html.escape(str(operator_trusted_source_provider_status.get('handshake_state', '') or '<none>'))}</code></p>"
            f"<p><strong>Operator credential validity:</strong> <code>{html.escape(str(operator_trusted_source_credential_status.get('validation_state', '') or '<none>'))}</code></p>"
            "</section>"
            "<section class='card'>"
    ...
    def render_workspace_page(self) -> str:
        ...
        body = (
            ...
            "<div>"
            "<div class='section-heading'>"
            "<h2>Workspace Story</h2>"
            f"{_render_state_pill('Current result', showcase_summary['recent_accomplishment'], tone=str(showcase_summary['recent_accomplishment_tone']))}"
            "</div>"
            "<p>This page stays read-only, but it now surfaces the most meaningful workspace outputs first so demos and operator spot checks can start from the strongest current evidence.</p>"
            f"<div class='story-grid'>{workspace_story_cards}</div>"
            "</div>"
            "<div>"
            "<div class='section-heading'>"
            "<h3>Key Outputs</h3>"
            f"{_render_state_pill('Tracked outputs', showcase_summary['output_summary_label'], tone=str(showcase_summary['output_summary_tone']))}"
            "</div>"
            "<p>Open these first for a quick read of the current bounded result, then use the grouped tables below when you want the full workspace picture.</p>"
            f"<div class='story-grid'>{workspace_output_cards}</div>"
            "<div class='section-links' style='margin-top: 14px;'>"
            "<a href='/observability'>Return to observability</a>"
            "<a href='/cycle'>Open latest cycle summary</a>"
            "<a href='/timeline'>Open runtime timeline</a>"
            "</div>"
            "</div>"
    ...
    def render_timeline_page(self) -> str:
        ...
        body = (
            ...
            f"{_render_state_pill('Current result', showcase_summary['recent_accomplishment'], tone=str(showcase_summary['recent_accomplishment_tone']))}"
            "</div>"
            "<p>The timeline remains the source of ordered runtime truth. This top strip just pulls the latest few structured events forward so a first-time viewer can see the recent story before reading the full ledger.</p>"
            f"<div class='story-grid'>{recent_event_cards}</div>"
            "</div>"
            "<div>"
            "<div class='section-heading'>"
            "<h3>Why It Matters</h3>"
            f"{_render_state_pill('Operator attention', showcase_summary['why_it_matters'], tone=str(showcase_summary['why_it_matters_tone']))}"
            "</div>"
            f"<div class='story-grid'>{_render_story_card('Recent outcome', showcase_summary['recent_accomplishment'], detail=str(showcase_summary['recent_accomplishment_detail']), tone=str(showcase_summary['recent_accomplishment_tone']))}{_render_story_card('Why it matters', showcase_summary['why_it_matters'], detail=str(showcase_summary['why_it_matters_detail']), tone=str(showcase_summary['why_it_matters_tone']))}</div>"
            "<div class='section-links' style='margin-top: 14px;'>"
            "<a href='/observability'>Return to observability</a>"
            "<a href='/cycle'>Open latest cycle summary</a>"
            "<a href='/workspace'>Browse workspace outputs</a>"
            "</div>"
            "</div>"
            "</div>"
            "</section>"
    ...
    def render_cycle_summary_page(self) -> str:
        ...
        body = (
            ...
            "<div class='section-heading'>"
            "<h2>Cycle Story</h2>"
            f"{_render_state_pill('Cycle result', showcase_summary['recent_accomplishment'], tone=str(showcase_summary['recent_accomplishment_tone']))}"
            "</div>"
            "<p>This page still keeps the full cycle detail, but the top now tells the short story first: what the last bounded cycle accomplished, why it matters, and what it produced.</p>"
            f"<div class='story-grid'>{cycle_story_cards}</div>"
            "</div>"
            "<div>"
            "<div class='section-heading'>"
            "<h3>Important Outputs</h3>"
            f"{_render_state_pill('Current intent', showcase_summary['current_intent'], tone=str(showcase_summary['current_intent_tone']))}"
            "</div>"
            "<p>These are the most useful artifacts to open first when you are demoing or auditing the latest bounded cycle.</p>"
            f"<div class='story-grid'>{cycle_output_cards}</div>"
            "<div class='section-links' style='margin-top: 14px;'>"
            "<a href='/workspace'>Browse workspace outputs</a>"
            "<a href='/timeline'>Open runtime timeline</a>"
            "<a href='/observability'>Return to observability</a>"
    ...
    def render_home_page(self, *, notice: dict[str, Any] | None = None) -> str:
        ...
        dashboard_pulse_cards = "".join(
            [
                ...
                _render_story_card(
                    "Current work family",
                    dashboard_actionability["current_work_family"],
                    detail=str(dashboard_actionability["current_work_family_detail"]),
                    tone=str(dashboard_actionability["current_work_family_tone"]),
                ),
                _render_story_card(
                    "Latest quality delta",
                    dashboard_actionability["latest_quality_delta"],
                    detail=str(dashboard_actionability["latest_quality_delta_detail"]),
                    tone=str(dashboard_actionability["latest_quality_delta_tone"]),
                ),
                _render_story_card(
                    "Campaign / cycle / loop",
                    dashboard_actionability["campaign_stack_value"],
                    detail=str(dashboard_actionability["campaign_stack_detail"]),
                    tone=str(dashboard_actionability["campaign_stack_tone"]),
                ),
            ]
        )
...
<body>
  <div class="page-shell">
    ...
    <section class="card" id="trusted-sources-section">
      ...
      <p>Bindings and secrets remain outside directive authority. External trusted-source use is bounded, operator-triggered, and audited. The credential is read into the current process session only, then validation artifacts record non-secret provider metadata. Trusted-source evidence never replaces directive, policy, or governance authority.</p>
      <div class="story-grid">
        {_render_story_card("Credential state", operator_trusted_source_credential_status.get("validation_state", "") or "no credential configured", detail="No raw secret is persisted. Validation writes only non-secret status metadata.", tone=_status_tone(operator_trusted_source_credential_status.get("validation_state", "") or "no credential configured"))}
        {_render_story_card("Provider state", operator_trusted_source_provider_status.get("validation_state", "") or "provider disabled until credential validation", detail=current_provider_base_url, tone=_status_tone(operator_trusted_source_provider_status.get("validation_state", "") or "provider disabled until credential validation"))}
        {_render_story_card("Handshake", operator_trusted_source_provider_status.get("handshake_state", "") or operator_trusted_source_handshake.get("handshake_state", "") or "not_started", detail="Request/response contract negotiation must succeed before external evidence exchange.", tone=_status_tone(operator_trusted_source_provider_status.get("handshake_state", "") or operator_trusted_source_handshake.get("handshake_state", "") or "not_started"))}
        ...
        {_render_story_card("Current external result", observability.get("successor_package_improvement_state", "") or observability.get("trusted_source_mission_advancement_state", "") or observability.get("trusted_source_external_zip_audit_state", "") or observability.get("trusted_source_mission_policy_state", "") or "not started", detail=str(observability.get("successor_package_improvement_summary", "") or observability.get("trusted_source_policy_reason", "") or observability.get("trusted_source_mission_advancement_summary", "") or observability.get("trusted_source_reuse_state", "") or observability.get("trusted_source_knowledge_state", "") or "No external knowledge is indexed yet."), tone=_status_tone(observability.get("successor_package_improvement_state", "") or observability.get("trusted_source_mission_advancement_state", "") or observability.get("trusted_source_external_zip_audit_state", "") or observability.get("trusted_source_mission_policy_state", "") or "not started"))}
      </div>
      ...
      <p class="muted">Paste a credential for this local session or point to a local file that contains either a raw API key line or <code>OPENAI_API_KEY=...</code>. The secret is read into the current process session only, never written into normal artifacts, and failed validation does not enable the provider binding.</p>
      <p>Bindings file: <code>{html.escape(str(operator_snapshot.get('trusted_source_bindings_path', '')))}</code><br/>Local secrets file: <code>{html.escape(str(operator_snapshot.get('trusted_source_secrets_path', '')))}</code><br/>Credential status artifact: <code>{html.escape(str(operator_snapshot.get('trusted_source_credential_status_path', '')))}</code><br/>Provider status artifact: <code>{html.escape(str(operator_snapshot.get('trusted_source_provider_status_path', '')))}</code><br/>Handshake artifact: <code>{html.escape(str(operator_snapshot.get('trusted_source_handshake_path', '')))}</code><br/>Session contract artifact: <code>{html.escape(str(operator_snapshot.get('trusted_source_session_contract_path', '')))}</code><br/>Request contract artifact: <code>{html.escape(str(operator_snapshot.get('trusted_source_request_contract_path', '')))}</code><br/>Response template artifact: <code>{html.escape(str(operator_snapshot.get('trusted_source_response_template_path', '')))}</code><br/>Environment template: <code>standalone_docker/standalone.env.template</code></p>
      <p><strong>Current external-provider status</strong></p>
      <pre>{html.escape(trusted_source_status_summary)}</pre>
      <p><strong>Bindings and availability summary</strong></p>
