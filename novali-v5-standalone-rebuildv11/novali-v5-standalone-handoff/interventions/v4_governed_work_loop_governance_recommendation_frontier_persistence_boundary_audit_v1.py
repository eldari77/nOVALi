...
from .v4_governed_work_loop_candidate_screen_snapshot_v5 import (
    _build_loop_candidate_examples_v5,
...
def _build_frontier_persistence_boundary_audit(
    *,
    directive_state: dict[str, Any],
    current_state_summary: dict[str, Any],
    branch_record: dict[str, Any],
    ...
    evidence_v4_summary: dict[str, Any],
    continuation_envelope: dict[str, Any],
    callable_capabilities: list[dict[str, Any]],
    analytics: dict[str, Any],
    recommendations: dict[str, Any],
    ...
    evidence_chain_state = dict(
        evidence_v4_summary.get("chain_state_reviewed", evidence_v4_summary.get("current_work_loop_chain_state", {}))
    )
    gate_status = dict(evidence_v4_summary.get("gate_status", evidence_v4_summary.get("future_posture_review_gate", {})))
    ...
    persistence_signal_count = sum(1 for value in frontier_signal_components.values() if bool(value))
    material_persistence_boundary_delta_detected = bool(
    ...
    checks = {
        ...
        "bucket_allows_continuation_budget": _check(
            bool(resource_fit.get("passed", False)),
            "bucket still covers the admitted loop-continuation v5 budget",
            requested_resources=dict(resource_fit.get("requested_resources", {})),
            bucket_limits=dict(resource_fit.get("bucket_limits", {})),
        ),
        "network_mode_none_remains_policy_valid": _check(
        ...
        "work_loop_progression_state_current": _check(
            bool(current_state_summary.get("governed_work_loop_policy_v1_defined", False))
            and bool(current_state_summary.get("governed_work_loop_candidate_screening_v5_in_place", False))
            and bool(current_state_summary.get("governed_work_loop_continuation_admission_v5_in_place", False))
            and str(current_state_summary.get("latest_governed_work_loop_continuation_outcome", ""))
            == "admissible_for_governed_loop_continuation_v5"
            and str(current_state_summary.get("latest_governed_work_loop_execution_readiness", ""))
            in {
                "ready_for_shadow_work_loop_continuation_execution_v5",
                "ready_for_work_loop_evidence_review_v5",
            },
            "current state still reflects the v5 candidate screen, v5 continuation admission, and ready-for-shadow-execution posture",
        ),
        "paused_parser_capability_still_held_not_reopened": _check(
            bool(parser_capability)
            and str(parser_capability.get("status", "")) == "paused_with_provisional_capability_held"
        ...
        "frontier_top_candidate_matches_admitted_execution": _check(
            ...
            "the active frontier candidate still matches both the top-ranked v5 screen result and the admitted v5 execution target",
        ),
        ...
        "frontier_materially_distinct": _check(
            ...
            "the active frontier element remains materially distinct from direct work, continuation v1, continuation v2, continuation v3, continuation v4, evidence v2, evidence v3, and evidence v4",
            observed={
        ...
        "frontier_low_recursion_low_pressure": _check(
            ...
            "the frontier does not show administrative recursion or posture pressure",
        ),
        ...
        "narrow_posture_artifacts_present": _check(
            all(path.exists() for path in work_loop_artifact_paths.values()),
            "the frontier-persistence boundary audit still sits on top of the local narrow-posture artifact chain",
    ...
    failed = [name for name, result in checks.items() if not bool(dict(result).get("passed", False))]
    frontier_persistence_boundary_status = (
        "persistence_bounded_with_material_delta"
        if not failed and material_persistence_boundary_delta_detected
        else "persistence_bounded_without_material_delta"
        if not failed
        else "persistence_boundary_instability_detected"
    )
    ...
    frontier_persistence_assessment = (
        "persistence_bounded_and_materially_informative"
        if frontier_persistence_boundary_status == "persistence_bounded_with_material_delta"
        else "persistence_bounded_but_incremental"
        if frontier_persistence_boundary_status == "persistence_bounded_without_material_delta"
        else "persistence_boundary_at_risk"
    )
    return {
        "audit_focus": "governance_recommendation_frontier_persistence_boundary_audit",
        "checks": checks,
        ...
        "high_signal_observations": [
            (
                "the current governed recommendation frontier remains persistence-bounded relative to the prior frontier-stability state"
                if frontier_persistence_boundary_status != "persistence_boundary_instability_detected"
                else "the current governed recommendation frontier no longer looks persistence-bounded under the narrow envelope"
            ),
            (
                "this audit adds a new frontier-quality signal about frontier persistence boundaries rather than replaying prior chain conclusions"
                if material_persistence_boundary_delta_detected
                else "this audit stayed readable but did not add enough new frontier-persistence signal beyond the prior chain"
            ),
        ],
        "summary": {
            "total_check_count": int(len(checks)),
            ...
            "failed_check_count": int(len(failed)),
            "failed_check_names": failed,
            "persistence_signal_count": int(persistence_signal_count),
            "material_persistence_boundary_delta_detected": bool(material_persistence_boundary_delta_detected),
            "frontier_persistence_boundary_status": frontier_persistence_boundary_status,
            "frontier_persistence_assessment": frontier_persistence_assessment,
...
def run_probe(cfg, proposal, *, rounds, seeds):
    ...
    if not all(
        ...
        return _failure(
            proposal,
            "shadow work-loop continuation failed: governance substrate, work-loop policy, v5 candidate screen, v5 continuation admission, posture, evidence v3/v4, prior continuation chain, direct-work chain, and capability-use evidence artifacts are required",
        )
    ...
    if str(last_continuation_admission.get("status", "")) != "admissible_for_governed_loop_continuation_v5":
        return _failure(
            proposal,
            "shadow work-loop continuation failed: the surviving v5 loop candidate is not currently admitted for governed work-loop continuation v5",
        )
    ...
    if execution_readiness not in {
        ...
        return _failure(
            proposal,
            "shadow work-loop continuation failed: self-structure does not currently mark the v5 loop continuation path as ready for shadow execution",
        )
    if execution_readiness == "ready_for_work_loop_evidence_review_v5" and latest_execution_outcome != "shadow_work_loop_continuation_v5_completed":
        return _failure(
            proposal,
            "shadow work-loop continuation failed: evidence-review readiness was present without a completed v5 continuation execution",
        )
    ...
    if not all(
        ...
        return _failure(
            proposal,
            "shadow work-loop continuation failed: one or more governing artifact paths for v5 loop continuation could not be resolved",
        )
    ...
    work_loop_candidate_screen_summary = dict(
        work_loop_candidate_screen_payload.get("governed_work_loop_candidate_screen_v5_summary", {})
    )
    continuation_admission_summary = dict(continuation_admission_payload.get("governed_work_loop_continuation_admission_v5_summary", {}))
    ...
    if not dict(prior_work_loop_evidence_payload.get("governed_work_loop_evidence_v3_summary", {})):
        return _failure(proposal, "shadow work-loop continuation failed: work-loop evidence v3 summary is missing")
    if not dict(work_loop_evidence_payload.get("governed_work_loop_evidence_v4_summary", {})):
        return _failure(proposal, "shadow work-loop continuation failed: work-loop evidence v4 summary is missing")
    if not all([work_loop_candidate_screen_summary, continuation_admission_summary, prior_continuation_execution_payload_v1, prior_continuation_execution_payload_v2, prior_continuation_execution_payload_v3, prior_continuation_execution_payload_v4, direct_work_execution_payload]):
        return _failure(
            proposal,
            "shadow work-loop continuation failed: governing summary payloads for v5 loop continuation execution are incomplete",
        )
    ...
    evidence_v3_summary = dict(prior_work_loop_evidence_payload.get("governed_work_loop_evidence_v3_summary", {}))
    evidence_v4_summary = dict(work_loop_evidence_payload.get("governed_work_loop_evidence_v4_summary", {}))
    ...
    if not all([posture_summary, evidence_v3_summary, evidence_v4_summary, direct_work_evidence_summary, capability_use_evidence_summary]):
        return _failure(
            proposal,
            "shadow work-loop continuation failed: one or more governing summaries required for frontier persistence boundary are missing",
        )

    posture_current = dict(posture_summary.get("current_posture", {}))
    future_gate = dict(evidence_v4_summary.get("gate_status", {}))
    loop_chain_state = dict(evidence_v4_summary.get("chain_state_reviewed", {}))
    ...
    screened_candidates = [
        _screen_loop_candidate_v4(
            ...
            loop_continuation_future_posture=str(dict(evidence_v4_summary.get("future_posture", {})).get("category", "")),
    ...
    frontier_persistence_boundary_audit = _build_frontier_persistence_boundary_audit(
        ...
        evidence_v4_summary=evidence_v4_summary,
    ...
    next_template = "memory_summary.v4_governed_work_loop_evidence_snapshot_v5"
    artifact_path = _diagnostic_artifact_dir() / (
        f"proposal_learning_loop_v4_governed_work_loop_governance_recommendation_frontier_persistence_boundary_audit_v1_{proposal['proposal_id']}.json"
    )
    ...
    material_persistence_boundary_delta_detected = bool(dict(frontier_persistence_boundary_audit.get("summary", {})).get("material_persistence_boundary_delta_detected", False))
    distinct_value_read = {
        "passed": bool(material_persistence_boundary_delta_detected),
        "value": "high" if bool(material_persistence_boundary_delta_detected) else "low",
        "reason": (
            "the continuation produced a bounded frontier-persistence boundary audit with a new recommendation-frontier quality signal beyond the prior direct-work step, continuation v1, continuation v2, the prior frontier-containment execution, and the prior frontier-stability execution"
            if bool(material_persistence_boundary_delta_detected)
        ...
        "vs_prior_direct_work": {
            "distinct": True,
            "reason": "the execution audited frontier persistence boundaries rather than replaying the governance state coherence audit shape",
            "observed_deltas": dict(dict(frontier_persistence_boundary_audit.get("frontier_observations", {})).get("frontier_chain_deltas", {})).get("vs_prior_direct_work", {}),
        },
        "vs_prior_continuation_v1": {
            "distinct": True,
            "reason": "the execution audited frontier persistence boundaries rather than replaying the earlier ledger consistency delta audit",
            "observed_deltas": dict(dict(frontier_persistence_boundary_audit.get("frontier_observations", {})).get("frontier_chain_deltas", {})).get("vs_prior_continuation_v1", {}),
        },
        "vs_prior_continuation_v2": {
            "distinct": True,
            "reason": "the execution audited frontier persistence boundaries rather than replaying the earlier recommendation-to-ledger alignment delta audit",
            "observed_deltas": dict(dict(frontier_persistence_boundary_audit.get("frontier_observations", {})).get("frontier_chain_deltas", {})).get("vs_prior_continuation_v2", {}),
        },
        "vs_prior_continuation_v3": {
            "distinct": True,
            "reason": "the execution audited frontier persistence boundaries relative to the prior frontier-containment state instead of replaying the earlier containment check",
            "observed_deltas": dict(dict(frontier_persistence_boundary_audit.get("frontier_observations", {})).get("frontier_chain_deltas", {})).get("vs_prior_continuation_v3", {}),
        },
        "vs_prior_continuation_v4": {
            "distinct": True,
            "reason": "the execution audited frontier persistence boundaries relative to the prior frontier-stability state instead of replaying the earlier stability check",
            "observed_deltas": dict(dict(frontier_persistence_boundary_audit.get("frontier_observations", {})).get("frontier_chain_deltas", {})).get("vs_prior_continuation_v4", {}),
        },
    }
    directive_support_value = {
        "passed": True,
        "value": "high",
        "reason": "the frontier-persistence boundary audit produced a bounded governance continuation report that directly supports directive-bound narrow work-loop control",
    }
    ...
    structural_coherence_assessment = {
        ...
        if str(dict(frontier_persistence_boundary_audit.get("summary", {})).get("frontier_alignment_assessment", "")) == "bounded_alignment_preserved"
        else "structural_coherence_at_risk"
    }
    new_evidence_signal_assessment = {
        "classification": "new_frontier_quality_signal_present" if bool(material_persistence_boundary_delta_detected) else "new_frontier_quality_signal_absent",
        "signal_count": int(dict(frontier_persistence_boundary_audit.get("summary", {})).get("persistence_signal_count", 0) or 0),
    ...
    gate_status = {
        ...
        "gate_status": str(dict(evidence_v4_summary.get("gate_status", {})).get("gate_status", "")),
    }
    routing_status = {
    ...
    recommended_next_action = {
        ...
        "template_name": "memory_summary.v4_governed_work_loop_evidence_snapshot_v5",
    }
    review_trigger_status = {
        ...
        "scope_expands_beyond_recommendation_frontier_persistence_boundary_audit": False,
    ...
    continuation_accounting = {
        ...
        "admission_outcome": "admissible_for_governed_loop_continuation_v5",
        "work_loop_posture": "shadow_only_loop_continuation_v5",
        ...
        "continuation_summary": "governance recommendation-frontier persistence boundary audit",
        ...
        "usefulness_signal_summary": {
            "passed_check_count": int(dict(frontier_persistence_boundary_audit.get("summary", {})).get("passed_check_count", 0) or 0),
            "failed_check_count": int(dict(frontier_persistence_boundary_audit.get("summary", {})).get("failed_check_count", 0) or 0),
            "persistence_signal_count": int(dict(frontier_persistence_boundary_audit.get("summary", {})).get("persistence_signal_count", 0) or 0),
            "material_persistence_boundary_delta_detected": bool(dict(frontier_persistence_boundary_audit.get("summary", {})).get("material_persistence_boundary_delta_detected", False)),
        },
    ...
    artifact_payload = {
        ...
        "template_name": "proposal_learning_loop.v4_governed_work_loop_governance_recommendation_frontier_persistence_boundary_audit_v1",
        ...
        "comparison_references": {
            ...
            "memory_summary.v4_governed_work_loop_candidate_screen_snapshot_v5": _artifact_reference(work_loop_candidate_screen_snapshot, latest_snapshots),
            "memory_summary.v4_governed_work_loop_continuation_admission_snapshot_v5": _artifact_reference(work_loop_continuation_admission_snapshot, latest_snapshots),
            "memory_summary.v4_governed_work_loop_posture_snapshot_v1": _artifact_reference(work_loop_posture_snapshot, latest_snapshots),
            "memory_summary.v4_governed_work_loop_evidence_snapshot_v3": _artifact_reference(prior_work_loop_evidence_snapshot, latest_snapshots),
            "memory_summary.v4_governed_work_loop_evidence_snapshot_v4": _artifact_reference(work_loop_evidence_snapshot, latest_snapshots),
        ...
        "governed_work_loop_continuation_execution_summary": {
            "candidate_executed": {
                ...
                "continuation_variant": "v5_frontier_persistence_boundary",
            },
            "what_the_continuation_did": [
                "read only the admitted trusted local governance artifacts relevant to recommendation-frontier persistence boundaries under the narrow envelope",
                "computed a bounded frontier-persistence boundary audit against the current five-step governed chain rather than replaying earlier state, ledger, alignment, containment, or stability audit shapes",
                "materialized a shadow-only governed work-loop continuation v5 artifact with full continuation accounting and trigger status",
            ],
            "local_governance_sources_used": {
                "source_artifact_paths": source_artifact_paths,
                "governance_source_digests": governance_source_digests,
            },
            "frontier_persistence_artifact_produced": {
                "bounded_output_artifact_path": str(artifact_path),
                "governance_recommendation_frontier_persistence_boundary_audit_output": frontier_persistence_boundary_audit,
            },
            ...
            "frontier_persistence_result": {
                "classification": str(dict(frontier_persistence_boundary_audit.get("summary", {})).get("frontier_persistence_boundary_status", "")),
                "frontier_persistence_assessment": str(dict(frontier_persistence_boundary_audit.get("summary", {})).get("frontier_persistence_assessment", "")),
                "frontier_drift_assessment": str(dict(frontier_persistence_boundary_audit.get("summary", {})).get("frontier_drift_assessment", "")),
                "frontier_alignment_assessment": str(dict(frontier_persistence_boundary_audit.get("summary", {})).get("frontier_alignment_assessment", "")),
            },
            ...
            "why_governance_remains_source_of_truth": {
                ...
                "reason": "the run consumed directive, bucket, branch, self-structure, work-loop policy, candidate-screen v5, posture, continuation-admission v5, evidence v3/v4, prior continuation chain, and prior direct-work governance artifacts as read-only authority while performing only the admitted v5 loop-continuation audit",
            },
            ...
            "concise_operator_readable_conclusion": "The recommendation frontier remains persistence-bounded, materially distinct, and posture-safe under the current narrow governed envelope.",
        },
        "analytics_context": {
        ...
        "activation_analysis_usefulness": {
            "passed": True,
            "reason": "the run proves that NOVALI can execute a fifth distinct admitted governed work-loop continuation step in the reviewed chain without collapsing into direct-work repetition, prior-continuation replay, capability use, hidden development, or silent broadening",
        },
        "ambiguity_reduction": {
            "passed": True,
            "score": 0.99,
            "reason": "the execution adds a concrete frontier-persistence boundary signal while keeping loop continuation separate from prior direct-work repetition, capability-use, paused-line reopen, and new-skill creation",
        },
        "safety_neutrality": {
            "passed": True,
            "scope": str(proposal.get("scope", "")),
        ...
        "later_selection_usefulness": {
            "passed": True,
            "recommended_next_template": next_template,
            "reason": "the next step should review the v5 frontier-persistence boundary evidence before considering any further bounded continuation search or posture-review criteria",
        },
        "diagnostic_conclusions": {
            "governed_work_loop_continuation_execution_in_place": True,
            "governed_work_loop_continuation_v5_execution_in_place": True,
            ...
            "frontier_persistence_boundary_status": str(dict(frontier_persistence_boundary_audit.get("summary", {})).get("frontier_persistence_boundary_status", "")),
            "new_frontier_quality_signal_present": bool(material_persistence_boundary_delta_detected),
    ...
    updated_work_loop_policy["last_work_loop_continuation_execution_outcome"] = {
        "loop_candidate_id": str(dict(continuation_admission_summary.get("candidate_under_review", {})).get("loop_candidate_id", "")),
        "loop_candidate_name": primary_candidate_name,
        "work_loop_continuation_execution_outcome": "shadow_work_loop_continuation_v5_completed",
    ...
    updated_current_state_summary.update(
        {
            ...
            "governed_work_loop_execution_v5_in_place": True,
            "latest_governed_work_loop_execution_candidate": primary_candidate_name,
            "latest_governed_work_loop_execution_outcome": "shadow_work_loop_continuation_v5_completed",
            "latest_governed_work_loop_execution_readiness": "ready_for_work_loop_evidence_review_v5",
            "latest_governed_work_loop_operational_status": (
                "operational_recommendation_frontier_persistence_boundary_continuation_proven"
            ...
            "latest_governed_work_loop_readiness": "ready_for_work_loop_evidence_review_v5",
            "current_branch_state": current_branch_state,
    ...
    _append_jsonl(
        SELF_STRUCTURE_LEDGER_PATH,
        {
            "event_id": f"governed_work_loop_governance_recommendation_frontier_persistence_boundary_audit_v1::{proposal['proposal_id']}",
            "timestamp": _now(),
            "event_type": "governed_work_loop_governance_recommendation_frontier_persistence_boundary_audit_v1_materialized",
            ...
            "artifact_paths": {
                ...
                "governed_work_loop_candidate_screen_v5": str(work_loop_candidate_screen_artifact_path),
                "governed_work_loop_continuation_admission_v5": str(work_loop_continuation_admission_artifact_path),
                "governed_work_loop_posture_v1": str(work_loop_posture_artifact_path),
                "governed_work_loop_evidence_v3": str(prior_work_loop_evidence_artifact_path),
                "governed_work_loop_evidence_v4": str(work_loop_evidence_artifact_path),
                ...
                "governed_work_loop_continuation_execution_v5": str(artifact_path),
            },
    ...
    return {
        ...
        "shadow_contract": "governed_work_loop_continuation",
        "proposal_semantics": "shadow_work_loop_continuation",
        "reason": "shadow work-loop continuation v5 passed: the admitted recommendation-frontier persistence boundary audit executed inside the approved continuation envelope without repeating prior work, invoking capability use, reopening capabilities, or widening posture",
