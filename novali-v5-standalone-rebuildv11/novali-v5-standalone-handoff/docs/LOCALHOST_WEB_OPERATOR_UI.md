...
It may materialize an eligible already-approved next objective class without another approval click only when the class is whitelisted, first-entry approval rules are satisfied, and the configured chain cap has not been reached.
...
The browser observability views expose whether auto-continue is enabled, why it did or did not happen, and whether the current step was manual or policy-auto-continued.
