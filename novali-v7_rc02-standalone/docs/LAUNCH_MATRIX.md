...
- `.\standalone_docker\run_web_operator_llm_stack.ps1` when the Ask Novali local LLM sidecar should run with the Web Shell

For packaged trusted-source onboarding, the container helper also passes through these session env vars when present:

- `OPENAI_API_KEY`
- `NOVALI_TRUSTED_SOURCE_API_BASE_URL`

For packaged browser use, Home now exposes one integrated startup surface with `Long-run / low-touch` as the practical default preset. The same surface combines initialization status visualization, per-step action controls, compact launch guidance, and the launch form itself while still distinguishing fresh-start, resume-ready, review-required, held, and long-run continuation states truthfully. When review is required, the operator review workspace groups pending decisions, review reasons, recommended actions, explicit approve/defer/reject/acknowledge actions, and links to the supporting evidence without collapsing advanced controls back onto Home. If the Controller launches a bounded child, the same operator surfaces also show the delegation lease, child scope, returned bundle, explicit Controller adoption/defer/reject/revoke outcome, the delegation plan, any blocked alternatives, and the typed Librarian-to-Verifier handoff state together with the mission or verification linkage showing whether an adopted Librarian bundle strengthened the active library-hygiene/readiness step, whether a Verifier return supported bounded adoption readiness, or whether the fixed sequential Librarian-to-Verifier workflow materially improved the mission before final Controller adoption. Use `/settings` when you want the tighter focused preset's low-level details, provider onboarding, trusted-source governance tuning, runtime policy editing, or auto-continue controls.
...
The autonomy kernel is an additive supervisor layer above that chain. It can maintain self-authored goals, local approved-source research evidence, board decisions, and Novali-stack operation proposals, but protected execution still flows through explicit evidence, approval, operation broker checks, and emergency-stop state. See `AUTONOMY_KERNEL.md`.
...
- build the image with `docker build -t novali-v7-standalone:local -f Dockerfile .`
- or use `.\standalone_docker\build_standalone_image.ps1`
- for local LLM operation, use `docker-compose.llm.yml` or `.\standalone_docker\run_web_operator_llm_stack.ps1`; Novali reaches Ollama at `http://ollama:11434` inside the compose network and defaults to `qwen3.6:35b-a3b` for Ask Novali drafts plus broker-gated local coding evidence
- export the packaged image archive with `.\standalone_docker\export_standalone_image_archive.ps1`
- include that exported archive inside the canonical handoff under `image\novali-v7-standalone.tar`
