...
This is the browser-based operator surface for the active `novali-v7` clean baseline.

Branch-status note:

- `novali-v7` is the active clean baseline
- `novali-v6` is the frozen historical reference line at `rc88.1`
- `novali-v5` remains preserved historical context only
...
- a stronger dashboard pulse that surfaces current state, operator attention, current recommendation, and recent activity before the deeper inspection views
- a stronger showcase layer that surfaces what NOVALI just accomplished, why the current result matters, and important outputs before you drop into the full artifact/evidence views
