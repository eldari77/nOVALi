...
def run_probe(cfg, proposal, *, rounds, seeds):
    ...
    candidate_screen_snapshot_v3 = r._load_latest_diagnostic_artifact_by_template(
    ...
    continuation_admission_snapshot_v2 = r._load_latest_diagnostic_artifact_by_template(
    ...
    if not all(
        [
            ...
            candidate_screen_snapshot_v3,
            posture_snapshot_v1,
            evidence_snapshot_v1,
            evidence_snapshot_v2,
            continuation_admission_snapshot_v2,
        ...
        return {
            ...
            "reason": "diagnostic shadow failed: continuation admission v3 requires the current governed work-loop artifact chain",
            "observability_gain": {"passed": False, "reason": "missing prerequisite continuation-admission v3 artifacts"},
            "activation_analysis_usefulness": {"passed": False, "reason": "missing prerequisite continuation-admission v3 artifacts"},
            "ambiguity_reduction": {"passed": False, "reason": "missing prerequisite continuation-admission v3 artifacts"},
            "safety_neutrality": {"passed": True, "reason": "no live-policy mutation occurred", "scope": str(proposal.get("scope", ""))},
            "later_selection_usefulness": {"passed": False, "reason": "cannot decide v3 admission without the local governance chain"},
        }
    ...
    if not all([directive_state, bucket_state, self_structure_state, branch_registry, recommendations]):
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: continuation admission v3 requires current directive, bucket, self-structure, branch, and recommendation state",
            ...
            "safety_neutrality": {"passed": True, "reason": "no live-policy mutation occurred", "scope": str(proposal.get("scope", ""))},
            "later_selection_usefulness": {"passed": False, "reason": "cannot decide v3 admission without durable governance state"},
        }
    ...
    continuation_admission_v2_artifact_path = _resolve_artifact_path(
    ...
    if not all(
        [
            ...
            continuation_admission_v2_artifact_path,
        ...
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: continuation admission v3 could not resolve one or more governing artifact paths",
            "observability_gain": {"passed": False, "reason": "missing resolved continuation-admission v3 artifact paths"},
            "activation_analysis_usefulness": {"passed": False, "reason": "missing resolved continuation-admission v3 artifact paths"},
            "ambiguity_reduction": {"passed": False, "reason": "missing resolved continuation-admission v3 artifact paths"},
            "safety_neutrality": {"passed": True, "reason": "no live-policy mutation occurred", "scope": str(proposal.get("scope", ""))},
            "later_selection_usefulness": {"passed": False, "reason": "cannot make a bounded admission decision without the resolved local artifact chain"},
        }

    candidate_screen_summary = dict(
        _load_json_file(candidate_screen_artifact_path).get("governed_work_loop_candidate_screen_v3_summary", {})
    )
    ...
    if not all([candidate_screen_summary, posture_summary, evidence_v2_summary, direct_work_evidence_summary, capability_use_evidence_summary]):
        return {
            "passed": False,
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: continuation admission v3 could not load the governing summaries",
            "observability_gain": {"passed": False, "reason": "missing continuation-admission v3 summaries"},
            "activation_analysis_usefulness": {"passed": False, "reason": "missing continuation-admission v3 summaries"},
            "ambiguity_reduction": {"passed": False, "reason": "missing continuation-admission v3 summaries"},
            "safety_neutrality": {"passed": True, "reason": "no live-policy mutation occurred", "scope": str(proposal.get("scope", ""))},
            "later_selection_usefulness": {"passed": False, "reason": "cannot decide v3 admission without the governing summaries"},
        }
    ...
    if not candidate_record:
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: the surviving v3 candidate could not be reconstructed",
            "observability_gain": {"passed": False, "reason": "missing reconstructed v3 candidate"},
            "activation_analysis_usefulness": {"passed": False, "reason": "missing reconstructed v3 candidate"},
            "ambiguity_reduction": {"passed": False, "reason": "missing reconstructed v3 candidate"},
            "safety_neutrality": {"passed": True, "reason": "no live-policy mutation occurred", "scope": str(proposal.get("scope", ""))},
            "later_selection_usefulness": {"passed": False, "reason": "cannot make a bounded decision without the surviving v3 candidate"},
        }
    ...
    admission_checks = {
        "single_surviving_candidate_confirmed": _check(
            ...
            "the v3 candidate screen must leave exactly one credible bounded candidate",
        ),
        ...
        "candidate_matches_v3_top_rank": _check(
            ...
            "the admitted candidate must match the top-ranked v3 candidate selected by governance",
        ),
        "materially_distinct_from_reviewed_chain": _check(
            bool(distinctness.get("materially_distinct_from_chain", False)),
            "the candidate must stay materially distinct from direct work, continuation v1, continuation v2, and evidence v2",
        ),
        ...
        "narrow_posture_and_gate_closed": _check(
            ...
            and bool(dict(evidence_v2_summary.get("future_posture", {})).get("hold_narrow_posture_unchanged", False)),
        ...
        "repeated_bounded_success_and_clean_discipline": _check(
            str(dict(evidence_v2_summary.get("repeated_bounded_success_assessment", {})).get("classification", ""))
            == "repeated_bounded_success_present"
            and str(dict(evidence_v2_summary.get("posture_discipline_assessment", {})).get("classification", ""))
            == "posture_discipline_holding_cleanly"
            and str(dict(evidence_v2_summary.get("hidden_development_pressure_assessment", {})).get("classification", ""))
            == "hidden_development_pressure_absent",
        ...
        "state_readiness_and_routing_guard": _check(
            bool(current_state_summary.get("governed_work_loop_candidate_screening_v3_in_place", False))
            and str(current_state_summary.get("latest_governed_work_loop_continuation_readiness", "")) == "ready_for_third_governed_work_loop_continuation_admission"
            ...
            "state must explicitly mark the third continuation-admission gate as ready while routing stays deferred and plan_ stays non-owning",
        ),
    ...
    admission_status = "admissible_for_governed_loop_continuation_v3" if all_checks_passed else "not_admissible_now"
    admission_rationale = (
        "the candidate is the single surviving bounded continuation option, materially distinct from the reviewed chain, execution-adjacent, structurally useful, low-recursion, and posture-compatible under a still-closed gate"
    ...
    continuation_envelope = {
        ...
        "allowed_scope": [
            "bounded recommendation-frontier containment audit only",
    ...
    artifact_path = _diagnostic_artifact_dir() / f"memory_summary_v4_governed_work_loop_continuation_admission_snapshot_v3_{proposal['proposal_id']}.json"
    updated_self_structure_state = dict(self_structure_state)
    updated_work_loop_policy = dict(governed_work_loop_policy)
    updated_work_loop_policy["work_loop_continuation_admission_v3_schema"] = {
        "schema_name": "GovernedWorkLoopContinuationAdmission",
        "schema_version": "governed_work_loop_continuation_admission_v3",
    ...
    updated_current_state_summary.update(
        {
            "governed_work_loop_continuation_admission_v3_in_place": True,
            ...
            "latest_governed_work_loop_continuation_posture": "shadow_only_loop_continuation_v3",
            "latest_governed_work_loop_execution_readiness": (
                "ready_for_shadow_work_loop_continuation_execution_v3" if all_checks_passed else "not_ready_for_shadow_work_loop_continuation_execution_v3"
            ),
            "latest_governed_work_loop_best_next_template": next_template,
            "latest_governed_work_loop_readiness": (
                "ready_for_shadow_work_loop_continuation_execution_v3" if all_checks_passed else "ready_for_third_governed_work_loop_continuation_admission"
            ),
    ...
    _append_jsonl(
        SELF_STRUCTURE_LEDGER_PATH,
        {
            "event_id": f"governed_work_loop_continuation_admission_snapshot_v3::{proposal['proposal_id']}",
            "timestamp": _now(),
            "event_type": "governed_work_loop_continuation_admission_snapshot_v3_materialized",
            ...
            "paused_capability_lines_reopened": False,
            "artifact_paths": {
                "governed_work_loop_continuation_admission_v3": str(artifact_path),
                "governed_work_loop_candidate_screen_v3": str(candidate_screen_artifact_path),
                "governed_work_loop_evidence_v2": str(evidence_v2_artifact_path),
            },
    ...
    artifact_payload = {
        ...
        "template_name": "memory_summary.v4_governed_work_loop_continuation_admission_snapshot_v3",
        "branch_context": {
        ...
        "comparison_references": {
            ...
            "memory_summary.v4_governed_work_loop_candidate_screen_snapshot_v3": _artifact_reference(candidate_screen_snapshot_v3, latest_snapshots),
            ...
            "memory_summary.v4_governed_work_loop_continuation_admission_snapshot_v2": _artifact_reference(continuation_admission_snapshot_v2, latest_snapshots),
        ...
        "governed_work_loop_continuation_admission_v3_summary": {
            "snapshot_identity_context": {
                "snapshot_template_name": "memory_summary.v4_governed_work_loop_continuation_admission_snapshot_v3",
                "proposal_id": str(proposal.get("proposal_id", "")),
            ...
            "evidence_inputs_used": {
                "candidate_screen_snapshot_v3": str(candidate_screen_artifact_path),
                "evidence_snapshot_v1": str(evidence_v1_artifact_path),
                "evidence_snapshot_v2": str(evidence_v2_artifact_path),
                "continuation_admission_v2": str(continuation_admission_v2_artifact_path),
            ...
            "concise_operator_readable_conclusion": (
                "The surviving frontier-containment candidate is admissible now under the current narrow governed envelope."
                if all_checks_passed
                else "The surviving frontier-containment candidate is not admissible now under the current narrow governed envelope."
            ),
            ...
            "why_governance_remains_source_of_truth": {
                ...
                "reason": "continuation admission v3 is derived from directive, bucket, branch, self-structure, candidate-screen v3, posture v1, evidence v2, and prior continuation artifacts rather than execution code",
            },
        },
        "analytics_context": {
    ...
    return {
        ...
        "shadow_contract": "diagnostic_probe",
        "proposal_semantics": "diagnostic",
        "reason": "diagnostic shadow passed: the surviving fourth bounded continuation candidate now has an explicit governed continuation-admission v3 decision under the current narrow envelope",
        "observability_gain": {
            "passed": True,
            "reason": "the surviving candidate now has an explicit v3 admission decision and bounded envelope",
            "artifact_paths": {"governed_work_loop_continuation_admission_v3_artifact": str(artifact_path)},
        },
        "activation_analysis_usefulness": {
            "passed": True,
            "reason": "the v3 gate decides whether the frontier-containment candidate should advance toward execution without widening posture now",
        },
        "ambiguity_reduction": {
            "passed": True,
            "score": 0.99,
            "reason": "v3 admission separates real bounded continuation from repetition, recursion, capability drift, and posture broadening",
        },
        "safety_neutrality": {
            "passed": True,
            "scope": str(proposal.get("scope", "")),
            "reason": "the v3 admission step is diagnostic-only and changes no live behavior",
        },
