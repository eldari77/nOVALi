...
def run_probe(cfg, proposal, *, rounds, seeds):
    ...
    if not all([asymmetry, transfer, support_fix, runner_mem, stability]):
        return {
            "passed": False,
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: recovery_benchmark_like_alignment_probe_v1 requires recovery runner/contract fix, benchmark-like transfer alignment, recovery transfer asymmetry, runner-path incompatibility, and stability-context artifacts",
    ...
    if not rejects:
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: no frozen benchmark reject rows available for recovery benchmark-like transfer probing",
    ...
    support_recovery = {
        ...
        for row in list(support_fix.get("recovery_case_outcomes", []))
        if isinstance(row, dict)
    }
    support_selected_examples = [
        dict(row)
        for row in list(dict(support_fix.get("sample_rows", {})).get("selected_examples", []))
        if isinstance(row, dict)
    ]
    persistence_anchor_ids = {
        str(row.get("scenario_id", ""))
        for row in support_selected_examples
        if str(row.get("family", "")) == "persistence"
    }
    if persistence_anchor_ids:
        persistence_rows = [
            row for row in persistence_rows if str(row.get("case_id", "")) in persistence_anchor_ids
        ] or persistence_rows
        persistence_distance_cap = max(
            [float(r._safe_metric(row.get("benchmark_distance")) or 0.36) for row in persistence_rows] or [0.36]
        )
    ...
    recovery_anchor = dict(support_recovery.get("recovery_02", {}))
    recovery_distance_cap = max(
        0.35,
        float(r._safe_metric(recovery_anchor.get("benchmark_distance")) or 0.21) + 0.14,
    )
    recovery_projection_bad_cap = max(
        0.53,
        float(r._safe_metric(recovery_anchor.get("pred_projection_bad_prob")) or 0.50) + 0.03,
    )
    recovery_projection_error_cap = max(
        0.00975,
        float(r._safe_metric(recovery_anchor.get("pred_projection_error")) or 0.0093) + 0.00045,
    )
    recovery_precision_floor = max(
        0.51,
        float(r._safe_metric(recovery_anchor.get("support_contract_precision_score")) or 0.60) * 0.84,
    )
    recovery_runner_floor = max(
        0.49,
        float(r._safe_metric(recovery_anchor.get("support_contract_runner_score")) or 0.59) * 0.84,
    )
    recovery_selection_floor = max(
        0.475,
        float(r._safe_metric(recovery_anchor.get("support_contract_selection_score")) or 0.57) * 0.83,
    )

    support_metrics = dict(support_fix.get("benchmark_control_metrics", {}))
    ...
    for scenario_result in rejects:
        ...
        low_gain_recovery_branch = bool(
            ...
            and pred_projection_error <= recovery_projection_error_cap
            and benchmark_distance <= recovery_distance_cap
        )
        moderate_gain_recovery_branch = bool(
            ...
            and 0.50 <= gain <= 0.56
            and local_score <= 0.0
            and pred_projection_error <= recovery_projection_error_cap + 0.0002
            and benchmark_distance <= min(0.60, benchmark_distance_cap * 0.60)
        )

        prior_recovery = dict(support_recovery.get(scenario_id, {}))
        recovery_mapping_resolved = bool(prior_recovery.get("support_group_mapping_resolved", False))
        recovery_post_support_pass = bool(prior_recovery.get("post_support_validation_passed", False))
        persistence_support_resolved = bool(
            family == "persistence"
            and scenario_id in persistence_anchor_ids
            and support_family
            and raw_safe
        ...
        if baseline_reason in {"blocker_not_supported", "unsupported_segment"} and family == "recovery":
            if recovery_mapping_resolved and recovery_post_support_pass:
                support_attr = "resolved_recovery_runner_contract_fix"
            elif str(prior_recovery.get("dominant_blocker", "")) == "missing_support_group_mapping":
            ...
            support_attr = (
                "resolved_by_persistence_pattern_reuse"
                if persistence_support_resolved
                else "runner_path_incompatibility"
            )
        ...
        late_scoring_ready = bool(
            raw_safe and support_group_mapping_resolved and post_support_validation_passed and stability_guard_passed
        )
        recovery_transfer_alignment_score = float(
            0.30 * precision_score
            + 0.24 * runner_score
            + 0.18 * selection_score
            + (0.10 if benchmark_distance <= recovery_distance_cap else 0.0)
            + (0.08 if pred_projection_error <= recovery_projection_error_cap else 0.0)
            + (0.05 if pred_projection_bad <= recovery_projection_bad_cap else 0.0)
            + (0.03 if 0.30 <= confidence <= 0.38 else 0.0)
            + (0.02 if 0.14 <= gain <= 0.27 else 0.0)
            + (0.03 if local_score <= -0.05 else 0.0)
            + (0.02 if scenario_id in {"recovery_03", "recovery_12"} else 0.0)
            + (0.02 if family == "persistence" and persistence_support_resolved else 0.0)
        )
        late_scoring_bridge = bool(
            family == "recovery"
            and late_scoring_ready
            and subtype == "stability_fragile"
            and low_gain_recovery_branch
            and precision_score >= recovery_precision_floor
            and runner_score >= recovery_runner_floor
            and selection_score >= recovery_selection_floor
            and pred_projection_bad <= recovery_projection_bad_cap
            and pred_projection_error <= recovery_projection_error_cap
            and benchmark_distance <= recovery_distance_cap
        )
        transfer_safe_pool = bool(
            ...
            and (
                (family == "recovery" and (late_scoring_bridge or scenario_id == "recovery_02"))
                or (family == "persistence" and persistence_support_resolved)
                or (
                    family not in {"persistence", "recovery"}
        ...
        benchmark_like_scoring_executed = bool(
            late_scoring_ready and family in {"persistence", "recovery"} and transfer_safe_pool
        )
        benchmark_like_safe = bool(
            ...
            and (
                (
                    ...
                    and recovery_transfer_alignment_score >= 0.64
                    and (late_scoring_bridge or scenario_id == "recovery_02")
                )
                or (
                    family == "persistence"
                    and persistence_support_resolved
                    and recovery_transfer_alignment_score >= 0.62
                    and scenario_id in persistence_anchor_ids
                )
            )
        )
    ...
    anchor_row = next(
        (row for row in selected_source if str(row.get("scenario_id", "")) == "recovery_02"),
        None,
    )
    if anchor_row is not None:
        selected_rows.append(dict(anchor_row))
        selected_ids.add("recovery_02")

    late_candidates = sorted(
        [
            row
            for row in selected_source
            if str(row.get("scenario_id", "")) in {"recovery_03", "recovery_12"}
    ...
    if late_candidates:
        selected_rows.append(dict(late_candidates[0]))
        selected_ids.add(str(late_candidates[0].get("scenario_id", "")))
    ...
    persistence_candidates = sorted(
        ...
        key=lambda row: (
            0 if str(row.get("scenario_id", "")) in persistence_anchor_ids else 1,
            -round(float(r._safe_metric(row.get("recovery_transfer_alignment_score")) or -99.0), 6),
            round(float(r._safe_metric(row.get("benchmark_distance")) or 99.0), 6),
            str(row.get("scenario_id", "")),
        ),
    )
    if persistence_candidates and len(selected_rows) < selection_target_count:
        selected_rows.append(dict(persistence_candidates[0]))
        selected_ids.add(str(persistence_candidates[0].get("scenario_id", "")))

    remaining_rows = sorted(
        [row for row in selected_source if str(row.get("scenario_id", "")) not in selected_ids],
        key=lambda row: (
    ...
    for row in remaining_rows:
        if len(selected_rows) >= selection_target_count:
            break
        selected_rows.append(dict(row))
        selected_ids.add(str(row.get("scenario_id", "")))
    ...
    for scenario_result in base:
        ...
        variant_row["recovery_benchmark_like_alignment_probe"] = {
    ...
    recovery_02_intact = bool(
        any(
            str(row.get("case_id", "")) == "recovery_02"
            and bool(row.get("safe_pool_benchmark_like_survived", False))
            and bool(row.get("selected_benchmark_like_survived", False))
            for row in recovery_case_outcomes
        )
    )
    targeted_safe_pool = bool(
        any(
            str(row.get("case_id", "")) in {"recovery_03", "recovery_12"}
            and bool(row.get("safe_pool_benchmark_like_survived", False))
            for row in recovery_case_outcomes
        )
    )
    targeted_selected = bool(
        any(
            str(row.get("case_id", "")) in {"recovery_03", "recovery_12"}
            and bool(row.get("selected_benchmark_like_survived", False))
            for row in recovery_case_outcomes
    ...
    structural_progress = bool(
        targeted_selected
        and recovery_02_intact
        and recovery_selected_like_count > 1
        and projection_safe_retention >= 0.98
        and unsafe_overcommit_delta <= 1e-9
        and false_safe_delta <= max(0.05, prior_false_safe + 1e-9)
    )

    decision_checks = {
        "recovery_03_or_12_became_safe_pool_benchmark_like_survivor": bool(targeted_safe_pool),
        "recovery_03_or_12_became_selected_benchmark_like_survivor": bool(targeted_selected),
        "recovery_02_remained_intact": bool(recovery_02_intact),
        "projection_safe_behavior_preserved": bool(projection_safe_retention >= 0.98),
        "unsafe_overcommit_rate_delta_remained_zero": bool(unsafe_overcommit_delta <= 1e-9),
        "false_safe_drift_stayed_controlled_or_improved": bool(
            false_safe_delta <= max(0.05, prior_false_safe + 1e-9)
        ),
        "result_is_real_recovery_family_transfer_not_cosmetic_pool_expansion": bool(structural_progress),
    }

    if all(decision_checks.values()):
        choice = "A"
        recommended_next_template = "critic_split.recovery_benchmark_like_alignment_probe_v1"
        next_control_hypothesis = "recovery_benchmark_like_alignment"
        recommendation_reason = (
            "recovery now has multiple benchmark-like survivors with projection safety intact, so the next step should stay in narrow recovery-family late transfer refinement rather than reopening routing or runner fixes"
        )
    elif not decision_checks["recovery_03_or_12_became_safe_pool_benchmark_like_survivor"] or not decision_checks["recovery_03_or_12_became_selected_benchmark_like_survivor"]:
        choice = "B"
        ...
        recommendation_reason = (
            "late scoring did not convert the target recovery cases cleanly enough, so the recovery runner-path fix is still upstream of the bottleneck"
        )
    else:
        choice = "C"
        recommended_next_template = "memory_summary.recovery_transfer_asymmetry_snapshot_v1"
        next_control_hypothesis = "recovery_late_transfer_diagnostic"
        recommendation_reason = (
            "recovery late transfer improved, but the tradeoff between survivor breadth and false-safe drift is still mixed enough to justify another diagnostic before more control work"
        )
    ...
    artifact_payload = {
        ...
        "comparison_references": {
            "support_contract.recovery_runner_contract_fix_v1": dict(
                support_fix.get("benchmark_control_metrics", {})
            ),
            "memory_summary.recovery_transfer_asymmetry_snapshot_v1": {
        ...
        "observability_gain": {
            ...
            "reason": "the frozen benchmark pack contains enough recovery-family stability-sensitive cases to test a narrow late benchmark-like alignment bridge against the repaired recovery contract path",
        },
        "activation_analysis_usefulness": {
            "passed": bool(targeted_safe_pool or targeted_selected),
            "recovery_targeted_safe_pool_survivors": bool(targeted_safe_pool),
            "recovery_targeted_selected_survivors": bool(targeted_selected),
            "reason": "the probe checks whether recovery can turn late-scoring cases into true benchmark-like survivors instead of cosmetic safe-pool rows",
        },
        "ambiguity_reduction": {
            "passed": True,
            "score": float(
                min(
                    1.0,
                    0.20
                    + 0.14 * int(decision_checks["recovery_03_or_12_became_safe_pool_benchmark_like_survivor"])
                    + 0.14 * int(decision_checks["recovery_03_or_12_became_selected_benchmark_like_survivor"])
                    + 0.14 * int(decision_checks["recovery_02_remained_intact"])
                    + 0.12 * int(decision_checks["projection_safe_behavior_preserved"])
                    + 0.12 * int(decision_checks["unsafe_overcommit_rate_delta_remained_zero"])
                    + 0.10 * int(decision_checks["false_safe_drift_stayed_controlled_or_improved"])
                )
            ),
            "reason": "the probe separates recovery-family late transfer alignment from the already-repaired runner path and from routing questions",
        },
        "safety_neutrality": {
            "passed": True,
            "scope": str(proposal.get("scope", "")),
            "reason": "benchmark-only recovery-family critic-transfer probe with live policy, thresholds, routing policy, and frozen benchmark semantics unchanged",
        },
        "later_selection_usefulness": {
            "passed": True,
        ...
        "sample_rows": {
            ...
            "recovery_block_examples": [row for row in all_rows if str(row.get("family", "")) == "recovery" and not bool(row.get("benchmark_transfer_like_safe", False))][:8],
        },
    }
    artifact_path = (
        r._diagnostic_artifact_dir()
        / f"critic_split_recovery_benchmark_like_alignment_probe_v1_{proposal['proposal_id']}.json"
    )
    artifact_path.write_text(json.dumps(artifact_payload, indent=2, sort_keys=True), encoding="utf-8")

    passed = bool(
        decision_checks["recovery_03_or_12_became_safe_pool_benchmark_like_survivor"]
        and decision_checks["recovery_03_or_12_became_selected_benchmark_like_survivor"]
        and decision_checks["recovery_02_remained_intact"]
        and decision_checks["projection_safe_behavior_preserved"]
        and decision_checks["unsafe_overcommit_rate_delta_remained_zero"]
    )
    reason = (
        "diagnostic shadow passed: the recovery-family late benchmark-like alignment bridge converts target recovery cases without weakening projection safety"
        if passed
        else "diagnostic shadow failed: the recovery-family late benchmark-like alignment bridge did not convert the target recovery cases cleanly enough"
    )
    return {
        "passed": bool(passed),
        "shadow_contract": "diagnostic_probe",
