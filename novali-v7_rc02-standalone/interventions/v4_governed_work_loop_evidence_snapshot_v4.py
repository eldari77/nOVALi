...
def _select_latest_successful_frontier_stability_artifact(preferred_path: Path | None) -> Path | None:
    pattern = "proposal_learning_loop_v4_governed_work_loop_governance_recommendation_frontier_stability_delta_audit_v1_*.json"
    ...
    for candidate in candidates:
        ...
        result = dict(execution_summary.get("frontier_stability_result", {}))
        ...
        if (
            str(result.get("classification", "")) in {"stable_with_material_delta", "stable_without_material_delta"}
...
def _frontier_signal_summary(execution_summary: dict[str, Any]) -> dict[str, Any]:
    frontier_artifact = dict(execution_summary.get("frontier_stability_artifact_produced", {}))
    audit_output = dict(frontier_artifact.get("governance_recommendation_frontier_stability_delta_audit_output", {}))
    ...
    return {
        ...
        "frontier_stability_status": str(
            result.get("classification", summary.get("frontier_stability_delta_status", ""))
        ),
        ...
        "material_frontier_signal_detected": bool(
            usefulness.get(
                "material_stability_delta_detected",
                summary.get(
                    "material_stability_delta_detected",
...
def _operational_usefulness_assessment(execution_summary: dict[str, Any]) -> dict[str, Any]:
    ...
    operationally_meaningful = (
        ...
        and signal_summary["frontier_stability_status"] == "stable_with_material_delta"
    ...
    return {
        ...
        "audit_signal_summary": {
            "frontier_stability_status": signal_summary["frontier_stability_status"],
...
def _distinctness_chain_assessment(
    *,
    ...
    continuation_v4_name = str(dict(current_execution_summary.get("candidate_executed", {})).get("loop_candidate_name", ""))
    unique_names = {
        name
        for name in [
            direct_work_name,
    ...
    v4_checks = dict(
        dict(
            dict(current_execution_summary.get("frontier_stability_artifact_produced", {})).get(
                "governance_recommendation_frontier_stability_delta_audit_output", {}
            )
        ).get("checks", {})
    )
    v4_vs_chain = dict(dict(v4_checks.get("frontier_materially_distinct", {})).get("observed", {}))
    structurally_distinct = (
        bool(prior_distinctness.get("passed", False))
        and int(prior_distinctness.get("unique_step_name_count", 0) or 0) == 4
        and len(unique_names) == 5
        and bool(v4_vs_chain.get("distinct_from_direct_work", False))
        and bool(v4_vs_chain.get("distinct_from_continuation_v1", False))
        and bool(v4_vs_chain.get("distinct_from_continuation_v2", False))
        and bool(v4_vs_chain.get("distinct_from_continuation_v3", False))
        and bool(v4_vs_chain.get("distinct_from_evidence_snapshot_v2", False))
        and bool(v4_vs_chain.get("distinct_from_evidence_snapshot_v3", False))
    )
    return {
        ...
        "vs_prior_direct_work": {
            "continuation_v4": bool(v4_vs_chain.get("distinct_from_direct_work", False)),
        },
        "vs_prior_continuation_v1": {
            "continuation_v4": bool(v4_vs_chain.get("distinct_from_continuation_v1", False)),
        },
        "vs_prior_continuation_v2": bool(v4_vs_chain.get("distinct_from_continuation_v2", False)),
        "vs_prior_continuation_v3": bool(v4_vs_chain.get("distinct_from_continuation_v3", False)),
        "vs_evidence_snapshot_v2": bool(v4_vs_chain.get("distinct_from_evidence_snapshot_v2", False)),
        "vs_evidence_snapshot_v3": bool(v4_vs_chain.get("distinct_from_evidence_snapshot_v3", False)),
        "reason": (
            "direct work, continuation v1, continuation v2, frontier containment, and frontier stability delta are all measurably distinct bounded steps rather than diminishing-return governance recursion"
...
def _repeated_bounded_success_assessment(
    *,
    ...
    repeated_bounded_success = success_count == 5
    strengthened = repeated_bounded_success and prior_success_count == 4
    return {
        ...
        "reason": (
            "repeated bounded success is now stronger at the chain level because the fifth bounded execution step also executed cleanly"
            if strengthened
...
def _posture_discipline_assessment(
    *,
    ...
    return {
        ...
        "reason": (
            "narrow posture discipline is still holding cleanly across the full reviewed chain through the frontier-stability delta execution"
            if discipline_holds
...
def _structural_vs_recursive_assessment(
    *,
    ...
    low_recursion_check = bool(
        dict(
            dict(
                dict(execution_summary.get("frontier_stability_artifact_produced", {})).get(
                    "governance_recommendation_frontier_stability_delta_audit_output", {}
                )
            ).get("checks", {})
        )
...
def run_probe(cfg, proposal, *, rounds, seeds):
    ...
    if not all(
        ...
        return {
            ...
            "reason": "diagnostic shadow failed: work-loop evidence v4 requires the governance substrate, work-loop policy, candidate screen v4, continuation admission v4, posture v1, evidence v1, evidence v3, frontier-stability execution, frontier-containment execution, prior continuation executions, direct-work evidence, and capability-use evidence artifacts",
    ...
    if (
        ...
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: branch must remain paused_with_baseline_held, governed work-loop policy state must exist, and state must be at or beyond ready_for_work_loop_evidence_review_v4",
    ...
    current_execution_artifact_path = _select_latest_successful_frontier_stability_artifact(
    ...
    if not all(
        ...
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: expected governed work-loop v4, posture, direct-work, or capability-use artifact paths could not be resolved",
    ...
    if not all(
        ...
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: one or more required governed work-loop v4, posture, direct-work, or capability-use artifacts could not be loaded",
    ...
    candidate_screen_summary = dict(candidate_screen_payload.get("governed_work_loop_candidate_screen_v4_summary", {}))
    continuation_admission_summary = dict(
        continuation_admission_payload.get("governed_work_loop_continuation_admission_v4_summary", {})
    )
    ...
    prior_chain_evidence_summary = dict(
        prior_chain_evidence_payload.get("governed_work_loop_evidence_v3_summary", {})
    )
    ...
    if not all(
        ...
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: required governed work-loop v4, evidence v1, evidence v3, or direct-work summaries were missing from the loaded artifacts",
    ...
    future_posture_reason = (
        "the five-step governed work-loop chain is still accumulating distinct structural evidence under the narrow envelope, so the safe result remains to hold posture narrow and keep broader execution closed"
    ...
    next_template = (
        ...
        else "memory_summary.v4_governed_work_loop_evidence_snapshot_v5"
    ...
    question_answers = {
        ...
        "new_stability_delta_audit_materially_improves_evidence_picture": str(new_execution_signal.get("classification", ""))
        == "materially_improved_evidence_picture",
        "repeated_bounded_success_now_stronger_than_evidence_v3": str(
    ...
    artifact_path = _diagnostic_artifact_dir() / f"memory_summary_v4_governed_work_loop_evidence_snapshot_v4_{proposal['proposal_id']}.json"
    ...
    updated_work_loop_policy["governed_work_loop_evidence_review_schema"] = {
        ...
        "schema_version": "governed_work_loop_evidence_review_v4",
        "required_fields": [
    ...
    updated_work_loop_policy["last_work_loop_evidence_outcome"] = {
        ...
        "status": "hold_narrow_posture_after_evidence_v4",
    ...
    updated_current_state_summary.update(
        {
            ...
            "latest_governed_work_loop_evidence_outcome": "hold_narrow_posture_after_evidence_v4",
            ...
            "latest_governed_work_loop_operational_status": (
                "five_step_bounded_governed_work_loop_chain_reviewed"
                if bool(repeated_bounded_success.get("passed", False))
                else "five_step_bounded_governed_work_loop_chain_not_yet_clear"
            ),
            "latest_governed_work_loop_execution_outcome": str(
                dict(current_execution_summary.get("frontier_stability_result", {})).get("classification", "")
            ),
            "latest_governed_work_loop_readiness": (
                "ready_for_work_loop_candidate_screen_v5_later"
                if next_action_class == "bounded_candidate_search_later_supported"
                else "ready_for_work_loop_evidence_review_v5_later"
                if next_action_class == "bounded_evidence_review_later_supported"
    ...
    chain_state = {
        ...
        "continuation_v4": {
            ...
            "frontier_stability_delta_status": str(
                dict(current_execution_summary.get("frontier_stability_result", {})).get("classification", "")
            ),
            "frontier_drift_assessment": str(
                dict(current_execution_summary.get("frontier_stability_result", {})).get(
    ...
    evidence_inputs_used = {
        ...
        "governed_work_loop_candidate_screen_v4": str(candidate_screen_artifact_path),
        "governed_work_loop_continuation_admission_v4": str(continuation_admission_artifact_path),
        ...
        "governed_work_loop_evidence_v3": str(prior_chain_evidence_artifact_path),
        ...
        "governed_work_loop_continuation_execution_v4": str(current_execution_artifact_path),
    ...
    operator_readable_conclusion = (
        "The five-step governed work-loop chain is still accumulating distinct structural evidence under the narrow envelope; the frontier-stability execution materially improved the evidence picture without widening posture, so the safe next move is another bounded candidate search later while keeping the gate closed."
        if next_action_class == "bounded_candidate_search_later_supported"
        else "The frontier-stability execution stayed clean and bounded, but the safest next move is still another evidence-only review later rather than another candidate search."
        if next_action_class == "bounded_evidence_review_later_supported"
    ...
    ledger_event = {
        "event_id": f"governed_work_loop_evidence_snapshot_v4::{proposal['proposal_id']}",
        "timestamp": _now(),
        "event_type": "governed_work_loop_evidence_snapshot_v4_materialized",
        ...
        "artifact_paths": {
            ...
            "governed_direct_work_execution_v1": str(direct_work_execution_artifact_path),
            "governed_capability_use_evidence_v1": str(capability_use_evidence_artifact_path),
            "governed_work_loop_evidence_v4": str(artifact_path),
        },
    ...
    artifact_payload = {
        ...
        "template_name": "memory_summary.v4_governed_work_loop_evidence_snapshot_v4",
        ...
        "comparison_references": {
            ...
            "memory_summary.v4_governed_work_loop_evidence_snapshot_v3": _artifact_reference(prior_chain_evidence_snapshot, latest_snapshots),
            ...
            "proposal_learning_loop.v4_governed_work_loop_governance_recommendation_frontier_stability_delta_audit_v1": _artifact_reference(current_execution_snapshot, latest_snapshots),
        ...
        "governed_work_loop_evidence_v4_summary": {
            "snapshot_identity_context": {
                "template_name": "memory_summary.v4_governed_work_loop_evidence_snapshot_v4",
                "proposal_id": str(proposal.get("proposal_id", "")),
                "evaluation_semantics": str(proposal.get("evaluation_semantics", "")),
            ...
            "recommended_next_action": {
                ...
                "reason": (
                    "the five-step chain is still structurally yielding under the narrow envelope, so another bounded candidate search later is the cleanest next move"
                    if next_action_class == "bounded_candidate_search_later_supported"
            ...
            "current_work_state_summary": {
                ...
                "continuation_v4_execution_summary_present": bool(current_execution_payload),
            ...
            "why_governance_remains_source_of_truth": {
                ...
                "reason": "the v4 evidence review is derived from directive, bucket, branch, self-structure, work-loop policy, work-loop posture, candidate-screen v4, continuation-admission v4, evidence v1, evidence v3, direct-work evidence, continuation executions, frontier-containment execution, frontier-stability execution, and capability-use evidence artifacts rather than from execution code",
            },
        },
        "analytics_context": {
        ...
        "observability_gain": {
            "passed": True,
            "reason": "the governed work-loop chain now has an explicit governance-owned v4 evidence review consolidating the frontier-stability execution into a five-step bounded operator-readable artifact",
            "artifact_paths": {
        ...
        "ambiguity_reduction": {
            "passed": True,
            "score": 0.99,
            "reason": "the v4 evidence review separates chain-level structural value, frontier-stability signal, posture pressure, and later bounded-next-step support in one grounded decision artifact",
        },
        "safety_neutrality": {
            "passed": True,
        ...
        "diagnostic_conclusions": {
            ...
            "new_stability_delta_audit_materially_changed_evidence_picture": bool(new_execution_signal.get("passed", False)),
    ...
    return {
        ...
        "shadow_contract": "diagnostic_probe",
        "proposal_semantics": "diagnostic",
        "reason": "diagnostic shadow passed: the five-step governed work-loop chain now has a v4 evidence review that keeps posture narrow, keeps the gate closed, and determines whether bounded candidate search later is still supported",
