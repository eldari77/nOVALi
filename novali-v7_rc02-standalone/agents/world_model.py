...
class RSSMWorldModel(nn.Module):
    ...
    def compute_loss(
        ...
        # Nonlinear metrics must reduce after evaluating each sample. Reducing
        # states first lets opposite errors/motions cancel and understates risk.
        self._last_debug.update(self.layout.state_metrics(pred_next.detach(), action=actions.detach()))
        self._last_debug.update(
            self.layout.transition_metrics(
                states.detach(), next_states.detach(), predicted_next=pred_next.detach()
            )
        )
