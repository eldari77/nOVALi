...
class SelfImprovementController(nn.Module):
    ...
    def __init__(
        ...
        # Tensor-only state preserves the experiment runner's clone/rollback
        # contract, which copies every state_dict value with detach().clone().
        self.register_buffer("_learning_state", torch.zeros(3, dtype=torch.float64))
        self.last_diagnostic: Dict[str, float] = {}
        self.last_outcome: Dict[str, float] = {}

    @property
    def cooldown(self) -> int:
        return int(self._learning_state[0].item())

    @cooldown.setter
    def cooldown(self, value: int) -> None:
        self._learning_state[0] = int(value)

    @property
    def success_ema(self) -> float:
        return float(self._learning_state[1].item())

    @success_ema.setter
    def success_ema(self, value: float) -> None:
        self._learning_state[1] = float(value)

    @property
    def failure_ema(self) -> float:
        return float(self._learning_state[2].item())

    @failure_ema.setter
    def failure_ema(self, value: float) -> None:
        self._learning_state[2] = float(value)

    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
                              missing_keys, unexpected_keys, error_msgs):
        key = prefix + "_learning_state"
        if key not in state_dict and local_metadata.get("version", 1) < 2:
            state_dict = dict(state_dict)
            state_dict[key] = torch.zeros_like(self._learning_state)
        value = state_dict.get(key)
        if value is not None and (
            not torch.is_tensor(value) or value.shape != (3,)
            or not torch.isfinite(value).all() or (value < 0).any()
            or value[0] != value[0].floor()
        ):
            error_msgs.append(f"{key} must contain a nonnegative cooldown and finite outcome history")
            return
    ...
    def record_outcome(
        ...
        *,
        ...
        values = [dummy_improvement, dummy_score, patch_size]
        if realized_gain is not None:
            values.append(realized_gain)
        if not all(math.isfinite(float(value)) for value in values):
            raise ValueError("self-improvement outcomes must be finite")

        finalized = realized_gain is not None
        feedback_applied = finalized and adopted
        realized = float(realized_gain) if finalized else 0.0
        if finalized and self.cooldown > 0:
            self.cooldown -= 1
        if feedback_applied:
            # Trial predictions cannot erase a measured regression. A rollback
            # is failed adoption even when the aggregate reward improved.
            if rolled_back:
                realized = -max(abs(realized), float(self.cfg.persistence_floor), 1e-6)
            decay = float(self.cfg.outcome_decay)
            self.success_ema = decay * self.success_ema + (1.0 - decay) * max(0.0, realized)
            self.failure_ema = decay * self.failure_ema + (1.0 - decay) * max(0.0, -realized)
        ...
        self.last_outcome = {
            ...
            "realized_gain": float(realized_gain) if finalized else 0.0,
            "outcome_finalized": float(finalized),
            "feedback_applied": float(feedback_applied),
