...
def _write_missing_evidence_placeholder(
    target: Path,
    *,
    ...
    if suffix == ".json":
        _write_json(
            target,
            {
                "schema_name": "PackagedMissingEvidencePlaceholder",
                "schema_version": "novali_handoff_package_v1",
                "generated_at": _now(),
                "source_relative_path": source_relative_path,
                "target_relative_path": target_relative_path,
                "evidence_state": "source_missing_at_package_time",
                "review_required": True,
                "non_authoritative_note": (
                    "This placeholder records that optional historical demo evidence was "
                    "not present in the source checkout during package assembly."
                ),
            },
        )
        return
    if suffix == ".py":
        _write_text(
            target,
            '"""Optional trusted-source demo evidence was missing at package time.\n\n'
            "This placeholder is intentionally non-authoritative; it preserves the "
            "packaged evidence path for review without claiming executable demo "
            "functionality.\n"
            '"""\n\n'
            "EVIDENCE_STATE = \"source_missing_at_package_time\"\n",
        )
    ...
    _write_text(
        ...
        (
            ...
            f"- Packaged path: `{target_relative_path}`\n"
            "- Evidence state: `source_missing_at_package_time`\n\n"
            "This placeholder is review-only and does not claim the missing evidence was present.\n"
        ),
    )


def _copy_trusted_demo_evidence(
    *,
    source_root: Path,
    package_root: Path,
) -> dict[str, Any]:
    ...
    for source_relative, target_relative in TRUSTED_DEMO_EVIDENCE_FILE_MAP.items():
        ...
        if source_path.exists():
            _copy_file(source_root, package_root, source_relative, target_name=target_relative)
            copied.append(
                {
                    "source_relative_path": source_relative,
                    "target_relative_path": target_relative,
                    "evidence_state": "copied",
                }
            )
            continue
        _write_missing_evidence_placeholder(
            target_path,
            source_relative_path=source_relative,
            target_relative_path=target_relative,
        )
    ...
    manifest = {
        "schema_name": "TrustedDemoEvidenceCarryForward",
        "schema_version": "novali_handoff_package_v1",
        "generated_at": _now(),
        ...
        "review_required": bool(missing_sources),
        "non_authoritative_note": (
            "Missing optional historical trusted-demo evidence is represented by "
            "placeholders so package assembly remains deterministic and reviewable."
        ),
    }
    _write_json(
        package_root
        ...
        / "trusted_demo_evidence_carry_forward_manifest.json",
        manifest,
    )
    return manifest
...
def _package_successor_improvement_evidence(
    *,
    ...
    fallback_workspace_id = "workspace_trusted_source_guided_successor_iteration_live_e"

    def _write_benchmark_support_placeholder(
        *,
        workspace_id: str,
        relative_path: str,
        ...
        _write_missing_evidence_placeholder(
            ...
            source_relative_path=(
                ...
                and source_root in source_path.parents
                else normalized_relative_path
            ),
            target_relative_path=package_relative_path,
        )
        return {
    ...
    if not selected:
        packaged_entries = [
            _write_benchmark_support_placeholder(
        ...
        evidence_manifest = {
            ...
            "package_improvement_state": "source_missing_at_package_time",
            "package_improvement_summary": (
                "No recommended successor-improvement evidence bundle was available "
                "when this handoff package was assembled."
            ),
            "handoff_refresh_recommended": False,
            "packaged_entries": packaged_entries,
            "packaged_entry_count": len(packaged_entries),
            "missing_source_count": len(packaged_entries),
            "governance_authority_note": (
                "Placeholder evidence is review-only and does not claim the missing "
                "successor-improvement workspace was present."
            ),
        }
...
def assemble_standalone_handoff_package(
    *,
    ...
    trusted_demo_evidence_carry_forward = _copy_trusted_demo_evidence(
    ...
    return {
        ...
        "trusted_demo_evidence_carry_forward": trusted_demo_evidence_carry_forward,
    }
