...
def build_default_operator_gui_profile(package_root: str | Path | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "recent_runtime_policy_directive_file": "",
...
def sanitize_operator_gui_profile(
    ...
    *,
    ...
    cleaned = {
        ...
        "recent_runtime_policy_directive_file": str(
            source.get(
                "recent_runtime_policy_directive_file",
                defaults["recent_runtime_policy_directive_file"],
            )
        ),
...
def inspect_directive_wrapper(
    ...
    *,
    ...
    directive_id = str(directive_spec.get("directive_id", "")).strip()
    selected_workspace_id = _slug(path.stem, fallback="workspace_default")
    return {
        "path": path_text,
        "exists": True,
        "is_valid": True,
        ...
        "summary": (
            ...
            f"directive_id={directive_id or '<missing>'}, "
        ...
        rows.append(
            {
                ...
                "endpoint_base": str(row.get("endpoint_base", "")),
    ...
    *,
    ...
    if expected_execution_profile:
        ...
        if profile_matches_selected_action:
            ...
            blocking_reasons.append(profile_alignment_detail)
...
def build_launch_result_summary(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if status == "terminated_by_watchdog":
        ...
        headline = f"Process started — {action_label} is now running."
        if str(workspace_policy.get("workspace_root", "")).strip():
            details.append(
                "Active workspace: "
                f"{workspace_policy.get('workspace_id', '')} -> {workspace_policy.get('workspace_root', '')}"
            )
        if str(result.get("pid", "")):
            details.append(f"Process identifier: {result.get('pid')}")
        if str(result.get("backend_kind", "")).strip():
            details.append(f"Backend kind: {result.get('backend_kind', '')}")
    elif exit_code == 0:
    ...
    if session:
        ...
        directive_outcome_packet = dict(work_cycle.get("directive_outcome_packet", {}) or {})
        directive_outcome_artifact_path = str(
            directive_outcome_packet.get("directive_outcome_artifact_path", "")
            or work_cycle.get("latest_directive_outcome_artifact", "")
        ).strip()
        if directive_outcome_artifact_path:
            details.append(f"Directive outcome artifact: {directive_outcome_artifact_path}")
        directive_track_id = str(
            directive_outcome_packet.get("selected_directive_track_id", "")
            or directive_outcome_packet.get("selected_track_id", "")
            or work_cycle.get("selected_directive_track_id", "")
        ).strip()
        if directive_track_id:
            details.append(f"Directive track advanced: {directive_track_id}")
...
def render_trusted_sources_summary(snapshot: dict[str, Any]) -> str:
    ...
    for row in summary["rows"]:
        ...
        if row.get("endpoint_base"):
            lines.append(f"  endpoint: {row['endpoint_base']}")
