...
async function approveCurrentReviewAction(request, baseUrl) {
  ...
  const approval = await postJson(
    ...
    baseUrl,
    {
      ...
      operator_note: "rc74 packaged proof approval",
    },
    { timeout: 180000 },
  );
...
const runSummaryPath = path.join(artifactRoot, "rc74_operator_journey.json");
const runMarkdownPath = path.join(artifactRoot, "rc74_operator_journey.md");
const managerDigestSummaryJsonPath = path.join(artifactRoot, "manager_digest_summary.json");
const managerDigestSummaryMdPath = path.join(artifactRoot, "manager_digest_summary.md");
const operatorQueueSummaryJsonPath = path.join(artifactRoot, "operator_queue_summary.json");
const operatorQueueSummaryMdPath = path.join(artifactRoot, "operator_queue_summary.md");
const groupedQueueCountsPath = path.join(artifactRoot, "grouped_queue_counts.json");
const nextActionAgendaSummaryPath = path.join(artifactRoot, "next_action_agenda_summary.json");
...
try {
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc74_01_shell_digest_and_queue.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc74_02_digest_anchor_recorded.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc74_03_queue_to_blocker.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc74_04_selected_session_after_continue.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc74_05_shell_after_return.png"),
    fullPage: true,
  });
  ...
  writeJson(managerDigestSummaryJsonPath, {
  ...
  writeText(
    managerDigestSummaryMdPath,
    [
      "# manager digest summary",
      "",
      `- digest headline before action: \`${summary.manager_digest_before?.headline || "<none>"}\``,
      `- digest anchor before action: \`${summary.manager_digest_before?.anchor || "<none>"}\``,
  ...
  writeJson(operatorQueueSummaryJsonPath, {
  ...
  writeText(
    operatorQueueSummaryMdPath,
    [
      "# operator queue summary",
      "",
      `- queue action label: \`${summary.queue_action.label || "<none>"}\``,
      `- queue action worked: \`${summary.queue_action.worked}\``,
      `- queue sections before action: \`${summary.operator_queue_before.sections.length}\``,
      `- queue sections after return: \`${summary.operator_queue_after_return.sections.length}\``,
      "",
    ].join("\n"),
  );
  writeJson(groupedQueueCountsPath, {
    before: summary.queue_counts_before,
    after_return: summary.queue_counts_after_return,
  });
  writeJson(nextActionAgendaSummaryPath, {
    before: {
  ...
  try {
    await page.screenshot({
      path: path.join(screensDir, "rc74_zz_failure.png"),
      fullPage: true,
    });
  } catch {
