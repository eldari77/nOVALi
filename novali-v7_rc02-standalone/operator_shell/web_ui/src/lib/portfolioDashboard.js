const FILTER_GROUPS = [
  ...
  {
    key: "conveyor_queued",
    label: "Conveyor queued",
    description: "Child directives waiting for scheduler capacity.",
    matches: (card) => card?.queue_bucket === "conveyor_queued",
    overlay: false,
  },
  {
    key: "conveyor_running",
    label: "Conveyor running",
    description: "Child containers currently pursuing bounded directives.",
    matches: (card) =>
      card?.queue_bucket === "conveyor_spawning" ||
      card?.queue_bucket === "conveyor_running",
    overlay: false,
  },
  {
    key: "conveyor_returned_for_review",
    label: "Conveyor review",
    description: "Child return packets waiting for operator review.",
    matches: (card) =>
      card?.queue_bucket === "conveyor_blocked" ||
      card?.queue_bucket === "conveyor_returned_for_review",
    overlay: false,
  },
  {
    key: "conveyor_completed",
    label: "Conveyor completed",
    description: "Conveyor directives accepted, requeued, rejected, or split after review.",
    matches: (card) =>
      card?.queue_bucket === "conveyor_accepted" ||
      card?.queue_bucket === "conveyor_requeued" ||
      card?.queue_bucket === "conveyor_rejected" ||
      card?.queue_bucket === "conveyor_split",
    overlay: false,
  },
  {
