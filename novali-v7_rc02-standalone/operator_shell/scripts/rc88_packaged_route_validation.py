...
def _markdown_summary(summary: dict[str, Any]) -> str:
    lines = [
        "# rc88 Packaged Route Validation",
        "",
...
def run_packaged_route_validation(
    *,
    ...
    summary = {
        "schema_name": "novali_rc88_packaged_route_validation_summary_v1",
        "generated_at": _now_iso(),
        "result": "success" if ok else "failure",
        "package_root": str(package_root),
        "routes": routes,
    }
    artifact_root = resolve_rc88_artifact_root(ROOT, env=env)
...
def main() -> int:
    ...
    parser.add_argument(
        "--package-root",
        default=str(ROOT / "dist" / "novali-v6_rc88-standalone"),
    )
