...
from operator_shell.external_adapter import resolve_rc85_artifact_root
...
FAKE_SEEDS = (
    ...
    _fake_seed("api", "key"),
    _fake_seed("cookie"),
    _fake_seed("external", "payload", "secret"),
    _fake_seed("rollback", "context"),
...
def _text_from_zip_member(package: zipfile.ZipFile, info: zipfile.ZipInfo) -> str:
    ...
    if not info.filename.lower().endswith(
        (".md", ".txt", ".json", ".yaml", ".yml", ".py", ".env", ".ps1", ".bat", ".mjs", ".js", ".ts", ".svelte")
    ):
        return ""
...
def _markdown_summary(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- Delta bytes vs rc84: {summary.get('delta_bytes', 0)}",
...
def run_package_hygiene(
    *,
    ...
    summary = {
        "schema_name": "novali_rc85_package_hygiene_summary_v1",
    ...
    artifact_root = resolve_rc85_artifact_root(ROOT)
