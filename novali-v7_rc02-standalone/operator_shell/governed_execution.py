...
def run_bounded_governed_execution(
    *,
    ...
    raw_max_total_cycles = governed_execution_policy.get(
        "max_total_cycles",
    ...
    max_total_cycles = (
        ...
        else int(raw_max_total_cycles)
    )
    max_wall_clock_seconds = int(
        governed_execution_policy.get("max_wall_clock_seconds", 900) or 900
    )
    raw_max_restart_attempts = governed_execution_policy.get("max_restart_attempts", None)
    if raw_max_restart_attempts in {None, ""}:
        max_restart_attempts = 0 if int(max_total_cycles or 0) == 0 else 3
    else:
        max_restart_attempts = int(raw_max_restart_attempts)
    ...
    invocation_window_evaluation_id = str(
        governed_execution_policy.get("effective_invocation_window_evaluation_id", "")
    ).strip()
    effective_max_cycles_per_invocation = int(
        governed_execution_policy.get(
            "effective_max_cycles_per_invocation",
            max_cycles_per_invocation,
        )
        or max_cycles_per_invocation
    )
    session_max_cycles_per_invocation = effective_max_cycles_per_invocation
    ...
    payload = {
        ...
        "governed_execution_policy": {
            ...
            "effective_max_cycles_per_invocation": effective_max_cycles_per_invocation,
            "invocation_window_evaluation_id": invocation_window_evaluation_id,
            "invocation_window": dict(governed_execution_policy.get("invocation_window", {}) or {}),
    ...
    if duplicate_launch_reason:
        ...
        payload["long_run_session"] = build_governed_long_run_session_payload(
            ...
            max_cycles_per_invocation=session_max_cycles_per_invocation,
