...
def _safe_metadata(row: Mapping[str, Any]) -> dict[str, Any]:
    safe_keys = {
        ...
        "pack_summary_quality",
        "pack_distillation_state",
        "next_concrete_artifact",
        "deliverable_artifact_ref",
        "deliverable_artifact_kind",
        "deliverable_artifact_signature",
        "deliverable_artifact_materially_new",
        "deliverable_artifact_rejection_reason",
        "deliverable_artifact_relative_path",
        "deliverable_artifact_generated_at",
        "next_concrete_artifact_state",
        "directive_deliverable_maturity_state",
        "directive_deliverable_next_maturity_state",
        "directive_deliverable_maturity_target",
        "directive_deliverable_maturity_satisfied",
        "directive_deliverable_stage_blocker",
        "directive_artifact_density_state",
        "directive_artifact_domain_semantic_state",
        "directive_domain_entity_count",
        "directive_domain_flow_count",
    ...
    safe_list_keys = {
        "reusable_pack_refs",
        "directive_dossier_evidence_pack_refs",
        "directive_deliverable_missing_sections",
        "requested_tags",
        "directive_deliverable_section_delta",
        "capability_pack_refs",
        "capability_pack_candidate_paths",
        "capability_pack_staged_refs",
    }
    payload: dict[str, Any] = {}
    sources: list[Mapping[str, Any]] = [row]
    nested_result = row.get("result")
    if isinstance(nested_result, Mapping):
        sources.append(nested_result)
    for source in sources:
        for key in safe_keys:
            if key not in source or key in payload:
                continue
        ...
        for key in safe_list_keys:
            ...
            if not isinstance(values, list):
                continue
            safe_values = [
                _safe_text(item, max_chars=160)
                for item in values
                if isinstance(item, str) and item.strip() and not _looks_absolute_path(item)
            ][:12]
            if safe_values:
...
def _operation_has_directive_evidence(payload: Mapping[str, Any]) -> bool:
    return bool(
        payload.get("directive_dossier_artifact_relative_path")
        or payload.get("latest_directive_dossier_ref")
        or payload.get("directive_dossier_signature")
        or payload.get("selected_directive_track_id")
        or payload.get("directive_track_artifact_relative_path")
        or payload.get("deliverable_artifact_ref")
        or payload.get("deliverable_artifact_signature")
        or payload.get("deliverable_artifact_relative_path")
    )


def _directive_evidence_overlay(
    latest_operation: Mapping[str, Any],
    directive_track: Mapping[str, Any],
    directive_dossier: Mapping[str, Any],
) -> dict[str, Any]:
    evidence: dict[str, Any] = {}
    if directive_dossier:
        evidence.update(dict(directive_dossier))
    elif directive_track:
        evidence.update(dict(directive_track))
    if not evidence:
        return {}
    evidence["schema_name"] = "NovaliLatestDirectiveEvidence"
    evidence["schema_version"] = MATERIALIZED_VIEW_SCHEMA_VERSION
    if directive_dossier:
        evidence["directive_progress_overlay_source"] = "workspace_dossier_progress"
    else:
        evidence["directive_progress_overlay_source"] = "workspace_track_progress"
    latest = dict(latest_operation)
    if _operation_has_directive_evidence(latest):
        return evidence
    if str(latest.get("action", "") or "") != "governed_start_next_invocation":
        return evidence
    if not bool(evidence.get("directive_dossier_materially_new", False)):
        return evidence
    latest["directive_progress_overlay_applied"] = True
    latest["directive_progress_overlay_source"] = evidence[
        "directive_progress_overlay_source"
    ]
    latest["latest_directive_evidence"] = evidence
    return {
        "latest_operation_result": latest,
        "latest_directive_evidence": evidence,
    }


def _catalog_directive_dossier_evidence(state_root: str | Path) -> dict[str, Any]:
    search = search_data_at_rest_catalog(
        state_root,
        query="",
    ...
    if not isinstance(entry, Mapping):
        return {}
    evidence = _safe_metadata(entry)
    for source_key, target_key in (
        ("deliverable_kind", "selected_deliverable_kind"),
        ("track_id", "selected_directive_track_id"),
        ("relative_path", "directive_dossier_artifact_relative_path"),
    ):
        value = entry.get(source_key)
        if value and target_key not in evidence and isinstance(value, str):
            evidence[target_key] = _safe_text(value, max_chars=180)
    if "latest_directive_dossier_ref" not in evidence and evidence.get(
        "directive_dossier_artifact_relative_path"
    ):
        evidence["latest_directive_dossier_ref"] = evidence[
            "directive_dossier_artifact_relative_path"
        ]
    if not evidence:
        return {}
    evidence["schema_name"] = "NovaliLatestDirectiveEvidence"
    evidence["schema_version"] = MATERIALIZED_VIEW_SCHEMA_VERSION
    evidence["directive_progress_overlay_source"] = "data_at_rest_catalog"
    return evidence


def _view_directive_evidence(state_root: str | Path) -> dict[str, Any]:
    view = _safe_read_json(materialized_views_root(state_root) / "latest_directive_evidence.json")
    payload = view.get("payload", {}) if isinstance(view.get("payload", {}), Mapping) else {}
    evidence = _safe_metadata(payload)
    if evidence:
        evidence["schema_name"] = "NovaliLatestDirectiveEvidence"
        evidence["schema_version"] = MATERIALIZED_VIEW_SCHEMA_VERSION
        evidence.setdefault("directive_progress_overlay_source", "materialized_view")
    return evidence


def _operation_directive_evidence(operator_root: str | Path) -> dict[str, Any]:
    latest = _safe_metadata(latest_ledger_entry(operator_root, "operation_results"))
    if not _operation_has_directive_evidence(latest):
        return {}
    latest["schema_name"] = "NovaliLatestDirectiveEvidence"
    latest["schema_version"] = MATERIALIZED_VIEW_SCHEMA_VERSION
    latest["directive_progress_overlay_source"] = "operation_result"
    return latest


def _merge_directive_evidence_supplement(
    ...
    if not supplement:
        return result
    primary_signature = str(primary.get("directive_dossier_signature", "") or "").strip()
    supplement_signature = str(
        supplement.get("directive_dossier_signature", "") or ""
    ).strip()
    primary_ref = str(
        primary.get("latest_directive_dossier_ref")
        or primary.get("directive_dossier_artifact_relative_path")
        or ""
    ).strip()
    supplement_ref = str(
        supplement.get("latest_directive_dossier_ref")
        or supplement.get("directive_dossier_artifact_relative_path")
        or ""
    ).strip()
    if primary_signature and supplement_signature and primary_signature != supplement_signature:
        return result
    if primary_ref and supplement_ref and primary_ref != supplement_ref:
        return result
    for key, value in _safe_metadata(supplement).items():
        if key in {"schema_name", "schema_version", "directive_progress_overlay_source"}:
            continue
        existing = result.get(key)
        if isinstance(value, list):
            if value and not (isinstance(existing, list) and existing):
                result[key] = value
        elif value not in ("", None, False) and existing in ("", None, [], {}):
            result[key] = value
    return result


def _directive_evidence_timestamp(evidence: Mapping[str, Any]) -> float:
    for key in (
        "directive_dossier_progress_generated_at",
        "generated_at",
        "created_at",
...
def resolve_latest_directive_evidence(
    *,
    operator_root: str | Path,
    state_root: str | Path,
    workspace_root: str | Path | None = None,
    ...
    catalog_evidence = (
        _catalog_directive_dossier_evidence(state_root) if include_catalog else {}
    )
    candidates: list[dict[str, Any]] = []
    for evidence in (
        _operation_directive_evidence(operator_root),
        _view_directive_evidence(state_root),
        _latest_directive_dossier_progress(workspace_root),
        catalog_evidence,
    ):
        if not evidence:
            continue
        if include_catalog and evidence is not catalog_evidence:
            evidence = _merge_directive_evidence_supplement(evidence, catalog_evidence)
        safe = _safe_metadata(evidence)
        for key in (
            "schema_name",
            "schema_version",
            "directive_progress_overlay_source",
            ...
            if key in evidence:
                safe[key] = _safe_text(evidence.get(key, ""), max_chars=120)
        if not safe.get("directive_progress_overlay_source"):
            safe["directive_progress_overlay_source"] = "workspace_dossier_progress"
        if safe.get("directive_dossier_artifact_relative_path") and not safe.get(
            "latest_directive_dossier_ref"
        ):
            safe["latest_directive_dossier_ref"] = safe[
                "directive_dossier_artifact_relative_path"
            ]
        if not safe.get("directive_progress_credit_kind"):
            coverage = str(safe.get("directive_source_coverage_state", "") or "")
            pack_quality = str(safe.get("pack_summary_quality", "") or "")
            try:
                source_pack_count = int(safe.get("source_pack_count", 0) or 0)
            except (TypeError, ValueError):
                source_pack_count = 0
            if coverage == "trusted_source_evidence_staged":
                safe["directive_progress_credit_kind"] = "trusted_source_backed_dossier"
            elif (
                coverage == "librarian_evidence_seeded"
                or bool(safe.get("reusable_pack_refs"))
                or source_pack_count > 0
                or pack_quality in {"rich", "derived"}
            ):
                safe["directive_progress_credit_kind"] = "pack_backed_dossier"
            elif bool(safe.get("directive_dossier_materially_new", False)):
                safe["directive_progress_credit_kind"] = "directive_dossier"
        if safe:
            candidates.append(safe)
    if not candidates:
        return {}
    selected = candidates[0]
    selected_ts = _directive_evidence_timestamp(selected)
    for candidate in candidates[1:]:
        candidate_ts = _directive_evidence_timestamp(candidate)
        if candidate_ts > selected_ts and bool(
            candidate.get("directive_dossier_materially_new", False)
        ):
            selected = candidate
...
def refresh_materialized_views(
    ...
    overlay = _directive_evidence_overlay(
        latest_operation,
        directive_track,
        directive_dossier,
    )
    if overlay.get("latest_operation_result"):
        latest_operation = dict(overlay["latest_operation_result"])
    latest_directive_evidence = dict(
        overlay.get("latest_directive_evidence")
        or directive_dossier
        or directive_track
        or {}
    )
    resolved_evidence = resolve_latest_directive_evidence(
        operator_root=operator_root,
        state_root=state_root,
        workspace_root=workspace_root,
    ...
    if resolved_evidence:
        latest_directive_evidence = resolved_evidence
        if (
            str(latest_operation.get("action", "") or "") == "governed_start_next_invocation"
            and not _operation_has_directive_evidence(latest_operation)
            and bool(resolved_evidence.get("directive_dossier_materially_new", False))
        ):
            latest_operation = {
                **latest_operation,
                "directive_progress_overlay_applied": True,
                "directive_progress_overlay_source": str(
                    resolved_evidence.get(
                        "directive_progress_overlay_source",
                        "workspace_dossier_progress",
                    )
                ),
                "latest_directive_evidence": resolved_evidence,
            }
    ...
    views = {
        ...
        "latest_directive_evidence": latest_directive_evidence,
...
def _catalog_entry_from_directive_dossier(
    ...
    *,
    ...
    return {
        ...
        "directive_deliverable_content_quality": _safe_text(
            payload.get("directive_deliverable_content_quality", "")
        ),
        "directive_deliverable_section_delta": [
            _safe_text(item, max_chars=80)
            for item in list(payload.get("directive_deliverable_section_delta", []) or [])
            if isinstance(item, str) and item.strip()
        ][:12],
        "pack_summary_quality": _safe_text(payload.get("pack_summary_quality", "")),
        "pack_distillation_state": _safe_text(payload.get("pack_distillation_state", "")),
        "next_concrete_artifact": _safe_text(
            payload.get("next_concrete_artifact", ""),
            max_chars=180,
        ),
        "deliverable_artifact_ref": _safe_text(payload.get("deliverable_artifact_ref", "")),
        "deliverable_artifact_kind": _safe_text(
            payload.get("deliverable_artifact_kind", "")
        ),
        "deliverable_artifact_signature": _safe_text(
            payload.get("deliverable_artifact_signature", "")
        ),
        "deliverable_artifact_materially_new": bool(
            payload.get("deliverable_artifact_materially_new", False)
        ),
        "deliverable_artifact_rejection_reason": _safe_text(
            payload.get("deliverable_artifact_rejection_reason", "")
        ),
        "deliverable_artifact_relative_path": _safe_text(
            payload.get("deliverable_artifact_relative_path", "")
        ),
        "next_concrete_artifact_state": _safe_text(
            payload.get("next_concrete_artifact_state", "")
        ),
        "directive_deliverable_maturity_state": _safe_text(
            payload.get("directive_deliverable_maturity_state", "")
        ),
        "directive_deliverable_next_maturity_state": _safe_text(
            payload.get("directive_deliverable_next_maturity_state", "")
        ),
        "directive_deliverable_maturity_target": _safe_text(
            payload.get("directive_deliverable_maturity_target", "")
        ),
        "directive_deliverable_maturity_satisfied": bool(
            payload.get("directive_deliverable_maturity_satisfied", False)
        ),
        "directive_deliverable_stage_blocker": _safe_text(
            payload.get("directive_deliverable_stage_blocker", "")
        ),
        "directive_artifact_density_state": _safe_text(
            payload.get("directive_artifact_density_state", "")
        ),
        "directive_artifact_domain_semantic_state": _safe_text(
            payload.get("directive_artifact_domain_semantic_state", "")
        ),
        "directive_domain_entity_count": int(
            payload.get("directive_domain_entity_count", 0) or 0
        )
        if str(payload.get("directive_domain_entity_count", 0) or "0").isdigit()
        else 0,
        "directive_domain_flow_count": int(
            payload.get("directive_domain_flow_count", 0) or 0
        )
        if str(payload.get("directive_domain_flow_count", 0) or "0").isdigit()
        else 0,
        "capability_quality_gate_state": _safe_text(
            payload.get("capability_quality_gate_state", "")
        ),
        "capability_quality_gate_blockers": [
            _safe_text(item, max_chars=80)
            for item in list(payload.get("capability_quality_gate_blockers", []) or [])
            if isinstance(item, str) and item.strip()
        ][:12],
        "trusted_source_gap_scope": _safe_text(
            payload.get("trusted_source_gap_scope", "")
        ),
        "trusted_source_gap_selection_state": _safe_text(
            payload.get("trusted_source_gap_selection_state", "")
        ),
        "directive_progress_credit_kind": _safe_text(
            payload.get("directive_progress_credit_kind", "")
        ),
        "directive_progress_blocker": _safe_text(
            payload.get("directive_progress_blocker", "")
        ),
        "reusable_pack_refs": [
            _safe_text(item, max_chars=120)
            for item in list(payload.get("reusable_pack_refs", []) or [])
            if isinstance(item, str) and item.strip()
        ][:12],
        "directive_dossier_evidence_pack_refs": [
            _safe_text(item, max_chars=120)
            for item in list(payload.get("directive_dossier_evidence_pack_refs", []) or [])
            if isinstance(item, str) and item.strip()
        ][:12],
        "source_pack_count": int(payload.get("source_pack_count", 0) or 0)
        if str(payload.get("source_pack_count", 0) or "0").isdigit()
        else 0,
        "directive_dossier_materially_new": bool(
            payload.get("directive_dossier_materially_new", False)
        ),
        "directive_dossier_signature": _safe_text(
            payload.get("directive_dossier_signature", "")
        ),
        "directive_dossier_progress_generated_at": _safe_text(
            payload.get("directive_dossier_progress_generated_at", "")
            or payload.get("generated_at", "")
        ),
        "directive_dossier_artifact_relative_path": relative_path
        if relative_path and not _looks_absolute_path(relative_path)
        else "",
        "latest_directive_dossier_ref": relative_path
        if relative_path and not _looks_absolute_path(relative_path)
        else "",
