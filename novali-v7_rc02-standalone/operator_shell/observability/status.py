...
_STATE_UPDATED_AT_MONOTONIC = time.monotonic()
_STATE: dict[str, Any] = {
    "enabled": False,
    "mode": "disabled",
    "status": "disabled",
    ...
    "metrics_pipeline_state": "normal",
    "metric_consecutive_failure_count": 0,
    "metric_circuit_open_until": "",
    "trace_pipeline_state": "normal",
    "trace_consecutive_failure_count": 0,
    "trace_circuit_open_until": "",
...
def configure_observability_status(
    ...
    *,
    ...
    global _STATE_UPDATED_AT_MONOTONIC
    with _LOCK:
        _STATE_UPDATED_AT_MONOTONIC = time.monotonic()
...
def mark_metric_export_success() -> None:
    with _LOCK:
        _STATE["metrics_pipeline_state"] = "normal"
        _STATE["metric_consecutive_failure_count"] = 0
        _STATE["metric_circuit_open_until"] = ""


def mark_trace_export_success() -> None:
    with _LOCK:
        _STATE["trace_pipeline_state"] = "normal"
        _STATE["trace_consecutive_failure_count"] = 0
        _STATE["trace_circuit_open_until"] = ""


def mark_metric_export_failure_state(
    *,
    error_type: str,
    ...
    with _LOCK:
        _STATE["metrics_pipeline_state"] = (
            "metric_circuit_open" if circuit_open_until else "degraded"
        )
        _STATE["metric_consecutive_failure_count"] = int(consecutive_failure_count or 0)
        _STATE["metric_circuit_open_until"] = str(circuit_open_until or "")


def mark_trace_export_failure_state(
    *,
    error_type: str,
    consecutive_failure_count: int,
    circuit_open_until: str = "",
    ...
    with _LOCK:
        _STATE["trace_pipeline_state"] = (
            "trace_circuit_open" if circuit_open_until else "degraded"
        )
        _STATE["trace_consecutive_failure_count"] = int(consecutive_failure_count or 0)
        _STATE["trace_circuit_open_until"] = str(circuit_open_until or "")
...
def get_observability_status() -> dict[str, Any]:
    with _LOCK:
        ...
        cache_age_seconds = round(max(0.0, time.monotonic() - _STATE_UPDATED_AT_MONOTONIC), 3)
        return {
            ...
            "last_refresh_result": str(_STATE.get("last_export_result", "not_attempted")),
            ...
            "metrics_pipeline_state": str(_STATE.get("metrics_pipeline_state", "normal")),
            "metric_consecutive_failure_count": int(
                _STATE.get("metric_consecutive_failure_count", 0) or 0
            ),
            "metric_circuit_open_until": str(_STATE.get("metric_circuit_open_until", "")),
            "trace_pipeline_state": str(_STATE.get("trace_pipeline_state", "normal")),
            "trace_consecutive_failure_count": int(
                _STATE.get("trace_consecutive_failure_count", 0) or 0
            ),
            "trace_circuit_open_until": str(_STATE.get("trace_circuit_open_until", "")),
