...
_OBSERVABILITY_COMPLETENESS_CANARY_DAY = ""
...
def reset_observability_completeness_canaries_for_tests() -> None:
    global _OBSERVABILITY_COMPLETENESS_CANARY_DAY
    _OBSERVABILITY_COMPLETENESS_CANARY_DAY = ""


def emit_observability_completeness_canaries(
    *,
    today: str | None = None,
) -> dict[str, Any]:
    global _OBSERVABILITY_COMPLETENESS_CANARY_DAY
    if not _env_bool("NOVALI_OBSERVABILITY_CANARY_ENABLED", True):
        return {"emitted": False, "reason": "disabled"}
    day = str(today or datetime.now(timezone.utc).date().isoformat())
    if _OBSERVABILITY_COMPLETENESS_CANARY_DAY == day:
        return {"emitted": False, "reason": "already_emitted", "canary_day": day}
    attrs = {
        "novali.canary": True,
        "novali.result": "canary",
        "novali.action": "observability_completeness_canary",
        "novali.operation_ref": "canary",
        "novali.canary_day": day,
    }
    for span_name in (
        "novali.autonomy.operation.proposed",
        "novali.autonomy.operation.executed",
        "novali.autonomy.operation.skipped",
        "novali.trusted_source.triage",
    ):
        with trace_span(span_name, attrs):
            record_event(span_name, attrs)
    _OBSERVABILITY_COMPLETENESS_CANARY_DAY = day
    return {"emitted": True, "canary_day": day}
