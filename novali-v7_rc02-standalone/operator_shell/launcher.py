...
class OperatorLaunchTimeoutError(RuntimeError):
    def __init__(
        self,
        message: str,
        *,
...
_BACKGROUND_PROCESS_TRACKED_AT: dict[int, float] = {}
...
def _track_background_process(process: subprocess.Popen[str]) -> None:
    pid = int(process.pid)
    with _BACKGROUND_PROCESS_LOCK:
        _BACKGROUND_PROCESSES[pid] = process
        _BACKGROUND_PROCESS_TRACKED_AT[pid] = time.time()

    def _reap_background_process() -> None:
        try:
            process.wait()
        except OSError:
            return
            ...
            with _BACKGROUND_PROCESS_LOCK:
                ...
                _BACKGROUND_PROCESS_TRACKED_AT.pop(pid, None)

    threading.Thread(
        target=_reap_background_process,
        name=f"novali-background-launch-reaper-{pid}",
        daemon=True,
    ).start()


def background_process_snapshot() -> list[dict[str, Any]]:
    with _BACKGROUND_PROCESS_LOCK:
        items = list(_BACKGROUND_PROCESSES.items())
        tracked_at = dict(_BACKGROUND_PROCESS_TRACKED_AT)
        ...
        snapshot.append(
            {
                ...
                "tracked_at_epoch_seconds": float(tracked_at.get(pid, 0.0) or 0.0),
            }
        )
    return snapshot
...
def prepare_effective_operator_session(
    *,
    ...
    session, errors = freeze_effective_operator_session(
        ...
        runtime_constraints_override=runtime_constraints_override,
    )
    if errors:
        record_operator_launch_event(
            {
...
def _record_launch_phase_event(
    *,
    root: Path,
    launch_kind: str,
    phase: str,
    ...
    if backend_kind:
        payload["backend_kind"] = str(backend_kind)
    if timeout_seconds is not None:
        payload["timeout_seconds"] = float(timeout_seconds)
    if details:
        payload["details"] = list(details)
    record_operator_launch_event(payload, root=root)
...
def _run_process_with_watchdog(
    *,
    ...
    for stream, sink in ((process.stdout, stdout_chunks), (process.stderr, stderr_chunks)):
        if stream is None:
            continue
        reader = threading.Thread(target=_drain_stream, args=(stream, sink), daemon=True)
        reader.start()
        stream_threads.append(reader)
...
def launch_python_entrypoint(
    *,
    ...
    pre_spawn_timeout_seconds: float | None = None,
    runtime_constraints_override: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    package_root_path = Path(package_root) if package_root is not None else Path(__file__).resolve().parents[1]
    ...
    pre_spawn_deadline = None
    pre_spawn_timeout = 0.0
    if not wait and pre_spawn_timeout_seconds is not None:
        try:
            pre_spawn_timeout = float(pre_spawn_timeout_seconds)
        except (TypeError, ValueError):
            pre_spawn_timeout = 0.0
        if pre_spawn_timeout > 0:
            pre_spawn_deadline = time.monotonic() + pre_spawn_timeout

    def _check_pre_spawn_deadline(phase: str, *, session_id: str = "", backend_kind: str = "") -> None:
        if pre_spawn_deadline is None or time.monotonic() <= pre_spawn_deadline:
            return
        _record_launch_phase_event(
            root=operator_root_path,
            launch_kind=launch_kind,
            phase=phase,
            status="timeout_before_spawn",
            directive_file=directive_file,
            state_root=state_root,
            entry_script=entry_script_path,
            session_id=session_id,
            backend_kind=backend_kind,
            timeout_seconds=pre_spawn_timeout,
        )
        raise OperatorLaunchTimeoutError(
            f"operator launch exceeded pre-spawn timeout during {phase}",
            phase=phase,
            timeout_seconds=pre_spawn_timeout,
        )
    ...
    _check_pre_spawn_deadline("session_prepare", session_id=session_id)
    _record_launch_phase_event(
        root=operator_root_path,
        launch_kind=launch_kind,
        phase="session_prepare",
    ...
    _check_pre_spawn_deadline("launch_plan_build", session_id=session_id, backend_kind=backend_kind)
    _record_launch_phase_event(
        root=operator_root_path,
        launch_kind=launch_kind,
        phase="launch_plan_build",
    ...
    _check_pre_spawn_deadline("launch_plan_write", session_id=session_id, backend_kind=backend_kind)
    _record_launch_phase_event(
        root=operator_root_path,
        launch_kind=launch_kind,
        phase="launch_plan_write",
    ...
    _check_pre_spawn_deadline("process_spawn", session_id=session_id, backend_kind=backend_kind)
    ...
    stdin_target = subprocess.DEVNULL if not wait else None
    stdout_target = subprocess.DEVNULL if not wait else subprocess.PIPE
    stderr_target = subprocess.DEVNULL if not wait else subprocess.PIPE
    _record_launch_phase_event(
        root=operator_root_path,
        launch_kind=launch_kind,
        phase="process_spawn",
...
def launch_novali_main(
    *,
    ...
    return launch_python_entrypoint(
        ...
        wait=wait,
        pre_spawn_timeout_seconds=pre_spawn_timeout_seconds,
        runtime_constraints_override=runtime_constraints_override,
    )
...
def build_operator_dashboard_snapshot(
    *,
    ...
    governed_execution_summary = governed_execution_session_summary_for_workspace(effective_workspace_root)
    governed_execution_controller = governed_execution_controller_summary_for_workspace(effective_workspace_root)
    ...
    return {
        ...
        "launch_context": {
            ...
            "governed_execution": {
                "status": str(governed_execution_summary.get("status", "")),
                "reason": str(governed_execution_summary.get("reason", "")),
                "workspace_id": str(governed_execution_summary.get("workspace_id", "")),
                "workspace_root": str(governed_execution_summary.get("workspace_root", "")),
                "work_cycle": dict(governed_execution_summary.get("work_cycle", {})),
                "controller": governed_execution_controller,
                "session_artifact_path": effective_workspace_root and str(
                    Path(effective_workspace_root) / "artifacts" / "governed_execution_session_latest.json"
                ) or "",
            },
        },
