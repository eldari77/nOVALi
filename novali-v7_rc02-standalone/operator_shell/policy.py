...
BLANK_JSON_ARTIFACT_REPAIR_SCHEMA_NAME = "BlankJsonArtifactRepair"
BLANK_JSON_ARTIFACT_REPAIR_SCHEMA_VERSION = "blank_json_artifact_repair_v1"
...
MAX_GOVERNED_EXECUTION_CYCLES_PER_INVOCATION = 24
GOVERNED_INVOCATION_WINDOW_MODE_FIXED = "fixed"
GOVERNED_INVOCATION_WINDOW_MODE_ADAPTIVE = "adaptive"
SUPPORTED_GOVERNED_INVOCATION_WINDOW_MODES = {
    GOVERNED_INVOCATION_WINDOW_MODE_FIXED,
    GOVERNED_INVOCATION_WINDOW_MODE_ADAPTIVE,
}
DEFAULT_GOVERNED_INVOCATION_WINDOW_POLICY = {
    "mode": GOVERNED_INVOCATION_WINDOW_MODE_FIXED,
    "min_cycles_per_invocation": 2,
    "default_cycles_per_invocation": 8,
    "max_cycles_per_invocation": 24,
    "growth_step": 2,
    "shrink_step": 4,
    "success_window": 3,
    "failure_window": 2,
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not trimmed:
        repaired = _repair_blank_latest_json_artifact(path)
        if repaired:
            return {}
    ...
    if (
        ...
        and payload.get("schema_name") == BLANK_JSON_ARTIFACT_REPAIR_SCHEMA_NAME
        and payload.get("artifact_state") == "repaired_blank_artifact"
    ):
        return {}
...
def _blank_json_repair_audit_path(path: Path) -> Path:
    return path.parent / "blank_json_artifact_repairs.jsonl"


def _repair_blank_latest_json_artifact(path: Path) -> bool:
    ...
    repair_payload = {
        "schema_name": BLANK_JSON_ARTIFACT_REPAIR_SCHEMA_NAME,
        "schema_version": BLANK_JSON_ARTIFACT_REPAIR_SCHEMA_VERSION,
        "artifact_state": "repaired_blank_artifact",
        "generated_at": _now(),
        "artifact_filename": path.name,
        "repair_mode": "blank_latest_sentinel",
        "redaction_status": "redacted",
        "non_authoritative_note": (
            "This sentinel replaces a blank latest JSON artifact so loaders can treat "
            "it as uninitialized state without repeated noisy warnings."
        ),
    }
    audit_payload = {
        "schema_name": BLANK_JSON_ARTIFACT_REPAIR_SCHEMA_NAME,
        "schema_version": BLANK_JSON_ARTIFACT_REPAIR_SCHEMA_VERSION,
        "artifact_state": "repaired_blank_artifact",
        "generated_at": repair_payload["generated_at"],
        "artifact_filename": path.name,
        "repair_mode": "blank_latest_sentinel",
        "prior_size_bytes": 0,
        "redaction_status": "redacted",
    ...
    try:
        _atomic_write_text(path, _stable_json(repair_payload))
        _append_jsonl(_blank_json_repair_audit_path(path), audit_payload)
    except OSError as exc:
        warning_key = f"{str(path.resolve())}::blank_repair_failed"
        if warning_key not in _SEEN_JSON_LOAD_WARNINGS:
            LOGGER.warning(
                "Treating blank JSON artifact as uninitialized operator state: %s (%s)",
                path,
                exc,
            )
            _SEEN_JSON_LOAD_WARNINGS.add(warning_key)
...
def _write_json(path: Path, payload: dict[str, Any]) -> None:
    _atomic_write_text(path, _stable_json(payload))
...
def build_runtime_constraints_for_profile(
    ...
    *,
    ...
    if controller_mode == GOVERNED_EXECUTION_MODE_MULTI_CYCLE:
        window_policy = (
            dict(invocation_window)
            if invocation_window is not None
            else {
                **DEFAULT_GOVERNED_INVOCATION_WINDOW_POLICY,
                "mode": GOVERNED_INVOCATION_WINDOW_MODE_ADAPTIVE,
            }
        )
        payload["governed_execution"]["invocation_window"] = window_policy
    return payload


def build_default_runtime_constraints(
    package_root: str | Path | None = None,
    *,
    operator_root: str | Path | None = None,
) -> dict[str, Any]:
...
def build_default_trusted_source_bindings(package_root: str | Path | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "bindings": [
            {
                "source_id": "local_repo:novali-v7",
                "source_kind": "local_path",
                "enabled": package_root_path.exists(),
                "credential_strategy": "none",
                "credential_ref": "",
                "path_hint": _normalize_path(package_root_path),
            },
            {
                "source_id": "local_artifacts:novali-v7/data",
                "source_kind": "local_path",
                "enabled": (package_root_path / "data").exists(),
                "credential_strategy": "none",
                "credential_ref": "",
...
def trusted_source_provider_readiness_from_env(
    *,
    ...
    endpoint = str(
        endpoint_base
        or env_values.get("NOVALI_TRUSTED_SOURCE_API_BASE_URL", "")
        or "https://api.openai.com/v1"
    ).strip()
    key_present = bool(str(env_values.get(credential_ref, "") or "").strip())
    endpoint_matches_openai = endpoint.rstrip("/") == "https://api.openai.com/v1"
    if not key_present:
        validation_state = "disabled_missing_credential"
    elif not endpoint_matches_openai:
        validation_state = "configured_custom_endpoint"
    else:
        validation_state = "configured"
    return {
        "schema_name": "TrustedSourceProviderReadiness",
        "schema_version": "trusted_source_provider_readiness_v1",
        "generated_at": _now(),
        ...
        "error_class": "" if key_present else "MissingCredential",
    }
    ...
    *,
    ...
    return {
        ...
        "governed_runtime_defaults": {
            ...
            "max_total_cycles": int(governed_execution.get("max_total_cycles", 1) or 1),
            "invocation_window": dict(governed_execution.get("invocation_window", {}) or {}),
        },
...
def initialize_operator_policy_files(
    *,
    ...
    if _json_artifact_requires_default(constraints_path):
        _write_json(
            constraints_path,
            build_default_runtime_constraints(package_root, operator_root=root),
        )
...
def load_runtime_constraints_or_default(
    *,
    ...
    return payload if payload else build_default_runtime_constraints(
        package_root,
        operator_root=root,
    )
...
def validate_runtime_constraints(
    ...
    *,
    ...
    if max_total_cycles is not None and max_total_cycles < 0:
        errors.append("governed_execution.max_total_cycles must be >= 0")
        max_total_cycles = None
    if governed_execution_mode == GOVERNED_EXECUTION_MODE_SINGLE_CYCLE:
    ...
    raw_invocation_window = governed_execution.get("invocation_window", None)
    normalized_invocation_window: dict[str, Any] | None = None
    if raw_invocation_window is not None and raw_invocation_window != "":
        if not isinstance(raw_invocation_window, Mapping):
            errors.append("governed_execution.invocation_window must be an object when provided")
        elif governed_execution_mode == GOVERNED_EXECUTION_MODE_SINGLE_CYCLE:
            errors.append("governed_execution.invocation_window is only supported in multi_cycle mode")
        else:
            invocation_window = dict(raw_invocation_window)
            invocation_mode = str(
                invocation_window.get(
                    "mode",
                    GOVERNED_INVOCATION_WINDOW_MODE_FIXED,
                )
            ).strip() or GOVERNED_INVOCATION_WINDOW_MODE_FIXED
            if invocation_mode not in SUPPORTED_GOVERNED_INVOCATION_WINDOW_MODES:
                errors.append(
                    "governed_execution.invocation_window.mode must be one of "
                    + ", ".join(sorted(SUPPORTED_GOVERNED_INVOCATION_WINDOW_MODES))
                )
                invocation_mode = GOVERNED_INVOCATION_WINDOW_MODE_FIXED

            def _window_positive_int(name: str, fallback: int) -> int:
                raw_value = invocation_window.get(name, fallback)
                try:
                    coerced = int(raw_value)
                except (TypeError, ValueError):
                    errors.append(f"governed_execution.invocation_window.{name} must be an integer")
                    return int(fallback)
                if coerced <= 0:
                    errors.append(f"governed_execution.invocation_window.{name} must be >= 1")
                    return int(fallback)
                return coerced

            min_window = _window_positive_int(
                "min_cycles_per_invocation",
                int(DEFAULT_GOVERNED_INVOCATION_WINDOW_POLICY["min_cycles_per_invocation"]),
            )
            default_window = _window_positive_int(
                "default_cycles_per_invocation",
                int(DEFAULT_GOVERNED_INVOCATION_WINDOW_POLICY["default_cycles_per_invocation"]),
            )
            max_window = _window_positive_int(
                "max_cycles_per_invocation",
                int(DEFAULT_GOVERNED_INVOCATION_WINDOW_POLICY["max_cycles_per_invocation"]),
            )
            growth_step = _window_positive_int(
                "growth_step",
                int(DEFAULT_GOVERNED_INVOCATION_WINDOW_POLICY["growth_step"]),
            )
            shrink_step = _window_positive_int(
                "shrink_step",
                int(DEFAULT_GOVERNED_INVOCATION_WINDOW_POLICY["shrink_step"]),
            )
            success_window = _window_positive_int(
                "success_window",
                int(DEFAULT_GOVERNED_INVOCATION_WINDOW_POLICY["success_window"]),
            )
            failure_window = _window_positive_int(
                "failure_window",
                int(DEFAULT_GOVERNED_INVOCATION_WINDOW_POLICY["failure_window"]),
            )
            if min_window < 2:
                errors.append("governed_execution.invocation_window.min_cycles_per_invocation must be >= 2")
                min_window = 2
            if max_window > MAX_GOVERNED_EXECUTION_CYCLES_PER_INVOCATION:
                errors.append(
                    "governed_execution.invocation_window.max_cycles_per_invocation must be <= "
                    f"{MAX_GOVERNED_EXECUTION_CYCLES_PER_INVOCATION}"
                )
                max_window = MAX_GOVERNED_EXECUTION_CYCLES_PER_INVOCATION
            if default_window < min_window:
                errors.append("governed_execution.invocation_window.default_cycles_per_invocation must be >= min_cycles_per_invocation")
                default_window = min_window
            if default_window > max_window:
                errors.append("governed_execution.invocation_window.default_cycles_per_invocation must be <= max_cycles_per_invocation")
                default_window = max_window
            if min_window > max_window:
                errors.append("governed_execution.invocation_window.min_cycles_per_invocation must be <= max_cycles_per_invocation")
                min_window = min(max_window, min_window)
            normalized_invocation_window = {
                "mode": invocation_mode,
                "min_cycles_per_invocation": min_window,
                "default_cycles_per_invocation": default_window,
                "max_cycles_per_invocation": max_window,
    ...
    if normalized_invocation_window is not None:
        normalized["governed_execution"]["invocation_window"] = normalized_invocation_window
...
def validate_trusted_source_bindings(
    ...
    *,
    ...
    for index, item in enumerate(raw_bindings):
        ...
        capability_tags = [
            str(tag).strip()
            for tag in list(binding.get("capability_tags", []) or [])
            if str(tag).strip()
        ]
        allowed_directive_queue_ids = [
            str(queue_id).strip()
            for queue_id in list(binding.get("allowed_directive_queue_ids", []) or [])
            if str(queue_id).strip()
        ]
        try:
            max_requests_per_directive = max(0, int(binding.get("max_requests_per_directive", 1) or 0))
        except (TypeError, ValueError):
            max_requests_per_directive = 0
            errors.append(f"{source_id or f'binding[{index}]'}: max_requests_per_directive must be an integer")
        requires_operator_review = bool(binding.get("requires_operator_review", False))
        ...
        if enabled:
            ...
            elif source_kind in {"mcp_server", "agent_endpoint"}:
                ready_for_launch = bool(endpoint_base)
                availability_class = "ready" if ready_for_launch else "missing_endpoint"
                availability_reason = (
                    "kernel-mediated trusted source endpoint is configured"
                    if ready_for_launch
                    else "kernel-mediated trusted source endpoint is missing"
                )
                if not endpoint_base:
                    errors.append(f"{source_id}: endpoint_base is required for {source_kind} bindings")
        ...
        normalized_bindings.append(
            {
                ...
                "allowed_directive_queue_ids": allowed_directive_queue_ids,
                "max_requests_per_directive": max_requests_per_directive,
...
def freeze_effective_operator_session(
    *,
    ...
    runtime_constraints_override: Mapping[str, Any] | None = None,
) -> tuple[dict[str, Any], list[str]]:
    ...
    runtime_constraints_payload = (
        dict(runtime_constraints_override)
        if runtime_constraints_override is not None
        else load_runtime_constraints(root)
    )
...
def read_operator_status_snapshot(
    *,
    ...
    runtime_errors, normalized_constraints, enforcement = validate_runtime_constraints(
        runtime_payload
        if runtime_payload
        else build_default_runtime_constraints(package_root_path, operator_root=root),
        package_root=package_root_path,
        operator_root=root,
    )
