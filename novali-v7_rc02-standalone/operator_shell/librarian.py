...
from .support_evidence import extract_consumable_support_evidence
...
SUPPORTED_SCHEMA_KINDS = {
    ...
    "DirectiveBuildGradeSupportDigest": "knowledge_pack",
    "OperatorApprovedSupportSeedPack": "knowledge_pack",
...
def _pack_family(payload: Mapping[str, Any], kind: str) -> str:
    if kind == "knowledge_pack":
        ...
        if str(payload.get("schema_name", "") or "") == "DirectiveBuildGradeSupportDigest":
            if payload.get("requested_pack_family"):
                return _safe_text(payload.get("requested_pack_family"))
            return _safe_text(
                "directive_build_grade_support::"
                + str(
                    payload.get("directive_id")
                    or payload.get("support_request_id")
...
def _summary_payload(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key in allowed:
        if key in payload:
            ...
            if isinstance(value, str):
                ...
                summary[key] = value
    scope = payload.get("requested_topic_scope") or payload.get("knowledge_pack_id")
    if isinstance(scope, str) and scope.strip():
        summary["requested_scope_preview"] = _safe_text(scope, max_chars=180)
    tags = payload.get("domain_topic_tags")
    if not isinstance(tags, list):
        tags = payload.get("requested_tags")
    ...
    has_rich_preview = any(bool(summary.get(key)) for key in rich_keys)
    scope_preview = _safe_text(
        summary.get("requested_scope_preview")
        or summary.get("requested_topic_scope")
        or payload.get("requested_topic_scope")
        or payload.get("knowledge_pack_id")
        or payload.get("knowledge_id")
        or payload.get("schema_name")
        or "",
        max_chars=160,
    ...
    tag_preview = [
        _safe_text(item, max_chars=80)
        for item in list(summary.get("tag_preview", []) or [])
        if isinstance(item, str) and item.strip()
    ][:6]
    if has_rich_preview:
        summary["pack_summary_quality"] = "rich"
        summary["pack_distillation_state"] = "native_summary"
    elif scope_preview or tag_preview:
        tag_text = ", ".join(tag_preview) if tag_preview else "general directive evidence"
        summary["pack_summary_quality"] = "derived"
        summary["pack_distillation_state"] = "derived_from_metadata"
        summary.setdefault("requested_scope_preview", scope_preview)
        summary["claim_preview"] = [
            f"{scope_preview or 'Reusable pack'} provides bounded evidence for {tag_text}."
        ]
        summary["claim_count"] = len(summary["claim_preview"])
        summary["implementation_guidance_preview"] = [
            f"Use this pack as evidence input for {scope_preview or 'the selected deliverable'}; validate concrete claims before execution."
        ]
        summary["caveat_preview"] = [
            "Derived from reusable pack metadata; treat as evidence scaffolding until richer source content is available."
        ]
        summary["action_suggestions_preview"] = [
            f"Seed the dossier decision matrix with {tag_text} and request richer evidence for uncovered sections."
        ]
        summary["action_suggestion_count"] = 1
        summary["candidate_assumptions_preview"] = [
            f"Candidate assumptions should stay scoped to {scope_preview or tag_text} until richer evidence is staged."
        ]
        summary["decision_axes_preview"] = [
            f"Evidence quality for {tag_text}",
            "Validation burden before recommendation",
        ]
        summary["decision_axis_count"] = len(summary["decision_axes_preview"])
        summary["validation_questions_preview"] = [
            f"What source-backed evidence would validate {scope_preview or tag_text} for the selected deliverable?"
        ]
        summary["validation_question_count"] = len(summary["validation_questions_preview"])
    else:
        summary["pack_summary_quality"] = "metadata_only"
        summary["pack_distillation_state"] = "insufficient_metadata"
    support_evidence = extract_consumable_support_evidence(payload)
    support_rows = list(support_evidence.get("rows", []) or [])
    if support_rows:
        summary["child_support_rows"] = support_rows
    summary["consumable_support_row_count"] = int(
        support_evidence.get("consumable_support_row_count", 0) or 0
    )
    summary["support_pack_consumability_state"] = str(
        support_evidence.get("support_pack_consumability_state", "") or ""
    )
    summary["non_consumable_support_reasons"] = list(
        support_evidence.get("non_consumable_support_reasons", []) or []
    )
    summary["support_pack_source_missing"] = bool(
        support_evidence.get("support_pack_source_missing", False)
    )
    return summary


def _ensure_summary_quality(summary: Mapping[str, Any]) -> dict[str, Any]:
    result = dict(summary)
    if result.get("pack_summary_quality") and result.get("pack_distillation_state"):
        return result
    rich_keys = (
        "implementation_guidance_preview",
        "claim_preview",
    ...
    if any(bool(result.get(key)) for key in rich_keys):
        result["pack_summary_quality"] = "rich"
        result["pack_distillation_state"] = "native_summary"
        return result
    scope_preview = _safe_text(
        result.get("requested_scope_preview")
        or result.get("requested_topic_scope")
        or result.get("knowledge_pack_id")
    ...
    tag_preview = [
        _safe_text(item, max_chars=80)
        for item in list(result.get("tag_preview", []) or [])
        if isinstance(item, str) and item.strip()
    ][:6]
    if scope_preview or tag_preview:
        tag_text = ", ".join(tag_preview) if tag_preview else "general directive evidence"
        result["pack_summary_quality"] = "derived"
        result["pack_distillation_state"] = "derived_from_metadata"
        if scope_preview:
            result["requested_scope_preview"] = scope_preview
        result["claim_preview"] = [
            f"{scope_preview or 'Reusable pack'} provides bounded evidence for {tag_text}."
        ]
        result["claim_count"] = len(result["claim_preview"])
        result["implementation_guidance_preview"] = [
            f"Use this pack as evidence input for {scope_preview or 'the selected deliverable'}; validate concrete claims before execution."
        ]
        result["caveat_preview"] = [
            "Derived from reusable pack metadata; treat as evidence scaffolding until richer source content is available."
        ]
        result["action_suggestions_preview"] = [
            f"Seed the dossier decision matrix with {tag_text} and request richer evidence for uncovered sections."
        ]
        result["action_suggestion_count"] = 1
        result["candidate_assumptions_preview"] = [
            f"Candidate assumptions should stay scoped to {scope_preview or tag_text} until richer evidence is staged."
        ]
        result["decision_axes_preview"] = [
            f"Evidence quality for {tag_text}",
            "Validation burden before recommendation",
        ]
        result["decision_axis_count"] = len(result["decision_axes_preview"])
        result["validation_questions_preview"] = [
            f"What source-backed evidence would validate {scope_preview or tag_text} for the selected deliverable?"
        ]
        result["validation_question_count"] = len(result["validation_questions_preview"])
    else:
        result["pack_summary_quality"] = "metadata_only"
        result["pack_distillation_state"] = "insufficient_metadata"
    return result
...
def librarian_search(
    ...
    *,
    ...
    return search_librarian_catalog(
        state_root,
        query=query,
        kind=kind,
        family=family,
...
def librarian_pack_summaries_by_refs(
    ...
    *,
    ...
    for pack_id in wanted:
        ...
        payload_summary = _ensure_summary_quality(payload_summary)
        pack_summary_quality = _safe_text(
            payload_summary.get("pack_summary_quality", "metadata_only"),
            max_chars=40,
        )
        pack_distillation_state = _safe_text(
            payload_summary.get("pack_distillation_state", "insufficient_metadata"),
            max_chars=80,
        )
        summaries.append(
            {
                ...
                "pack_summary_quality": pack_summary_quality,
                "pack_distillation_state": pack_distillation_state,
