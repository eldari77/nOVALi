...
from operator_shell.controller_isolation import resolve_rc86_artifact_root
...
FAKE_SEEDS = (
    ...
    _fake_seed("api", "key"),
    _fake_seed("cookie"),
    _fake_seed("doctrine", "secret"),
    _fake_seed("memory", "secret"),
    _fake_seed("cross", "lane", "secret"),
)
...
def _markdown_summary(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- Delta bytes vs rc85: {summary.get('delta_bytes', 0)}",
...
def run_package_hygiene(
    *,
    ...
    summary = {
        "schema_name": "novali_rc86_package_hygiene_summary_v1",
    ...
    artifact_root = resolve_rc86_artifact_root(ROOT)
...
def main() -> int:
    ...
    parser.add_argument(
        "--zip-path",
        default=str(ROOT / "dist" / "novali-v6_rc86-standalone.zip"),
    )
    parser.add_argument(
        "--baseline-zip-path",
        default=str(ROOT / "dist" / "novali-v6_rc85-standalone.zip"),
    )
