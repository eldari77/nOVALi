...
const runSummaryPath = path.join(artifactRoot, "rc71_operator_journey.json");
const runMarkdownPath = path.join(artifactRoot, "rc71_operator_journey.md");
const portfolioLifecycleSummaryJsonPath = path.join(artifactRoot, "portfolio_lifecycle_summary.json");
const portfolioLifecycleSummaryMdPath = path.join(artifactRoot, "portfolio_lifecycle_summary.md");
const closeoutHygieneJsonPath = path.join(artifactRoot, "session_closeout_hygiene_summary.json");
const closeoutHygieneMdPath = path.join(artifactRoot, "session_closeout_hygiene_summary.md");
const queueFocusAfterActionsPath = path.join(artifactRoot, "queue_focus_after_lifecycle_actions.json");
...
const summary = {
  ...
  portfolio_after_pin: [],
  portfolio_after_archive: [],
  portfolio_after_return: [],
  lifecycle_actions: {
    pin_worked: false,
    archive_worked: false,
  },
  queue_focus: {
...
try {
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc71_01_portfolio_before_actions.png"),
    fullPage: true,
  });
  ...
  await pinSessionCard(page, summary.session_a.session_id);
  summary.lifecycle_actions.pin_worked = true;
  summary.portfolio_after_pin = await readPortfolioCards(page);
  await page.screenshot({
    path: path.join(screensDir, "rc71_02_pinned_session_card.png"),
    fullPage: true,
  });

  await archiveSessionCard(page, summary.session_a.session_id);
  summary.lifecycle_actions.archive_worked = true;
  summary.portfolio_after_archive = await readPortfolioCards(page);
  summary.queue_focus.after_archive_non_archived_count = summary.portfolio_after_archive.filter(
    (card) => !card.archived,
  ).length;
  summary.queue_focus.archived_count_after_archive = summary.portfolio_after_archive.filter(
    (card) => card.archived,
  ).length;
  await page.screenshot({
    path: path.join(screensDir, "rc71_03_archived_closeout_card.png"),
    fullPage: true,
  });

  const archivedSessionA = summary.portfolio_after_archive.find(
    (card) => card.session_id === summary.session_a.session_id,
  );
  const currentSessionBAfterArchive = summary.portfolio_after_archive.find(
    (card) => card.session_id === summary.session_b.session_id,
  );
  ...
  summary.actionable_jump_worked = true;
  await page.screenshot({
    path: path.join(screensDir, "rc71_04_queue_jump_to_blocker.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc71_05_session_after_continue.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc71_06_portfolio_after_return.png"),
    fullPage: true,
  });
  ...
  writeJson(portfolioLifecycleSummaryJsonPath, {
    session_a: summary.session_a,
    session_b: summary.session_b,
    portfolio_before_actions: summary.portfolio_before_actions,
    portfolio_after_pin: summary.portfolio_after_pin,
    portfolio_after_archive: summary.portfolio_after_archive,
    portfolio_after_return: summary.portfolio_after_return,
    lifecycle_actions: summary.lifecycle_actions,
  });
  writeText(
    portfolioLifecycleSummaryMdPath,
    [
      "# portfolio lifecycle summary",
      "",
      `- blocking session: \`${summary.session_b.session_id || "<none>"}\``,
      `- closeout session: \`${summary.session_a.session_id || "<none>"}\``,
      `- pin worked: \`${summary.lifecycle_actions.pin_worked}\``,
      `- archive worked: \`${summary.lifecycle_actions.archive_worked}\``,
      `- same-session preserved after queue action: \`${summary.same_session_preserved}\``,
      "",
    ].join("\n"),
  );
  writeJson(closeoutHygieneJsonPath, {
    closeout_session_id: summary.session_a.session_id,
    closeout_lifecycle_state: summary.session_a.lifecycle_state,
    closeout_primary_cta: summary.session_a.primary_cta,
    archived_entry: archivedSessionA,
    queue_focus: summary.queue_focus,
  });
  writeText(
    closeoutHygieneMdPath,
    [
      "# session closeout hygiene summary",
      "",
      `- closeout session: \`${summary.session_a.session_id || "<none>"}\``,
      `- archived after lifecycle action: \`${archivedSessionA?.archived || false}\``,
  ...
  writeJson(queueFocusAfterActionsPath, {
    ...
    after_pin: summary.portfolio_after_pin,
    after_archive: summary.portfolio_after_archive,
  ...
  try {
    await page.screenshot({
      path: path.join(screensDir, "rc71_zz_failure.png"),
      fullPage: true,
    });
  } catch {
