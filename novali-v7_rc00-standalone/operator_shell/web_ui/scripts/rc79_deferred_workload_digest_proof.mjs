...
const secondaryBenchmarkDirectivePath = path.join(
  path.resolve(path.dirname(directivePath), ".."),
  "manual_acceptance_samples",
  "rc79_secondary_successor_package_readiness_benchmark_directive.json",
);
if (!fs.existsSync(secondaryBenchmarkDirectivePath)) {
  ...
  benchmarkDirective.directive_spec.directive_id =
    "directive_novali_v6_successor_package_readiness_benchmark_v1_rc79_secondary";
  ...
  benchmarkDirective.directive_spec.bucket_spec.bucket_id =
    "bucket_novali_v6_successor_package_readiness_benchmark_v1_rc79_secondary";
  fs.writeFileSync(
...
const runSummaryPath = path.join(artifactRoot, "rc79_operator_journey.json");
const runMarkdownPath = path.join(artifactRoot, "rc79_operator_journey.md");
const managerDigestAckSummaryJsonPath = path.join(artifactRoot, "deferred_workload_digest_summary.json");
const managerDigestAckSummaryMdPath = path.join(artifactRoot, "deferred_workload_digest_summary.md");
const operatorAgendaSummaryJsonPath = path.join(artifactRoot, "return_basis_throughput_summary.json");
const operatorAgendaSummaryMdPath = path.join(artifactRoot, "return_basis_throughput_summary.md");
...
const agendaProgressionAfterActionPath = path.join(
  artifactRoot,
  "agenda_progression_after_deferred_load_change.json",
);
...
try {
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc79_01_shell_digest_queue_and_deferred_workload.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc79_02_digest_anchor_recorded.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc79_03_two_deferred_items.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc79_04_queue_to_blocker.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc79_05_selected_session_after_continue.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc79_06_shell_after_action_return.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc79_07_due_item_current.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc79_08_manual_reopen.png"),
    fullPage: true,
  });
  ...
  writeText(
    ...
    [
      "# deferred workload digest summary",
      "",
  ...
  writeText(
    ...
    [
      "# return basis throughput summary",
      "",
  ...
  try {
    await page.screenshot({
      path: path.join(screensDir, "rc79_zz_failure.png"),
      fullPage: true,
    });
  } catch {
