...
const runSummaryPath = path.join(artifactRoot, "rc76_operator_journey.json");
const runMarkdownPath = path.join(artifactRoot, "rc76_operator_journey.md");
const managerDigestAckSummaryJsonPath = path.join(artifactRoot, "agenda_completion_summary.json");
const managerDigestAckSummaryMdPath = path.join(artifactRoot, "agenda_completion_summary.md");
const operatorAgendaSummaryJsonPath = path.join(artifactRoot, "manager_throughput_summary.json");
const operatorAgendaSummaryMdPath = path.join(artifactRoot, "manager_throughput_summary.md");
...
try {
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc76_01_shell_digest_and_queue.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc76_02_digest_anchor_recorded.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc76_03_queue_to_blocker.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc76_04_selected_session_after_continue.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc76_05_shell_after_return.png"),
    fullPage: true,
  });
  ...
  writeText(
    ...
    [
      ...
      `- overdue panel after return: \`${summary.manager_agenda_after_return?.overdue || "<none>"}\``,
      `- queue action used: \`${summary.queue_action?.label || "<none>"}\``,
      "",
    ].join("\n"),
  );
  ...
  writeText(
    ...
    [
      ...
      `- completed count after return: \`${summary.manager_agenda_after_return?.counts?.completed_since_last_check || "<none>"}\``,
      "",
    ].join("\n"),
  );
  ...
  try {
    await page.screenshot({
      path: path.join(screensDir, "rc76_zz_failure.png"),
      fullPage: true,
    });
  } catch {
