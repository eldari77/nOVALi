...
- current packaged standalone release-candidate version: `novali-v5-standalone-rc42`
...
- post-rollover campaign-wave execution now preserves cycle-selected follow-on attribution and clears resolved weak dimensions from later campaign/cycle governance refreshes
...
- the browser home page now carries a state-driven workflow guide for directive selection, bootstrap-only initialization, transition back to governed execution, and operator next-action guidance
- runtime-policy mismatches between the selected launch action and saved execution profile are now surfaced directly in the workflow and launch sections
...
- a bounded loop-of-loops governance layer now compares completed full campaign-refresh loops across successive rolled reference targets, records loop-over-loop deltas and diminishing-returns signals, and conservatively recommends whether to start another full loop, hold the current bounded target, allow only targeted remediation, or pause for review without broadening authority or mutating the protected live baseline
- the packaged browser-first operator flow now treats the runtime-profile switch as an explicit requirement: use `bootstrap_only_initialization` for `new_bootstrap` + `bootstrap_only`, then restore `bounded_active_workspace_coding` before `resume_existing` + `governed_execution`
- the first operator-productization pass is now in place: the browser UI presents directive-first initialization more clearly, stages the bootstrap-to-governed transition more explicitly, and gives observability a stronger product-facing summary layer without changing governance truth
