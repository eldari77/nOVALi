...
def render_manual_acceptance_markdown(evidence: dict[str, Any]) -> str:
    ...
    governed_execution = dict(launch_context.get("governed_execution", {}))
    lines = [
    ...
    if str(governed_execution.get("status", "")).strip():
        lines.extend(
            [
                ...
                "## Bounded Work",
                "",
                f"- Status: `{governed_execution.get('status', '')}`",
                f"- Reason: `{governed_execution.get('reason', '') or '<none>'}`",
            ]
        )
        work_cycle = dict(governed_execution.get("work_cycle", {}))
