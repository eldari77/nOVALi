...
def _markdown_summary(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# rc83.1 Package Hygiene Summary",
            "",
            ...
            f"- Delta bytes vs rc83: {summary.get('delta_bytes', 0)}",
...
def run_package_hygiene(
    *,
    ...
    summary = {
        "schema_name": "novali_rc83_1_package_hygiene_summary_v1",
...
def main() -> int:
    ...
    parser.add_argument(
        "--zip-path",
        default=str(ROOT / "dist" / "novali-v6_rc83_1-standalone.zip"),
    )
    parser.add_argument(
        "--baseline-zip-path",
