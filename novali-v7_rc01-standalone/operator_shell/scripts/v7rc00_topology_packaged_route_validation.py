...
def run_topology_packaged_route_validation(*, package_root: Path) -> dict[str, Any]:
    ...
    required_strings = {
        ...
        "planning/versioning/v7rc00_workspace_topology_fix.md": (
            "active physical source root: `novali-v7`",
            "the topology-correct canonical package is built from `novali-v7/dist/novali-v7_rc01-standalone.zip`",
        ),
    }
    forbidden_patterns = (
        ...
        "novali-v6/dist/novali-v7_rc01-standalone.zip",
        "novali-v6\\dist\\novali-v7_rc01-standalone.zip",
    )
    ...
    for label, path in package_docs.items():
        ...
        for forbidden in forbidden_patterns:
            if forbidden in text:
                if "superseded" in text and forbidden == "novali-v6/dist/novali-v7_rc01-standalone.zip":
                    continue
...
def main() -> int:
    ...
    parser.add_argument(
        "--package-root",
        default=str(ROOT / "dist" / "novali-v7_rc01-standalone"),
    )
