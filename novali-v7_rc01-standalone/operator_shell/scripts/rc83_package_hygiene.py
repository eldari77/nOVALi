...
from operator_shell.observability.rc83 import resolve_rc83_artifact_root, scan_forbidden_strings, write_summary_artifacts
...
def _text_from_zip_member(package: zipfile.ZipFile, info: zipfile.ZipInfo) -> str:
    ...
    if info.file_size > 1_000_000:
        return ""
    if not info.filename.lower().endswith((".md", ".txt", ".json", ".yaml", ".yml", ".py", ".env", ".ps1", ".bat")):
        return ""
...
def _markdown_summary(summary: dict[str, Any]) -> str:
    lines = [
        ...
        f"- Delta bytes vs rc82: {summary.get('delta_bytes', 0)}",
...
def run_package_hygiene(
    *,
    ...
    summary = {
        "schema_name": "novali_rc83_package_hygiene_summary_v1",
...
def main() -> int:
    ...
    parser.add_argument(
        "--zip-path",
        default=str(ROOT / "dist" / "novali-v6_rc83-standalone.zip"),
    )
    parser.add_argument(
        "--baseline-zip-path",
        default=str(ROOT / "dist" / "novali-v6_rc82-standalone.zip"),
    )
