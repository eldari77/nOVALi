...
def _load_current_bootstrap_context(package_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "completed_reference_branch": "novali-v6",
        "reference_operator_surface_branch": "novali-v6",
        "frozen_fallback_reference_version": "novali-v5",
        "additional_reference_versions": ["novali-v4", "novali-v3"],
        "older_reference_version": "novali-v3",
        ...
        "branch_transition_reason": (
            "novali-v6 is frozen as the historical reference baseline at rc88.1 while novali-v7 becomes the "
            "clean active baseline under the same held baseline and directive-first bootstrap model."
        ),
        "branch_transition_status": "novali_v6_frozen_reference_novali_v7_active",
        "branch_transition_timestamp": _now(),
        "branch_transition_id": "branch_transition::novali_v6_to_novali_v7::clean_baseline",
...
def build_active_line_bootstrap_context(package_root: str | Path) -> dict[str, Any]:
    """Return a bootstrap context that treats the current v7 tree as the only live line."""

    package_root_path = Path(package_root).resolve()
    context = deepcopy(_load_current_bootstrap_context(package_root_path))
    for key in (
        "additional_reference_versions",
        "older_reference_version",
        "branch_transition_status",
        "branch_transition_id",
        "branch_transition_timestamp",
        "carried_forward_safe_trio_reference",
    ):
    ...
    context.update(
        {
            "active_branch": "novali-v7",
            "completed_reference_branch": "novali-v7",
            "reference_operator_surface_branch": "novali-v7",
            "frozen_fallback_reference_version": "novali-v7",
            "active_package_root_hint": package_root_path.name,
            "branch_transition_reason": (
                "Active-line LLM draft promotion uses novali-v7 as its own reference "
                "surface; older Novali versions are historical only and are not runtime "
                "or trusted-source dependencies."
            ),
            "dependency_boundary": (
                "novali-v7 is the active runtime and planning root; older Novali versions "
                "are historical only and are not trusted-source or runtime dependencies."
            ),
        }
    )
    return context
...
def build_standalone_directive_payload(
    *,
    ...
    package_root_path = Path(package_root).resolve()
    bootstrap_context = deepcopy(
        bootstrap_context
        if bootstrap_context is not None
        else _load_current_bootstrap_context(package_root_path)
    )
    directive_token = _slug(directive_id, fallback="novali_v7_standalone_directive")
    ...
    payload = {
        ...
        "directive_spec": {
            ...
            "trusted_sources": list(
                trusted_sources
                or [
                    "local_repo:novali-v6",
                    "local_artifacts:novali-v6/data",
...
__all__ = [
    "DIRECTIVE_FILE_SCHEMA_NAME",
    "DIRECTIVE_FILE_SCHEMA_VERSION",
    "build_active_line_bootstrap_context",
    "build_standalone_directive_payload",
    "write_standalone_directive",
]
