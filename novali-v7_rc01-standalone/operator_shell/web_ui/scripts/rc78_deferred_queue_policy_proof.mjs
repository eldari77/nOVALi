...
const secondaryBenchmarkDirectivePath = path.join(
  path.resolve(path.dirname(directivePath), ".."),
  "manual_acceptance_samples",
  "rc78_secondary_successor_package_readiness_benchmark_directive.json",
);
if (!fs.existsSync(secondaryBenchmarkDirectivePath)) {
  ...
  benchmarkDirective.directive_spec.directive_id =
    "directive_novali_v6_successor_package_readiness_benchmark_v1_rc78_secondary";
  ...
  benchmarkDirective.directive_spec.bucket_spec.bucket_id =
    "bucket_novali_v6_successor_package_readiness_benchmark_v1_rc78_secondary";
  fs.writeFileSync(
...
const runSummaryPath = path.join(artifactRoot, "rc78_operator_journey.json");
const runMarkdownPath = path.join(artifactRoot, "rc78_operator_journey.md");
const managerDigestAckSummaryJsonPath = path.join(artifactRoot, "deferred_queue_policy_summary.json");
const managerDigestAckSummaryMdPath = path.join(artifactRoot, "deferred_queue_policy_summary.md");
const operatorAgendaSummaryJsonPath = path.join(artifactRoot, "due_return_ordering_summary.json");
const operatorAgendaSummaryMdPath = path.join(artifactRoot, "due_return_ordering_summary.md");
...
const agendaProgressionAfterActionPath = path.join(
  artifactRoot,
  "agenda_progression_after_defer_return_and_action.json",
);
...
try {
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc78_01_shell_digest_queue_and_deferred.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc78_02_digest_anchor_recorded.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc78_03_two_deferred_items.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc78_04_queue_to_blocker.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc78_05_selected_session_after_continue.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc78_06_shell_after_action_return.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc78_07_due_item_current.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc78_08_manual_reopen.png"),
    fullPage: true,
  });
  ...
  writeText(
    ...
    [
      "# deferred queue policy summary",
      "",
      `- current agenda before action: \`${summary.manager_agenda_before?.current || "<none>"}\``,
      `- deferred queue after first defer: \`${summary.manager_agenda_after_first_defer?.deferred || "<none>"}\``,
      `- deferred queue after second defer: \`${summary.manager_agenda_after_second_defer?.deferred || "<none>"}\``,
      `- due queue after next manager check: \`${summary.manager_agenda_after_due_return?.due || "<none>"}\``,
      `- deferred queue after manual reopen: \`${summary.manager_agenda_after_manual_reopen?.deferred || "<none>"}\``,
  ...
  writeText(
    ...
    [
      ...
      `- agenda throughput after second defer: \`${summary.manager_agenda_after_second_defer?.throughput || "<none>"}\``,
      `- agenda throughput after action return: \`${summary.manager_agenda_after_return?.throughput || "<none>"}\``,
      `- agenda throughput after due return: \`${summary.manager_agenda_after_due_return?.throughput || "<none>"}\``,
      `- due-now count after due return: \`${summary.manager_agenda_after_due_return?.counts?.due_now || "<none>"}\``,
      `- reopened-since-last-check count after manual reopen: \`${summary.manager_agenda_after_manual_reopen?.counts?.reopened_since_last_check || "<none>"}\``,
      "",
    ].join("\n"),
  );
  ...
  try {
    await page.screenshot({
      path: path.join(screensDir, "rc78_zz_failure.png"),
      fullPage: true,
    });
  } catch {
