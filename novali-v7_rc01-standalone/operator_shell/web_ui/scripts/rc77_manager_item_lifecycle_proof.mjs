...
async function readManagerAgenda(page) {
  ...
  const currentDefer = panel.locator('[data-testid="manager-agenda-current-defer"]');
  const nextAction = panel.locator('[data-testid="manager-agenda-next-action"]');
  const nextDefer = panel.locator('[data-testid="manager-agenda-next-defer"]');
  ...
  return {
    ...
    current_action_label:
      (await currentAction.count()) > 0
        ...
        : "",
    current_defer_label:
      (await currentDefer.count()) > 0
        ? String((await currentDefer.first().innerText()) || "").trim()
        : "",
    next_action_label:
      (await nextAction.count()) > 0
        ...
        : "",
    next_defer_label:
      (await nextDefer.count()) > 0
        ? String((await nextDefer.first().innerText()) || "").trim()
        : "",
    ...
    counts: {
      ...
      returned_from_snooze_since_last_check: String((await page.locator('[data-testid="manager-agenda-count-returned_from_snooze_since_last_check"]').innerText()) || "").trim(),
    },
  };
}
...
async function clickManagerAgendaCurrentDefer(page) {
  const button = page.locator('[data-testid="manager-agenda-current-defer"]');
  await button.waitFor({ timeout: 120000 });
  const label = String((await button.innerText()) || "").trim();
  await button.click();
  return label;
...
async function clickManagerAgendaNextDefer(page) {
  const button = page.locator('[data-testid="manager-agenda-next-defer"]');
  await button.waitFor({ timeout: 120000 });
  const label = String((await button.innerText()) || "").trim();
  await button.click();
  return label;
...
async function clickManagerAgendaDeferredReopen(page) {
  const button = page.locator('[data-testid="manager-agenda-deferred-reopen"]');
  await button.waitFor({ timeout: 120000 });
  const label = String((await button.innerText()) || "").trim();
  await button.click();
  return label;
...
const runSummaryPath = path.join(artifactRoot, "rc77_operator_journey.json");
const runMarkdownPath = path.join(artifactRoot, "rc77_operator_journey.md");
const managerDigestAckSummaryJsonPath = path.join(artifactRoot, "manager_item_lifecycle_summary.json");
const managerDigestAckSummaryMdPath = path.join(artifactRoot, "manager_item_lifecycle_summary.md");
const operatorAgendaSummaryJsonPath = path.join(artifactRoot, "defer_snooze_reopen_summary.json");
const operatorAgendaSummaryMdPath = path.join(artifactRoot, "defer_snooze_reopen_summary.md");
const sinceLastManagerCheckDeltaPath = path.join(artifactRoot, "since_last_manager_check_delta.json");
const agendaProgressionAfterActionPath = path.join(artifactRoot, "agenda_progression_after_defer_and_action.json");
...
try {
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc77_01_shell_digest_and_queue.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc77_02_digest_anchor_recorded.png"),
    fullPage: true,
  });

  summary.defer_action.label = await clickManagerAgendaNextDefer(page);
  summary.defer_action.worked = true;
  await waitForCondition(async () => {
    const agenda = await readManagerAgenda(page);
    return (
      String(agenda.deferred || "").includes(summary.session_a.session_id) &&
      String(agenda.current || "").includes(summary.session_b.session_id)
    );
  }, 180000, "Manager agenda did not show the deferred recent item while keeping the current live session actionable.");
  summary.manager_agenda_after_defer = await readManagerAgenda(page);
  await page.screenshot({
    path: path.join(screensDir, "rc77_03_item_deferred.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc77_04_queue_to_blocker.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc77_05_selected_session_after_continue.png"),
    fullPage: true,
  });
  ...
  if (firstNumber(summary.manager_agenda_after_return.counts.deferred_items) < 1) {
    throw new Error("Manager agenda did not keep the deferred item visible after returning to /shell.");
  }
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc77_06_shell_after_return.png"),
    fullPage: true,
  });

  summary.reopen_action.label = await clickManagerAgendaDeferredReopen(page);
  summary.reopen_action.worked = true;
  await waitForCondition(async () => {
    const agenda = await readManagerAgenda(page);
    return (
      !String(agenda.deferred || "").includes(summary.session_b.session_id) &&
      (String(agenda.current || "").includes(summary.session_b.session_id) ||
        String(agenda.next || "").includes(summary.session_b.session_id))
    );
  }, 180000, "Deferred manager item did not return to the active agenda after explicit reopen.");
  summary.manager_agenda_after_reopen = await readManagerAgenda(page);
  await page.screenshot({
    path: path.join(screensDir, "rc77_07_deferred_item_reopened.png"),
    fullPage: true,
  });
  ...
  writeJson(managerDigestAckSummaryJsonPath, {
    ...
    manager_agenda_after_defer: summary.manager_agenda_after_defer,
    manager_agenda_after_return: summary.manager_agenda_after_return,
    manager_agenda_after_reopen: summary.manager_agenda_after_reopen,
    defer_action: summary.defer_action,
  ...
  writeText(
    ...
    [
      "# manager item lifecycle summary",
      "",
      `- current agenda before action: \`${summary.manager_agenda_before?.current || "<none>"}\``,
      `- deferred panel after snooze: \`${summary.manager_agenda_after_defer?.deferred || "<none>"}\``,
      `- completed panel after return: \`${summary.manager_agenda_after_return?.completed || "<none>"}\``,
      `- deferred panel after reopen: \`${summary.manager_agenda_after_reopen?.deferred || "<none>"}\``,
      `- defer action used: \`${summary.defer_action?.label || "<none>"}\``,
  ...
  writeText(
    ...
    [
      ...
      `- agenda throughput before action: \`${summary.manager_agenda_before?.throughput || "<none>"}\``,
      `- agenda throughput after defer: \`${summary.manager_agenda_after_defer?.throughput || "<none>"}\``,
      `- agenda throughput after return: \`${summary.manager_agenda_after_return?.throughput || "<none>"}\``,
      `- deferred count after return: \`${summary.manager_agenda_after_return?.counts?.deferred_items || "<none>"}\``,
      `- returned-from-snooze count after reopen: \`${summary.manager_agenda_after_reopen?.counts?.returned_from_snooze_since_last_check || "<none>"}\``,
      "",
    ].join("\n"),
  );
  writeJson(sinceLastManagerCheckDeltaPath, {
    ...
    agenda_after_defer: summary.manager_agenda_after_defer,
    agenda_after_reopen: summary.manager_agenda_after_reopen,
    queue_counts_before: summary.queue_counts_before,
    queue_counts_after_return: summary.queue_counts_after_return,
  });
  writeJson(agendaProgressionAfterActionPath, {
    ...
    after_defer: {
      agenda_current: summary.manager_agenda_after_defer?.current || "",
      agenda_next: summary.manager_agenda_after_defer?.next || "",
      agenda_deferred: summary.manager_agenda_after_defer?.deferred || "",
      agenda_throughput: summary.manager_agenda_after_defer?.throughput || "",
    },
    after_return: {
    ...
    after_reopen: {
      agenda_current: summary.manager_agenda_after_reopen?.current || "",
      agenda_next: summary.manager_agenda_after_reopen?.next || "",
      agenda_deferred: summary.manager_agenda_after_reopen?.deferred || "",
      agenda_throughput: summary.manager_agenda_after_reopen?.throughput || "",
    },
    defer_action: summary.defer_action,
    agenda_action: summary.agenda_action,
    reopen_action: summary.reopen_action,
  ...
  try {
    await page.screenshot({
      path: path.join(screensDir, "rc77_zz_failure.png"),
      fullPage: true,
    });
  } catch {
