...
async function readDeferredWorkloadDigest(page) {
  ...
  return {
    ...
    response_outcome:
      (await page.locator('[data-testid="deferred-workload-response-outcome"]').count()) > 0
        ? String((await page.locator('[data-testid="deferred-workload-response-outcome"]').innerText()) || "").trim()
        : "",
    response_outcome_detail:
      (await page.locator('[data-testid="deferred-workload-response-outcome-detail"]').count()) > 0
        ? String((await page.locator('[data-testid="deferred-workload-response-outcome-detail"]').innerText()) || "").trim()
        : "",
    response_outcome_basis:
      (await page.locator('[data-testid="deferred-workload-response-outcome-basis"]').count()) > 0
        ? String((await page.locator('[data-testid="deferred-workload-response-outcome-basis"]').innerText()) || "").trim()
        : "",
    response_outcome_prior_band:
      (await page.locator('[data-testid="deferred-workload-response-outcome-prior-band"]').count()) > 0
        ? String((await page.locator('[data-testid="deferred-workload-response-outcome-prior-band"]').innerText()) || "").trim()
        : "",
    response_outcome_current_band:
      (await page.locator('[data-testid="deferred-workload-response-outcome-current-band"]').count()) > 0
        ? String((await page.locator('[data-testid="deferred-workload-response-outcome-current-band"]').innerText()) || "").trim()
        : "",
    response_outcome_prior_response:
      (await page.locator('[data-testid="deferred-workload-response-outcome-prior-response"]').count()) > 0
        ? String((await page.locator('[data-testid="deferred-workload-response-outcome-prior-response"]').innerText()) || "").trim()
        : "",
...
const secondaryBenchmarkDirectivePath = path.join(
  path.resolve(path.dirname(directivePath), ".."),
  "manual_acceptance_samples",
  "rc81_secondary_successor_package_readiness_benchmark_directive.json",
);
if (!fs.existsSync(secondaryBenchmarkDirectivePath)) {
  ...
  benchmarkDirective.directive_spec.directive_id =
    "directive_novali_v6_successor_package_readiness_benchmark_v1_rc81_secondary";
  ...
  benchmarkDirective.directive_spec.bucket_spec.bucket_id =
    "bucket_novali_v6_successor_package_readiness_benchmark_v1_rc81_secondary";
  fs.writeFileSync(
...
const runSummaryPath = path.join(artifactRoot, "rc81_operator_journey.json");
const runMarkdownPath = path.join(artifactRoot, "rc81_operator_journey.md");
const managerDigestAckSummaryJsonPath = path.join(artifactRoot, "response_outcome_summary.json");
const managerDigestAckSummaryMdPath = path.join(artifactRoot, "response_outcome_summary.md");
const operatorAgendaSummaryJsonPath = path.join(artifactRoot, "pressure_recovery_tracking_summary.json");
const operatorAgendaSummaryMdPath = path.join(artifactRoot, "pressure_recovery_tracking_summary.md");
...
const agendaProgressionAfterActionPath = path.join(
  artifactRoot,
  "agenda_progression_after_response.json",
);
...
try {
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc81_01_shell_digest_queue_and_deferred_workload.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc81_02_digest_anchor_recorded.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc81_03_two_deferred_items.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc81_04_queue_to_blocker.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(screensDir, "rc81_05_selected_session_after_continue.png"),
    fullPage: true,
  });
  ...
  if (!/stable/i.test(summary.deferred_workload_after_return.response_outcome || "")) {
    throw new Error("Deferred workload digest did not classify the last suggested response outcome after returning from the handled item.");
  }
  if (!/last suggested response/i.test(summary.deferred_workload_after_return.response_outcome_basis || "")) {
    throw new Error("Deferred workload digest did not explain the response-outcome comparison basis after the handled item.");
  }
  if (!/Prior pressure: Low/i.test(summary.deferred_workload_after_return.response_outcome_prior_band || "")) {
    throw new Error("Deferred workload digest did not preserve the prior low-pressure band after the handled item.");
  }
  if (!/Current pressure: Low/i.test(summary.deferred_workload_after_return.response_outcome_current_band || "")) {
    throw new Error("Deferred workload digest did not preserve the current low-pressure band after the handled item.");
  }
  await page.screenshot({
    path: path.join(screensDir, "rc81_06_shell_after_action_return.png"),
    fullPage: true,
  });
  ...
  if (!/worsened/i.test(summary.deferred_workload_after_due_return.response_outcome || "")) {
    throw new Error("Deferred workload digest did not classify the last suggested response as worsened after deferred pressure rose.");
  }
  if (!/due-now|pressure band moved/i.test(summary.deferred_workload_after_due_return.response_outcome_detail || "")) {
    throw new Error("Deferred workload digest did not explain the worsened response outcome with the actual deferred-pressure driver.");
  }
  if (!/Prior pressure: Low/i.test(summary.deferred_workload_after_due_return.response_outcome_prior_band || "")) {
    throw new Error("Deferred workload digest did not preserve the prior low-pressure band after deferred pressure rose.");
  }
  if (!/Current pressure: Rising/i.test(summary.deferred_workload_after_due_return.response_outcome_current_band || "")) {
    throw new Error("Deferred workload digest did not report the current rising-pressure band after deferred pressure rose.");
  }
  await page.screenshot({
    path: path.join(screensDir, "rc81_07_due_item_current.png"),
    fullPage: true,
  });
  ...
  if (!summary.deferred_workload_after_manual_reopen.response_outcome) {
    throw new Error("Deferred workload digest did not keep response-outcome tracking visible after manual reopen.");
  }
  await page.screenshot({
    path: path.join(screensDir, "rc81_08_manual_reopen.png"),
    fullPage: true,
  });
  ...
  writeText(
    ...
    [
      ...
      `- response outcome after action return: \`${summary.deferred_workload_after_return?.response_outcome || "<none>"}\``,
      `- outcome detail after action return: \`${summary.deferred_workload_after_return?.response_outcome_detail || "<none>"}\``,
      ...
      `- response outcome after due return: \`${summary.deferred_workload_after_due_return?.response_outcome || "<none>"}\``,
      `- outcome detail after due return: \`${summary.deferred_workload_after_due_return?.response_outcome_detail || "<none>"}\``,
  ...
  writeText(
    ...
    [
      "# pressure recovery tracking summary",
      "",
      ...
      `- response outcome after action return: \`${summary.deferred_workload_after_return?.response_outcome || "<none>"}\``,
      `- prior/current pressure after action return: \`${summary.deferred_workload_after_return?.response_outcome_prior_band || "<none>"} | ${summary.deferred_workload_after_return?.response_outcome_current_band || "<none>"}\``,
      ...
      `- response outcome after due return: \`${summary.deferred_workload_after_due_return?.response_outcome || "<none>"}\``,
      `- prior/current pressure after due return: \`${summary.deferred_workload_after_due_return?.response_outcome_prior_band || "<none>"} | ${summary.deferred_workload_after_due_return?.response_outcome_current_band || "<none>"}\``,
  ...
  writeJson(agendaProgressionAfterActionPath, {
    ...
    after_return: {
      ...
      deferred_response_outcome: summary.deferred_workload_after_return?.response_outcome || "",
      deferred_response_outcome_detail: summary.deferred_workload_after_return?.response_outcome_detail || "",
      deferred_response_outcome_basis: summary.deferred_workload_after_return?.response_outcome_basis || "",
      deferred_response_prior_band: summary.deferred_workload_after_return?.response_outcome_prior_band || "",
      deferred_response_current_band: summary.deferred_workload_after_return?.response_outcome_current_band || "",
    ...
    after_due_return: {
      ...
      deferred_response_outcome: summary.deferred_workload_after_due_return?.response_outcome || "",
      deferred_response_outcome_detail: summary.deferred_workload_after_due_return?.response_outcome_detail || "",
      deferred_response_outcome_basis: summary.deferred_workload_after_due_return?.response_outcome_basis || "",
      deferred_response_prior_band: summary.deferred_workload_after_due_return?.response_outcome_prior_band || "",
      deferred_response_current_band: summary.deferred_workload_after_due_return?.response_outcome_current_band || "",
    ...
    after_manual_reopen: {
      ...
      deferred_response_outcome: summary.deferred_workload_after_manual_reopen?.response_outcome || "",
      deferred_response_outcome_detail: summary.deferred_workload_after_manual_reopen?.response_outcome_detail || "",
      deferred_response_outcome_basis: summary.deferred_workload_after_manual_reopen?.response_outcome_basis || "",
      deferred_response_prior_band: summary.deferred_workload_after_manual_reopen?.response_outcome_prior_band || "",
      deferred_response_current_band: summary.deferred_workload_after_manual_reopen?.response_outcome_current_band || "",
  ...
  try {
    await page.screenshot({
      path: path.join(screensDir, "rc81_zz_failure.png"),
      fullPage: true,
    });
  } catch {
