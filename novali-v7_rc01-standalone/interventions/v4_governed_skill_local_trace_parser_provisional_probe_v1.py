...
def _bucket_pressure(bucket_state: dict[str, Any], resource_ceilings: dict[str, Any]) -> dict[str, Any]:
    current_bucket = dict(bucket_state.get("current_bucket_state", {}))
    pressure = {
        "cpu_ratio_to_bucket": float(resource_ceilings.get("cpu_parallel_units", 0) or 0)
        / max(float(dict(current_bucket.get("cpu_limit", {})).get("max_parallel_processes", 1) or 1), 1.0),
        "memory_ratio_to_bucket": float(resource_ceilings.get("memory_mb", 0) or 0)
        / max(float(dict(current_bucket.get("memory_limit", {})).get("max_working_set_mb", 1) or 1), 1.0),
        "storage_ratio_to_bucket": float(resource_ceilings.get("storage_write_mb", 0) or 0)
        / max(float(dict(current_bucket.get("storage_limit", {})).get("max_write_mb_per_action", 1) or 1), 1.0),
    }
    pressure["concern_level"] = "low" if max(pressure.values()) <= 0.25 else "moderate"
    return pressure


def _path_within_allowed_roots(path: Path, allowed_roots: list[Path]) -> bool:
    resolved_path = path.resolve()
    return any(str(resolved_path).startswith(str(root)) for root in allowed_roots)
...
def run_probe(cfg, proposal, *, rounds, seeds):
    ...
    if not all([governance_snapshot, trial_execution_snapshot, trial_evidence_snapshot, provisional_admission_snapshot]):
        return {
            "passed": False,
            "shadow_contract": "provisional_skill_probe",
            "proposal_semantics": "shadow_skill_trial",
            "reason": "shadow provisional probe failed: governance, prior trial, trial-evidence, and provisional-admission artifacts are required",
            "observability_gain": {"passed": False, "reason": "missing prerequisite provisional artifacts"},
            "activation_analysis_usefulness": {"passed": False, "reason": "missing prerequisite provisional artifacts"},
            "ambiguity_reduction": {"passed": False, "reason": "missing prerequisite provisional artifacts"},
            "safety_neutrality": {"passed": True, "scope": str(proposal.get("scope", "")), "reason": "no live-policy mutation occurred"},
            "later_selection_usefulness": {"passed": False, "reason": "the provisional probe cannot run without its governance-owned prerequisite chain"},
        }
    ...
    if not all([directive_state, bucket_state, self_structure_state, branch_registry]):
        return {
            ...
            "safety_neutrality": {"passed": True, "scope": str(proposal.get("scope", "")), "reason": "no live-policy mutation occurred"},
            "later_selection_usefulness": {"passed": False, "reason": "the provisional probe cannot stay governance-owned without directive, bucket, self-structure, and branch state"},
        }
    ...
    if current_branch_state != "paused_with_baseline_held" or str(last_provisional_admission.get("provisional_admission_outcome", "")) != "admissible_for_provisional_handling":
        return {
            ...
            "safety_neutrality": {"passed": True, "scope": str(proposal.get("scope", "")), "reason": "no live-policy mutation occurred"},
            "later_selection_usefulness": {"passed": False, "reason": "the provisional probe can only run inside the already admitted paused branch envelope"},
        }
    ...
    if not bool(log_group.get("passed", False)):
        return {
            ...
            "safety_neutrality": {"passed": True, "scope": str(proposal.get("scope", "")), "reason": "no live-policy mutation occurred"},
            "later_selection_usefulness": {"passed": False, "reason": "the provisional probe has no trusted local logs to parse"},
        }
    ...
    recognized_line_share = float(parsed_log_summary.get("recognized_line_share_weighted", 0.0) or 0.0)
    seed_log_coverage = int(parsed_log_summary.get("parsed_file_count", 0) or 0)
    ...
    artifact_path = _diagnostic_artifact_dir() / f"proposal_learning_loop_v4_governed_skill_local_trace_parser_provisional_probe_v1_{proposal['proposal_id']}.json"
    ...
    deprecation_trigger_status = {
        "repeated_low_distinct_utility": dummy_eval_count < int(evidence_obligations.get("dummy_eval_count_min", 100) or 100),
        "directive_relevance_decay": str(primary_candidate.get("directive_relevance", "")) != str(evidence_obligations.get("directive_relevance_required", "high")),
        "duplication_overlap_growth": str(primary_candidate.get("duplication_risk", "")) != "low",
    ...
    if evidence_obligation_status["passed"] and envelope_compliance["passed"] and not rollback_trigger_status["any_trigger_fired"] and not deprecation_trigger_status["any_trigger_active"]:
        ...
        best_next_template = "memory_summary.v4_governed_skill_provisional_evidence_snapshot_v1"
        ...
        outcome_reason = "the provisional rerun surfaced an envelope or rollback-trigger problem that blocks further escalation"
    else:
        candidate_status = "downgraded_to_sandbox_only"
        best_next_template = "memory_summary.v4_governed_skill_provisional_gap_snapshot_v1"
        outcome_reason = "the provisional rerun stayed safe but no longer clears the evidence bar for continued provisional handling"

    payload = {
        "proposal_id": str(proposal.get("proposal_id")),
        "template_name": "proposal_learning_loop.v4_governed_skill_local_trace_parser_provisional_probe_v1",
        ...
        "governed_skill_provisional_probe_summary": {
            ...
            "what_the_probe_did": [
                ...
                "read the latest trial execution, trial evidence, and provisional admission artifacts as trusted governance-owned context",
                "materialized only shadow-only evidence, obligation status, and rollback/deprecation status without opening or mutating a branch",
            ],
            ...
            "evidence_produced": {
                "utility_assessment": {
                    ...
                    "reason": outcome_reason if candidate_status == "viable_for_continued_provisional_handling" else "the provisional rerun did not fully preserve the admitted evidence contract",
                    "dummy_eval_count": dummy_eval_count,
                    "patch_tuple_count": patch_tuple_count,
                    "recognized_line_share_weighted": recognized_line_share,
                },
                "directive_relevance": {
                    "passed": str(primary_candidate.get("directive_relevance", "")) == "high",
                ...
                "duplication_overlap": {
                    "passed": str(primary_candidate.get("duplication_risk", "")) == "low",
            ...
            "governance_source_of_truth": {
                ...
                "reason": "the provisional probe derives its rules and evidence contract from governance artifacts rather than from execution code alone",
            },
        },
        "analytics_context": {
        ...
        "observability_gain": {
            "passed": True,
            "reason": "the admitted provisional skill now has a second bounded execution record with explicit obligation and trigger tracking",
            "artifact_paths": {
                "provisional_probe_artifact": str(artifact_path),
                "self_structure_state_latest": str(SELF_STRUCTURE_STATE_PATH),
                "self_structure_ledger": str(SELF_STRUCTURE_LEDGER_PATH),
            },
        },
        "activation_analysis_usefulness": {"passed": True, "reason": "the provisional probe shows whether continued provisional handling remains justified without drifting toward retained promotion"},
        "ambiguity_reduction": {"passed": True, "score": 0.99, "reason": "the provisional rerun cleanly distinguishes continued provisional viability, downgrade to sandbox-only, and block-from-escalation outcomes"},
        "safety_neutrality": {"passed": True, "scope": str(proposal.get("scope", "")), "reason": "the provisional probe remained shadow-only, local, reversible, and governance-bounded; no live behavior change occurred"},
        "later_selection_usefulness": {"passed": True, "recommended_next_template": best_next_template, "reason": "the next step should review this provisional evidence before any future retention discussion"},
        "diagnostic_conclusions": {
            "governed_skill_provisional_probe_in_place": True,
    ...
    updated_governed_skill_subsystem["last_provisional_execution_outcome"] = {
        ...
        "provisional_execution_outcome": "provisional_probe_completed",
    ...
    updated_current_state_summary.update(
        {
            "latest_skill_provisional_execution_outcome": "provisional_probe_completed",
    ...
    ledger_event = {
        "event_id": f"governed_skill_local_trace_parser_provisional_probe_v1::{proposal['proposal_id']}",
        "timestamp": _now(),
        "event_type": "governed_skill_local_trace_parser_provisional_probe_v1_materialized",
        ...
        "artifact_paths": {
            ...
            "skill_provisional_execution_artifact": str(artifact_path),
        },
    ...
    return {
        ...
        "shadow_contract": "provisional_skill_probe",
        "proposal_semantics": "shadow_skill_trial",
        "reason": "shadow provisional probe passed: the admitted local trace parser remained bounded and governance-compliant across another provisional run without reopening the branch",
