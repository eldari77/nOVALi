...
from .v4_governed_work_loop_candidate_screen_snapshot_v3 import (
    _build_loop_candidate_examples_v3,
...
def _build_frontier_containment_audit(
    *,
    directive_state: dict[str, Any],
    current_state_summary: dict[str, Any],
    branch_record: dict[str, Any],
    ...
    evidence_chain_state = dict(evidence_v2_summary.get("current_work_loop_chain_state", {}))
    future_posture = dict(evidence_v2_summary.get("future_posture", {}))
    ...
    frontier_signal_components = {
        ...
        "future_posture_review_gate_closed": str(dict(evidence_v2_summary.get("future_posture_review_gate", {})).get("gate_status", "")) == "defined_but_closed",
    ...
    frontier_signal_count = sum(1 for value in frontier_signal_components.values() if bool(value))
    ...
    checks = {
        ...
        "work_loop_progression_state_current": _check(
            bool(current_state_summary.get("governed_work_loop_policy_v1_defined", False))
            and bool(current_state_summary.get("governed_work_loop_candidate_screening_v3_in_place", False))
            and bool(current_state_summary.get("governed_work_loop_continuation_admission_v3_in_place", False))
            and str(current_state_summary.get("latest_governed_work_loop_continuation_outcome", ""))
            == "admissible_for_governed_loop_continuation_v3"
            and str(current_state_summary.get("latest_governed_work_loop_execution_readiness", ""))
            in {
                "ready_for_shadow_work_loop_continuation_execution_v3",
                "ready_for_work_loop_evidence_review_v3",
            },
            "current state still reflects the v3 candidate screen, v3 continuation admission, and ready-for-shadow-execution posture",
        ),
        "paused_parser_capability_still_held_not_reopened": _check(
            bool(parser_capability)
            and str(parser_capability.get("status", "")) == "paused_with_provisional_capability_held"
        ...
        "frontier_top_candidate_matches_admitted_execution": _check(
            ...
            "the active frontier candidate still matches both the top-ranked v3 screen result and the admitted v3 execution target",
        ),
        ...
        "frontier_materially_distinct": _check(
            ...
            "the active frontier element remains materially distinct from direct work, continuation v1, continuation v2, and evidence v2",
            observed={
        ...
        "future_posture_review_gate_stays_closed": _check(
            str(dict(evidence_v2_summary.get("future_posture_review_gate", {})).get("gate_status", "")) == "defined_but_closed"
        ...
        "narrow_posture_artifacts_present": _check(
            all(path.exists() for path in work_loop_artifact_paths.values()),
            "the frontier-containment audit still sits on top of the local narrow-posture artifact chain",
    ...
    failed = [name for name, result in checks.items() if not bool(dict(result).get("passed", False))]
    frontier_containment_status = "contained" if not failed and material_frontier_signal_detected else "not_contained"
    frontier_drift_status = (
        "frontier_drift_absent"
        if not unsupported_governed_templates
        and (str(posture_pressure.get("pressure", "")) == "absent" or str(posture_pressure.get("classification", "")) == "posture_pressure_absent")
        else "frontier_drift_present"
    )
    frontier_alignment_status = (
    ...
    return {
        "audit_focus": "governance_recommendation_frontier_containment_audit",
        "checks": checks,
        ...
        "high_signal_observations": [
            (
                "the current governed recommendation frontier remains contained to one materially distinct bounded continuation candidate"
                if frontier_containment_status == "contained"
                else "the current governed recommendation frontier is no longer cleanly contained under the narrow envelope"
            ),
            (
                "this audit adds a new frontier-quality signal about containment and frontier pressure rather than replaying prior chain conclusions"
                if material_frontier_signal_detected
                else "this audit stayed readable but did not add enough new frontier-quality signal beyond the prior chain"
            ),
        ],
        "summary": {
            "total_check_count": int(len(checks)),
            ...
            "failed_check_count": int(len(failed)),
            "failed_check_names": failed,
            "frontier_signal_count": int(frontier_signal_count),
            "material_frontier_signal_detected": bool(material_frontier_signal_detected),
            "frontier_containment_status": frontier_containment_status,
            "frontier_drift_status": frontier_drift_status,
            "frontier_alignment_status": frontier_alignment_status,
        },
    }
...
def run_probe(cfg, proposal, *, rounds, seeds):
    ...
    if not all(
        ...
        return _failure(
            proposal,
            "shadow work-loop continuation failed: governance substrate, work-loop policy, v3 candidate screen, v3 continuation admission, posture, evidence, prior continuation chain, direct-work chain, and capability-use evidence artifacts are required",
        )
    ...
    if str(last_continuation_admission.get("status", "")) != "admissible_for_governed_loop_continuation_v3":
        return _failure(
            proposal,
            "shadow work-loop continuation failed: the surviving v3 loop candidate is not currently admitted for governed work-loop continuation v3",
        )
    ...
    if execution_readiness not in {
        ...
        return _failure(
            proposal,
            "shadow work-loop continuation failed: self-structure does not currently mark the v3 loop continuation path as ready for shadow execution",
        )
    if execution_readiness == "ready_for_work_loop_evidence_review_v3" and latest_execution_outcome != "shadow_work_loop_continuation_v3_completed":
        return _failure(
            proposal,
            "shadow work-loop continuation failed: evidence-review readiness was present without a completed v3 continuation execution",
        )
    ...
    if not all(
        ...
        return _failure(
            proposal,
            "shadow work-loop continuation failed: one or more governing artifact paths for v3 loop continuation could not be resolved",
        )
    ...
    work_loop_candidate_screen_summary = dict(
        work_loop_candidate_screen_payload.get("governed_work_loop_candidate_screen_v3_summary", {})
    )
    continuation_admission_summary = dict(continuation_admission_payload.get("governed_work_loop_continuation_admission_v3_summary", {}))
    ...
    if not dict(prior_work_loop_evidence_payload.get("governed_work_loop_evidence_summary", {})):
        return _failure(proposal, "shadow work-loop continuation failed: prior work-loop evidence summary is missing")
    if not dict(work_loop_evidence_payload.get("governed_work_loop_evidence_v2_summary", {})):
        return _failure(proposal, "shadow work-loop continuation failed: work-loop evidence v2 summary is missing")
    if not all([work_loop_candidate_screen_summary, continuation_admission_summary, prior_continuation_execution_payload_v1, prior_continuation_execution_payload_v2, direct_work_execution_payload]):
        return _failure(
            proposal,
            "shadow work-loop continuation failed: governing summary payloads for v3 loop continuation execution are incomplete",
        )
    ...
    evidence_v2_summary = dict(work_loop_evidence_payload.get("governed_work_loop_evidence_v2_summary", {}))
    ...
    if not all([posture_summary, prior_work_loop_evidence_summary, evidence_v2_summary, direct_work_evidence_summary, capability_use_evidence_summary]):
        return _failure(
            proposal,
            "shadow work-loop continuation failed: one or more governing summaries required for frontier containment are missing",
        )

    posture_current = dict(posture_summary.get("current_posture", {}))
    future_gate = dict(evidence_v2_summary.get("future_posture_review_gate", {}))
    loop_chain_state = dict(evidence_v2_summary.get("current_work_loop_chain_state", {}))
    ...
    screened_candidates = [
        _screen_loop_candidate_v3(
            ...
            loop_continuation_future_posture=str(dict(evidence_v2_summary.get("future_posture", {})).get("category", "")),
    ...
    frontier_containment_audit = _build_frontier_containment_audit(
        ...
        evidence_v2_summary=evidence_v2_summary,
    ...
    next_template = "memory_summary.v4_governed_work_loop_evidence_snapshot_v3"
    artifact_path = _diagnostic_artifact_dir() / (
        f"proposal_learning_loop_v4_governed_work_loop_governance_recommendation_frontier_containment_audit_v1_{proposal['proposal_id']}.json"
    )
    ...
    material_frontier_signal_detected = bool(dict(frontier_containment_audit.get("summary", {})).get("material_frontier_signal_detected", False))
    distinct_value_read = {
        "passed": bool(material_frontier_signal_detected),
        "value": "high" if bool(material_frontier_signal_detected) else "low",
        "reason": (
            "the continuation produced a bounded frontier-containment audit with a new recommendation-frontier quality signal beyond the prior direct-work step, continuation v1, and continuation v2"
            if bool(material_frontier_signal_detected)
        ...
        "vs_prior_direct_work": {
            "distinct": True,
            "reason": "the execution audited frontier containment rather than replaying the governance state coherence audit shape",
            "observed_deltas": dict(dict(frontier_containment_audit.get("frontier_observations", {})).get("frontier_chain_deltas", {})).get("vs_prior_direct_work", {}),
        },
        "vs_prior_continuation_v1": {
            "distinct": True,
            "reason": "the execution audited frontier containment rather than replaying the earlier ledger consistency delta audit",
            "observed_deltas": dict(dict(frontier_containment_audit.get("frontier_observations", {})).get("frontier_chain_deltas", {})).get("vs_prior_continuation_v1", {}),
        },
        "vs_prior_continuation_v2": {
            "distinct": True,
            "reason": "the execution audited frontier containment rather than replaying the earlier recommendation-to-ledger alignment delta audit",
            "observed_deltas": dict(dict(frontier_containment_audit.get("frontier_observations", {})).get("frontier_chain_deltas", {})).get("vs_prior_continuation_v2", {}),
        },
    }
    directive_support_value = {
        "passed": True,
        "value": "high",
        "reason": "the frontier-containment audit produced a bounded governance continuation report that directly supports directive-bound narrow work-loop control",
    }
    ...
    review_trigger_status = {
        ...
        "scope_expands_beyond_recommendation_frontier_containment_audit": False,
    ...
    continuation_accounting = {
        ...
        "admission_outcome": "admissible_for_governed_loop_continuation_v3",
        "work_loop_posture": "shadow_only_loop_continuation_v3",
        ...
        "continuation_summary": "governance recommendation-frontier containment audit",
        ...
        "usefulness_signal_summary": {
            "passed_check_count": int(dict(frontier_containment_audit.get("summary", {})).get("passed_check_count", 0) or 0),
            "failed_check_count": int(dict(frontier_containment_audit.get("summary", {})).get("failed_check_count", 0) or 0),
            "frontier_signal_count": int(dict(frontier_containment_audit.get("summary", {})).get("frontier_signal_count", 0) or 0),
            "material_frontier_signal_detected": bool(dict(frontier_containment_audit.get("summary", {})).get("material_frontier_signal_detected", False)),
        },
    ...
    artifact_payload = {
        ...
        "template_name": "proposal_learning_loop.v4_governed_work_loop_governance_recommendation_frontier_containment_audit_v1",
        ...
        "comparison_references": {
            ...
            "memory_summary.v4_governed_work_loop_candidate_screen_snapshot_v3": _artifact_reference(work_loop_candidate_screen_snapshot, latest_snapshots),
            "memory_summary.v4_governed_work_loop_continuation_admission_snapshot_v3": _artifact_reference(work_loop_continuation_admission_snapshot, latest_snapshots),
            "memory_summary.v4_governed_work_loop_posture_snapshot_v1": _artifact_reference(work_loop_posture_snapshot, latest_snapshots),
            "memory_summary.v4_governed_work_loop_evidence_snapshot_v1": _artifact_reference(prior_work_loop_evidence_snapshot, latest_snapshots),
            "memory_summary.v4_governed_work_loop_evidence_snapshot_v2": _artifact_reference(work_loop_evidence_snapshot, latest_snapshots),
        ...
        "governed_work_loop_continuation_execution_summary": {
            "candidate_executed": {
                ...
                "continuation_variant": "v3_frontier_containment",
            },
            "what_the_continuation_did": [
                "read only the admitted trusted local governance artifacts relevant to recommendation-frontier containment under the narrow envelope",
                "computed a bounded frontier-containment audit against the current three-step governed chain rather than replaying earlier state, ledger, or alignment audit shapes",
                "materialized a shadow-only governed work-loop continuation v3 artifact with full continuation accounting and trigger status",
            ],
            "local_governance_sources_used": {
                "source_artifact_paths": source_artifact_paths,
                "governance_source_digests": governance_source_digests,
            },
            "frontier_containment_artifact_produced": {
                "bounded_output_artifact_path": str(artifact_path),
                "governance_recommendation_frontier_containment_audit_output": frontier_containment_audit,
            },
            ...
            "frontier_containment_result": {
                "classification": str(dict(frontier_containment_audit.get("summary", {})).get("frontier_containment_status", "")),
                "frontier_drift_status": str(dict(frontier_containment_audit.get("summary", {})).get("frontier_drift_status", "")),
                "frontier_alignment_status": str(dict(frontier_containment_audit.get("summary", {})).get("frontier_alignment_status", "")),
            },
            "new_frontier_quality_signal_assessment": {
                "classification": "new_frontier_quality_signal_present" if bool(material_frontier_signal_detected) else "new_frontier_quality_signal_absent",
                "signal_count": int(dict(frontier_containment_audit.get("summary", {})).get("frontier_signal_count", 0) or 0),
            ...
            "why_governance_remains_source_of_truth": {
                ...
                "reason": "the run consumed directive, bucket, branch, self-structure, work-loop policy, candidate-screen v3, posture, continuation-admission v3, prior continuation chain, and prior direct-work governance artifacts as read-only authority while performing only the admitted v3 loop-continuation audit",
            },
        },
        "analytics_context": {
        ...
        "activation_analysis_usefulness": {
            "passed": True,
            "reason": "the run proves that NOVALI can execute a third distinct admitted governed work-loop continuation without collapsing into direct-work repetition, prior-continuation replay, capability use, hidden development, or silent broadening",
        },
        "ambiguity_reduction": {
            "passed": True,
            "score": 0.99,
            "reason": "the execution adds a concrete frontier-containment signal while keeping loop continuation separate from prior direct-work repetition, capability-use, paused-line reopen, and new-skill creation",
        },
        "safety_neutrality": {
            "passed": True,
            "scope": str(proposal.get("scope", "")),
        ...
        "later_selection_usefulness": {
            ...
            "reason": "the next step should review the v3 frontier-containment evidence before considering any further bounded continuation search or posture-review criteria",
        },
        "diagnostic_conclusions": {
            "governed_work_loop_continuation_execution_in_place": True,
            "governed_work_loop_continuation_v3_execution_in_place": True,
            ...
            "frontier_containment_status": str(dict(frontier_containment_audit.get("summary", {})).get("frontier_containment_status", "")),
            "new_frontier_quality_signal_present": bool(material_frontier_signal_detected),
    ...
    updated_work_loop_policy["last_work_loop_continuation_execution_outcome"] = {
        "loop_candidate_id": str(dict(continuation_admission_summary.get("candidate_under_review", {})).get("loop_candidate_id", "")),
        "loop_candidate_name": primary_candidate_name,
        "work_loop_continuation_execution_outcome": "shadow_work_loop_continuation_v3_completed",
    ...
    updated_current_state_summary.update(
        {
            ...
            "governed_work_loop_execution_v3_in_place": True,
            "latest_governed_work_loop_execution_candidate": primary_candidate_name,
            "latest_governed_work_loop_execution_outcome": "shadow_work_loop_continuation_v3_completed",
            "latest_governed_work_loop_execution_readiness": "ready_for_work_loop_evidence_review_v3",
            "latest_governed_work_loop_operational_status": (
                "operational_recommendation_frontier_containment_continuation_proven"
            ...
            "latest_governed_work_loop_readiness": "ready_for_work_loop_evidence_review_v3",
            "current_branch_state": current_branch_state,
    ...
    _append_jsonl(
        SELF_STRUCTURE_LEDGER_PATH,
        {
            "event_id": f"governed_work_loop_governance_recommendation_frontier_containment_audit_v1::{proposal['proposal_id']}",
            "timestamp": _now(),
            "event_type": "governed_work_loop_governance_recommendation_frontier_containment_audit_v1_materialized",
            ...
            "artifact_paths": {
                ...
                "governed_work_loop_candidate_screen_v3": str(work_loop_candidate_screen_artifact_path),
                "governed_work_loop_continuation_admission_v3": str(work_loop_continuation_admission_artifact_path),
                ...
                "governed_work_loop_evidence_v2": str(work_loop_evidence_artifact_path),
                ...
                "governed_work_loop_continuation_execution_v3": str(artifact_path),
            },
    ...
    return {
        ...
        "shadow_contract": "governed_work_loop_continuation",
        "proposal_semantics": "shadow_work_loop_continuation",
        "reason": "shadow work-loop continuation v3 passed: the admitted recommendation-frontier containment audit executed inside the approved continuation envelope without repeating prior work, invoking capability use, reopening capabilities, or widening posture",
