...
def _select_latest_successful_frontier_containment_artifact(preferred_path: Path | None) -> Path | None:
    pattern = "proposal_learning_loop_v4_governed_work_loop_governance_recommendation_frontier_containment_audit_v1_*.json"
    ...
    for candidate in candidates:
        ...
        result = dict(execution_summary.get("frontier_containment_result", {}))
        ...
        if (
            str(result.get("classification", "")) == "contained"
            and str(result.get("frontier_drift_status", "")) == "frontier_drift_absent"
            and str(result.get("frontier_alignment_status", "")) == "bounded_alignment_preserved"
...
def _frontier_signal_summary(execution_summary: dict[str, Any]) -> dict[str, Any]:
    frontier_artifact = dict(execution_summary.get("frontier_containment_artifact_produced", {}))
    audit_output = dict(frontier_artifact.get("governance_recommendation_frontier_containment_audit_output", {}))
    ...
    return {
        ...
        "frontier_containment_status": str(
            result.get("classification", summary.get("frontier_containment_status", ""))
        ),
        "frontier_drift_status": str(result.get("frontier_drift_status", summary.get("frontier_drift_status", ""))),
        "frontier_alignment_status": str(
            result.get("frontier_alignment_status", summary.get("frontier_alignment_status", ""))
        ),
        ...
        "frontier_signal_count": int(
            usefulness.get(
                "frontier_signal_count",
                summary.get("frontier_signal_count", signal.get("signal_count", 0)),
            )
            or 0
        ),
        "material_frontier_signal_detected": bool(
            usefulness.get(
                "material_frontier_signal_detected",
                summary.get(
                    "material_frontier_signal_detected",
...
def _operational_usefulness_assessment(execution_summary: dict[str, Any]) -> dict[str, Any]:
    ...
    operationally_meaningful = (
        ...
        and signal_summary["frontier_containment_status"] == "contained"
    ...
    return {
        ...
        "audit_signal_summary": {
            "frontier_containment_status": signal_summary["frontier_containment_status"],
        ...
        "reason": (
            "the continuation produced a real directive-supportive frontier-containment artifact with a new material frontier-quality signal and no failed checks"
            if operationally_meaningful
            else "the continuation did not produce enough bounded frontier-containment signal to count as an operationally meaningful governed work-loop continuation"
        ),
    }
...
def _continuation_value_assessment(execution_summary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "reason": (
            "the continuation added real bounded frontier-containment value beyond the prior chain while staying cleanly separated from capability use, reopen, and new-skill paths"
...
def _distinctness_chain_assessment(
    *,
    ...
    continuation_v3_execution_summary: dict[str, Any],
) -> dict[str, Any]:
    ...
    continuation_v3_name = str(
        dict(continuation_v3_execution_summary.get("candidate_executed", {})).get("loop_candidate_name", "")
    )
    unique_names = {
        name
        for name in [direct_work_name, continuation_v1_name, continuation_v2_name, continuation_v3_name]
        if name
    }
    v1_vs_direct = bool(
    ...
    v3_checks = dict(
        dict(
            dict(continuation_v3_execution_summary.get("frontier_containment_artifact_produced", {})).get(
                "governance_recommendation_frontier_containment_audit_output", {}
            )
        ).get("checks", {})
    )
    v3_vs_chain = dict(dict(v3_checks.get("frontier_materially_distinct", {})).get("observed", {}))
    structurally_distinct = (
        len(unique_names) == 4
        and v1_vs_direct
        and v2_vs_direct
        and v2_vs_v1
        and bool(v3_vs_chain.get("distinct_from_direct_work", False))
        and bool(v3_vs_chain.get("distinct_from_continuation_v1", False))
        and bool(v3_vs_chain.get("distinct_from_continuation_v2", False))
        and bool(v3_vs_chain.get("distinct_from_evidence_snapshot_v2", False))
    )
    return {
        ...
        "vs_prior_direct_work": {
            ...
            "continuation_v3": bool(v3_vs_chain.get("distinct_from_direct_work", False)),
        },
        "vs_prior_continuation_v1": {
            "continuation_v2": v2_vs_v1,
            "continuation_v3": bool(v3_vs_chain.get("distinct_from_continuation_v1", False)),
        },
        "vs_prior_continuation_v2": bool(v3_vs_chain.get("distinct_from_continuation_v2", False)),
        "vs_evidence_snapshot_v2": bool(v3_vs_chain.get("distinct_from_evidence_snapshot_v2", False)),
        "reason": (
            "direct work, continuation v1, continuation v2, and frontier containment are all measurably distinct bounded steps rather than diminishing-return governance recursion"
...
def _repeated_bounded_success_assessment(
    *,
    ...
    continuation_v2_success = bool(
        dict(dict(prior_chain_evidence_summary.get("current_work_loop_chain_state", {})).get("continuation_v2", {})).get(
            "operational_success",
            False,
        )
    )
    continuation_v3_success = (
        _step_success(_operational_usefulness_assessment(continuation_v3_execution_summary))
        and _step_success(
        ...
        and _step_success(_envelope_compliance_assessment(continuation_v3_execution_summary, current_state_summary))
        and _step_success(_continuation_value_assessment(continuation_v3_execution_summary))
    )
    success_count = int(
        sum(
            1
    ...
    repeated_bounded_success = success_count == 4
    strengthened = repeated_bounded_success and prior_success_count == 3
    return {
        ...
        "reason": (
            "repeated bounded success is now stronger at the chain level because the fourth bounded continuation also executed cleanly"
            if strengthened
...
def _posture_discipline_assessment(
    *,
    ...
    continuation_v3_envelope = _envelope_compliance_assessment(continuation_v3_execution_summary, current_state_summary)
    discipline_holds = (
        ...
        and bool(continuation_v3_envelope.get("passed", False))
    )
    return {
        ...
        "reason": (
            "narrow posture discipline is still holding cleanly across direct work, continuation v1, continuation v2, and frontier containment"
            if discipline_holds
...
def _hidden_development_pressure_assessment(
    *,
    ...
    continuation_v3_clean = bool(
        dict(continuation_v3_execution_summary.get("hidden_capability_pressure_read", {})).get("passed", False)
    ) and str(dict(continuation_v3_execution_summary.get("hidden_capability_pressure_read", {})).get("value", "")) == "none"
    review_status = dict(
        dict(continuation_v3_execution_summary.get("review_rollback_deprecation_trigger_status", {})).get(
...
def _new_execution_signal_assessment(execution_summary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "reason": (
            "the frontier-containment execution adds a new frontier-quality signal about containment, drift, and bounded alignment rather than merely restating prior chain conclusions"
            if materially_improved
            else "the frontier-containment execution mostly restates prior chain conclusions without adding enough new frontier-quality evidence"
        ),
    }
...
def _structural_vs_recursive_assessment(
    *,
    ...
    low_recursion_check = bool(
        dict(
            dict(
                dict(execution_summary.get("frontier_containment_artifact_produced", {})).get(
                    "governance_recommendation_frontier_containment_audit_output", {}
                )
            ).get("checks", {})
        )
...
def _gate_status_assessment(
    *,
    ...
    prior_gate_status = str(dict(prior_chain_evidence_summary.get("future_posture_review_gate", {})).get("gate_status", ""))
    ...
    return {
        ...
        "proposed_gate_criteria": list(
            dict(prior_chain_evidence_summary.get("future_posture_review_gate", {})).get("proposed_gate_criteria", [])
        ),
        "reason": (
            "the future posture-review gate remains defined but closed after the frontier-containment execution"
            if gate_closed
...
def run_probe(cfg, proposal, *, rounds, seeds):
    ...
    if not all(
        ...
        return {
            ...
            "reason": "diagnostic shadow failed: work-loop evidence v3 requires the governance substrate, work-loop policy, candidate screen v3, continuation admission v3, posture v1, evidence v1, evidence v2, frontier-containment execution, prior continuation executions, direct-work evidence, and capability-use evidence artifacts",
    ...
    if (
        ...
        or not readiness.intersection(
            {
                ...
                "ready_for_work_loop_candidate_screen_v4_later",
        ...
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: branch must remain paused_with_baseline_held, governed work-loop policy state must exist, and state must be at or beyond ready_for_work_loop_evidence_review_v3",
    ...
    current_execution_artifact_path = _select_latest_successful_frontier_containment_artifact(
    ...
    if not all(
        ...
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: expected governed work-loop v3, posture, direct-work, or capability-use artifact paths could not be resolved",
    ...
    if not all(
        ...
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: one or more required governed work-loop v3, posture, direct-work, or capability-use artifacts could not be loaded",
    ...
    candidate_screen_summary = dict(candidate_screen_payload.get("governed_work_loop_candidate_screen_v3_summary", {}))
    continuation_admission_summary = dict(
        continuation_admission_payload.get("governed_work_loop_continuation_admission_v3_summary", {})
    )
    ...
    prior_chain_evidence_summary = dict(
        prior_chain_evidence_payload.get("governed_work_loop_evidence_v2_summary", {})
    )
    ...
    if not all(
        ...
        return {
            ...
            "shadow_contract": "diagnostic_probe",
            "proposal_semantics": "diagnostic",
            "reason": "diagnostic shadow failed: required governed work-loop v3, evidence v1, evidence v2, or direct-work summaries were missing from the loaded artifacts",
    ...
    distinctness_assessment = _distinctness_chain_assessment(
        ...
        continuation_v2_execution_summary=continuation_v2_execution_summary,
        continuation_v3_execution_summary=current_execution_summary,
    )
    ...
    future_posture_reason = (
        "the four-step governed work-loop chain is still accumulating distinct structural evidence under the narrow envelope, so the safe result remains to hold posture narrow and keep broader execution closed"
    ...
    next_template = (
        ...
        else "memory_summary.v4_governed_work_loop_evidence_snapshot_v4"
    ...
    question_answers = {
        "chain_still_accumulating_distinct_value": str(distinctness_assessment.get("classification", "")) == "accumulating_distinct_value",
        "new_containment_audit_materially_improves_evidence_picture": str(new_execution_signal.get("classification", ""))
        == "materially_improved_evidence_picture",
        "repeated_bounded_success_now_stronger_than_evidence_v2": str(
    ...
    artifact_path = _diagnostic_artifact_dir() / f"memory_summary_v4_governed_work_loop_evidence_snapshot_v3_{proposal['proposal_id']}.json"
    ...
    updated_work_loop_policy["governed_work_loop_evidence_review_schema"] = {
        ...
        "schema_version": "governed_work_loop_evidence_review_v3",
        "required_fields": [
    ...
    updated_work_loop_policy["last_work_loop_evidence_outcome"] = {
        ...
        "status": "hold_narrow_posture_after_evidence_v3",
    ...
    updated_current_state_summary.update(
        {
            ...
            "latest_governed_work_loop_evidence_outcome": "hold_narrow_posture_after_evidence_v3",
            ...
            "latest_governed_work_loop_operational_status": (
                "four_step_bounded_governed_work_loop_chain_reviewed"
                if bool(repeated_bounded_success.get("passed", False))
                else "four_step_bounded_governed_work_loop_chain_not_yet_clear"
            ),
            "latest_governed_work_loop_execution_outcome": str(
                dict(current_execution_summary.get("frontier_containment_result", {})).get("classification", "")
            ),
            "latest_governed_work_loop_readiness": (
                "ready_for_work_loop_candidate_screen_v4_later"
                if next_action_class == "bounded_candidate_search_later_supported"
                else "ready_for_work_loop_evidence_review_v4_later"
                if next_action_class == "bounded_evidence_review_later_supported"
    ...
    chain_state = {
        ...
        "direct_governed_work": dict(prior_chain_evidence_summary.get("current_work_loop_chain_state", {})).get(
            "direct_governed_work", {}
        ),
        "continuation_v1": dict(prior_chain_evidence_summary.get("current_work_loop_chain_state", {})).get(
            "continuation_v1", {}
        ),
        "continuation_v2": dict(prior_chain_evidence_summary.get("current_work_loop_chain_state", {})).get(
            "continuation_v2", {}
        ),
        "continuation_v3": {
            "loop_candidate_name": str(candidate_reviewed.get("loop_candidate_name", "")),
            ...
            "frontier_drift_status": str(
                dict(current_execution_summary.get("frontier_containment_result", {})).get(
    ...
    evidence_inputs_used = {
        ...
        "governed_work_loop_candidate_screen_v3": str(candidate_screen_artifact_path),
        "governed_work_loop_continuation_admission_v3": str(continuation_admission_artifact_path),
        ...
        "governed_work_loop_evidence_v2": str(prior_chain_evidence_artifact_path),
        ...
        "governed_work_loop_continuation_execution_v3": str(current_execution_artifact_path),
    ...
    operator_readable_conclusion = (
        "The four-step governed work-loop chain is still accumulating distinct structural evidence under the narrow envelope; the frontier-containment execution materially improved the evidence picture without widening posture, so the safe next move is another bounded candidate search later while keeping the gate closed."
        if next_action_class == "bounded_candidate_search_later_supported"
        else "The frontier-containment execution stayed clean and bounded, but the safest next move is still another evidence-only review later rather than another candidate search."
        if next_action_class == "bounded_evidence_review_later_supported"
    ...
    ledger_event = {
        "event_id": f"governed_work_loop_evidence_snapshot_v3::{proposal['proposal_id']}",
        "timestamp": _now(),
        "event_type": "governed_work_loop_evidence_snapshot_v3_materialized",
        ...
        "artifact_paths": {
            ...
            "governed_direct_work_execution_v1": str(direct_work_execution_artifact_path),
            "governed_capability_use_evidence_v1": str(capability_use_evidence_artifact_path),
            "governed_work_loop_evidence_v3": str(artifact_path),
        },
    ...
    artifact_payload = {
        ...
        "template_name": "memory_summary.v4_governed_work_loop_evidence_snapshot_v3",
        ...
        "comparison_references": {
            ...
            "memory_summary.v4_governed_work_loop_evidence_snapshot_v2": _artifact_reference(prior_chain_evidence_snapshot, latest_snapshots),
            ...
            "proposal_learning_loop.v4_governed_work_loop_governance_recommendation_frontier_containment_audit_v1": _artifact_reference(current_execution_snapshot, latest_snapshots),
        ...
        "governed_work_loop_evidence_v3_summary": {
            "snapshot_identity_context": {
                "template_name": "memory_summary.v4_governed_work_loop_evidence_snapshot_v3",
                "proposal_id": str(proposal.get("proposal_id", "")),
                "evaluation_semantics": str(proposal.get("evaluation_semantics", "")),
            ...
            "recommended_next_action": {
                ...
                "reason": (
                    "the four-step chain is still structurally yielding under the narrow envelope, so another bounded candidate search later is the cleanest next move"
                    if next_action_class == "bounded_candidate_search_later_supported"
                    else "another evidence-only review later is safer than another candidate search right now"
                    if next_action_class == "bounded_evidence_review_later_supported"
            ...
            "current_work_state_summary": {
                ...
                "continuation_v3_execution_summary_present": bool(current_execution_payload),
            ...
            "why_governance_remains_source_of_truth": {
                ...
                "reason": "the v3 evidence review is derived from directive, bucket, branch, self-structure, work-loop policy, work-loop posture, candidate-screen v3, continuation-admission v3, evidence v1, evidence v2, direct-work evidence, continuation executions, frontier-containment execution, and capability-use evidence artifacts rather than from execution code",
            },
        },
        "analytics_context": {
        ...
        "observability_gain": {
            "passed": True,
            "reason": "the governed work-loop chain now has an explicit governance-owned v3 evidence review consolidating the frontier-containment execution into a four-step bounded operator-readable artifact",
            "artifact_paths": {
        ...
        "ambiguity_reduction": {
            "passed": True,
            "score": 0.99,
            "reason": "the v3 evidence review separates chain-level structural value, frontier-quality signal, posture pressure, and later bounded-next-step support in one grounded decision artifact",
        },
        "safety_neutrality": {
            "passed": True,
        ...
        "diagnostic_conclusions": {
            ...
            "new_containment_audit_materially_changed_evidence_picture": bool(new_execution_signal.get("passed", False)),
    ...
    return {
        ...
        "shadow_contract": "diagnostic_probe",
        "proposal_semantics": "diagnostic",
        "reason": "diagnostic shadow passed: the four-step governed work-loop chain now has a v3 evidence review that keeps posture narrow, keeps the gate closed, and determines whether bounded candidate search later is still supported",
