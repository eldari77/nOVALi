...
def _load_current_bootstrap_context(package_root: Path) -> dict[str, Any]:
    directive_path = package_root / "directives" / "novali_v5_bootstrap_directive_v1.json"
    if directive_path.exists():
        try:
            ...
            pass
    return {
        "active_branch": "novali-v5",
        "completed_reference_branch": "novali-v4",
        "reference_operator_surface_branch": "novali-v4",
        "frozen_fallback_reference_version": "novali-v3",
        "additional_reference_versions": ["novali-v2"],
        ...
        "branch_transition_reason": (
            "novali-v5 remains the active branch while directive-first bootstrap and the standalone Docker handoff "
            "path stay governed under the held baseline."
        ),
        "branch_transition_status": "novali_v5_active_standalone_operator_handoff",
        "branch_transition_timestamp": _now(),
        "branch_transition_id": "branch_transition::novali_v5::standalone_handoff",
...
def build_standalone_directive_payload(
    *,
    ...
    directive_token = _slug(directive_id, fallback="novali_v5_standalone_directive")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Generate a formal NOVALIDirectiveBootstrapFile scaffold for the standalone novali-v5 operator path. "
