...
def _prior_analytics_observation(prior_execution_payload: dict[str, Any]) -> dict[str, Any]:
    direct_work_summary = dict(prior_execution_payload.get("governed_direct_work_execution_summary", {}))
...
def _build_delta_audit(
    *,
    ...
    prior_digests = _prior_digest_map(prior_direct_work_execution_payload)
    prior_analytics = _prior_analytics_observation(prior_direct_work_execution_payload)

    deltas = {
        "self_structure_ledger_row_delta": int(len(self_structure_ledger)) - int(dict(prior_digests.get(str(SELF_STRUCTURE_LEDGER_PATH), {})).get("row_count", 0) or 0),
        "intervention_ledger_row_delta": int(len(intervention_ledger)) - int(dict(prior_digests.get(str(INTERVENTION_LEDGER_PATH), {})).get("row_count", 0) or 0),
        "directive_history_row_delta": int(len(directive_history)) - int(dict(prior_digests.get(str(DIRECTIVE_HISTORY_PATH), {})).get("row_count", 0) or 0),
        "proposal_count_delta": int(analytics.get("proposal_count", 0) or 0) - int(prior_direct_work_execution_payload.get("analytics_context", {}).get("proposal_count", 0) or 0),
        "ranked_recommendation_count_delta": int(len(list(recommendations.get("all_ranked_proposals", [])))) - int(prior_analytics.get("ranked_recommendation_count", 0) or 0),
    }
    artifact_presence = {name: str(path.resolve()) for name, path in work_loop_artifact_paths.items() if path.exists()}
    material_delta_detected = any(int(value) > 0 for value in deltas.values()) or bool(artifact_presence)
    checks = {
        ...
        "work_loop_progression_state_current": _check(
            ...
            and bool(current_state_summary.get("governed_work_loop_candidate_screening_in_place", False))
            and bool(current_state_summary.get("governed_work_loop_continuation_admission_in_place", False))
            and str(current_state_summary.get("latest_governed_work_loop_continuation_outcome", "")) == "admissible_for_first_governed_work_loop_continuation",
            "current state still reflects the work-loop policy, candidate screen, and continuation admission chain",
        ),
        ...
        "loop_continuation_path_separation_holds": _check(
            bool(path_separation.get("continue_as_direct_work", False))
        ...
        "ledger_growth_since_prior_direct_work_detected": _check(
            bool(material_delta_detected),
            "governance ledgers or work-loop artifacts advanced since the prior direct-work execution",
            observed={"deltas": deltas, "artifact_presence": artifact_presence},
        ),
    ...
    return {
        "audit_focus": "governance_ledger_consistency_delta_audit",
        "checks": checks,
        "delta_observations": {"deltas": deltas, "work_loop_artifacts_present": artifact_presence},
        "high_signal_observations": [
            "governance ledgers and work-loop artifacts show bounded progression beyond the prior direct-work step"
            if material_delta_detected
            else "no material governance-ledger delta was detected beyond the prior direct-work step",
            "loop continuation remained distinct from prior direct work and stayed separate from capability-use and reopen paths",
        ],
        "summary": {
            ...
            "delta_signal_count": int(sum(1 for value in deltas.values() if int(value) > 0)),
            "material_delta_detected": bool(material_delta_detected),
            "alignment_status": (
                "coherent_with_material_delta"
                if not failed and material_delta_detected
                else "coherent_but_flat"
                if not failed
...
def run_probe(cfg, proposal, *, rounds, seeds):
    ...
    if not all(
        ...
        return _failure(
            proposal,
            "shadow work-loop continuation failed: governance substrate, work-loop policy, work-loop candidate screen, continuation admission, direct-work chain, and capability-use evidence artifacts are required",
        )
    ...
    if str(last_continuation_admission.get("status", "")) != "admissible_for_first_governed_work_loop_continuation":
        return _failure(
            proposal,
            "shadow work-loop continuation failed: the best screened loop candidate is not currently admitted for first governed work-loop continuation",
        )
    if str(current_state_summary.get("latest_governed_work_loop_execution_readiness", "")) != "ready_for_shadow_work_loop_continuation_execution":
        return _failure(
            proposal,
            "shadow work-loop continuation failed: self-structure does not currently mark the loop continuation path as ready for shadow execution",
        )
    ...
    if not all(
        ...
        return _failure(
            proposal,
            "shadow work-loop continuation failed: one or more governing artifact paths for loop continuation could not be resolved",
        )
    ...
    work_loop_candidate_screen_summary = dict(
        work_loop_candidate_screen_payload.get("governed_work_loop_candidate_screen_summary", {})
    )
    continuation_admission_summary = dict(
        continuation_admission_payload.get("governed_work_loop_continuation_admission_summary", {})
    )
    ...
    if not all([work_loop_candidate_screen_summary, continuation_admission_summary, direct_work_execution_payload]):
        return _failure(
            proposal,
            "shadow work-loop continuation failed: governing summary payloads for loop continuation execution are incomplete",
        )
    ...
    delta_audit = _build_delta_audit(
    ...
    next_template = "memory_summary.v4_governed_work_loop_evidence_snapshot_v1"
    artifact_path = _diagnostic_artifact_dir() / (
        f"proposal_learning_loop_v4_governed_work_loop_governance_ledger_consistency_delta_audit_v1_{proposal['proposal_id']}.json"
    )
    ...
    distinct_value_read = {
        "passed": bool(dict(delta_audit.get("summary", {})).get("material_delta_detected", False)),
        "value": (
            "medium" if bool(dict(delta_audit.get("summary", {})).get("material_delta_detected", False)) else "low"
        ),
        "reason": (
            "the continuation produced a bounded governance-ledger delta audit with real ledger and work-loop progression beyond the prior direct-work step"
            if bool(dict(delta_audit.get("summary", {})).get("material_delta_detected", False))
            else "the continuation did not surface enough new governance-ledger delta to justify distinct loop value over the prior narrow direct-work step"
        ),
    }
    directive_support_value = {
        "passed": True,
        "value": "high",
        "reason": "the delta audit produced a bounded governance-ledger continuation report that directly supports directive-bound work-loop control",
    }
    hidden_capability_pressure = {
        "passed": True,
        "value": "none",
        "reason": "the continuation remained governance-ledger maintenance and did not drift toward capability use, paused-line reopen, hidden development, or silent broadening",
    }
    review_trigger_status = {
        ...
        "scope_expands_beyond_governance_ledger_consistency_delta_audit": False,
    ...
    deprecation_trigger_status = {
        ...
        "governance_observability_drops_below_medium": str(
            dict(loop_candidate_record.get("screen_dimensions", {})).get("governance_observability", "")
        )
        != "high",
        "repeated_low_distinct_value_loop_continuation_without_new_delta": not bool(distinct_value_read.get("passed", False)),
        "duplicate_better_governed_path_supersedes_this_continuation": False,
    }
    continuation_accounting = {
        ...
        "continuation_summary": "governance ledger consistency delta audit",
        ...
        "usefulness_signal_summary": {
            "passed_check_count": int(dict(delta_audit.get("summary", {})).get("passed_check_count", 0) or 0),
            "failed_check_count": int(dict(delta_audit.get("summary", {})).get("failed_check_count", 0) or 0),
            "delta_signal_count": int(dict(delta_audit.get("summary", {})).get("delta_signal_count", 0) or 0),
            "material_delta_detected": bool(dict(delta_audit.get("summary", {})).get("material_delta_detected", False)),
        },
    ...
    artifact_payload = {
        ...
        "template_name": "proposal_learning_loop.v4_governed_work_loop_governance_ledger_consistency_delta_audit_v1",
        ...
        "comparison_references": {
            ...
            "memory_summary.v4_governed_work_loop_candidate_screen_snapshot_v1": _artifact_reference(work_loop_candidate_screen_snapshot, latest_snapshots),
            "memory_summary.v4_governed_work_loop_continuation_admission_snapshot_v1": _artifact_reference(work_loop_continuation_admission_snapshot, latest_snapshots),
        ...
        "governed_work_loop_continuation_execution_summary": {
            ...
            "what_the_continuation_did": [
                "read only the admitted trusted local governance artifacts relevant to governance-ledger consistency deltas",
                "computed a bounded governance-ledger delta audit against the prior direct-work execution rather than repeating the prior coherence-audit shape",
                "materialized a shadow-only governed work-loop continuation artifact with full continuation accounting and trigger status",
            ],
            "local_governance_sources_used": {
                "source_artifact_paths": source_artifact_paths,
                "governance_source_digests": governance_source_digests,
            },
            "delta_audit_artifact_produced": {
                "bounded_output_artifact_path": str(artifact_path),
                "governance_ledger_delta_audit_output": delta_audit,
            },
            ...
            "why_governance_remains_source_of_truth": {
                ...
                "reason": "the run consumed directive, bucket, branch, self-structure, work-loop policy, candidate-screen, continuation-admission, and prior direct-work governance artifacts as read-only authority while performing only the admitted loop-continuation audit",
            },
        },
        "analytics_context": {
        ...
        "activation_analysis_usefulness": {
            "passed": True,
            "reason": "the run proves that NOVALI can execute an admitted governed work-loop continuation without collapsing into direct-work repetition, capability use, hidden development, or silent broadening",
        },
        "ambiguity_reduction": {
            "passed": True,
            "score": 0.99,
        ...
        "later_selection_usefulness": {
            ...
            "reason": "the next step should review the governed work-loop continuation evidence before broadening loop execution patterns",
        },
    ...
    updated_work_loop_policy["last_work_loop_continuation_execution_outcome"] = {
        "loop_candidate_id": str(dict(continuation_admission_summary.get("candidate_evaluated", {})).get("loop_candidate_id", "")),
        "loop_candidate_name": primary_candidate_name,
        "work_loop_continuation_execution_outcome": "shadow_work_loop_continuation_completed",
    ...
    updated_current_state_summary.update(
        {
            ...
            "latest_governed_work_loop_execution_outcome": "shadow_work_loop_continuation_completed",
            "latest_governed_work_loop_execution_readiness": "ready_for_work_loop_evidence_review",
            "latest_governed_work_loop_operational_status": (
                "operational_bounded_work_loop_continuation_proven"
    ...
    _append_jsonl(
        SELF_STRUCTURE_LEDGER_PATH,
        {
            "event_id": f"governed_work_loop_governance_ledger_consistency_delta_audit_v1::{proposal['proposal_id']}",
            "timestamp": _now(),
            "event_type": "governed_work_loop_governance_ledger_consistency_delta_audit_v1_materialized",
            ...
            "artifact_paths": {
                ...
                "governed_work_loop_continuation_execution_v1": str(artifact_path),
            },
    ...
    return {
        ...
        "shadow_contract": "governed_work_loop_continuation",
        "proposal_semantics": "shadow_work_loop_continuation",
        "reason": "shadow work-loop continuation passed: the admitted governance ledger consistency delta audit executed inside the approved continuation envelope without repeating the prior direct-work shape or reopening capabilities",
